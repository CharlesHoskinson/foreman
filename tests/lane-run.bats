...
@test "lane-run resolves launcher and WSL preflight from FOREMAN_TOOL_ROOT; admission uses a bound profile" {
...
EOF
  ...
  # R7B2: vendor admission reads the profile-bound record only.
...
JSON
  profile_result="$(node "$SCRIPTS/../runtime/dist/credential-profile.js" init \
    --state-root "$FOREMAN_HOME" --worktree "$WT" \
    --profile grok-default --vendor grok)"
  profile_identity="$(jq -er '.profileIdentity' <<<"$profile_result")"
  profile_preflight="$FOREMAN_HOME/credential-profiles/grok-default/preflight"
  mkdir -p "$profile_preflight"
  jq -cn --arg profileIdentity "$profile_identity" \
    ...
    > "$profile_preflight/grok.json"
  chmod 600 "$profile_preflight/grok.json"
