...
| Auth status | `codex login status`, distinguish ChatGPT-managed auth | `claude auth status` JSON, exit 0/1 | no documented status command; cached-auth inspection plus bounded read-only live probe | `grok login`; use authenticated `grok models` as a bounded read-only probe |
