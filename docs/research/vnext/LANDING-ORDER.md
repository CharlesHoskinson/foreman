...
| **S4** | `decision-lineage-and-telemetry` → `three-outcome-verdicts` → `round-ownership-default` → `lane-ownership-and-reaping` → `doctrine-reality-drift` (serial on `config.toml`/`lane-run.sh`) | Telemetry precedes every comparative claim in the release. Round ownership needs the S1 lock. |
