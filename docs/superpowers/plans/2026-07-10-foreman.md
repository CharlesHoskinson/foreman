...
- [ ] **Step 1: Install prerequisites in WSL2 and set repo git identity**
...
- [ ] **Step 2: Write `LICENSE` (MIT, standard text, copyright 2026 Charles Hoskinson) and `.gitignore`**
...
- [ ] **Step 3: Write the failing test** — `tests/test_common.bats`:
...
- [ ] **Step 5: Write `skills/foreman/scripts/lib/common.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_task_new.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/task-new.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_gate_eval.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/gate-eval.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_docker_run.bats`:
...
- [ ] **Step 3: Write `sandbox/docker-run.sh`**
...
- [ ] **Step 4: Write `sandbox/Dockerfile.worker`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_adapters.bats`:
...
- [ ] **Step 4: Write `skills/foreman/scripts/adapters/verdict.schema.json`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_worker_run.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/worker-run.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_checks_run.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/checks-run.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_evidence_collect.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/evidence-collect.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_audit_run.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/audit-run.sh`**
...
- [ ] **Step 1: Write the failing test** — `tests/test_pr_open.bats`:
...
- [ ] **Step 3: Write `skills/foreman/scripts/pr-open.sh`**
...
- [ ] **Step 1: Write the red-team tests** — `tests/test_redteam.bats`:
...
- [ ] **Step 1: Write `skills/foreman/SKILL.md`**
...
- [ ] **Step 2: Write the four reference files**
...
- [ ] **Step 3: Validate skill constraints**
...
- [ ] **Step 1: Write the failing test** — `tests/test_install.bats`:
...
- [ ] **Step 4: Write `config/foreman.toml.example`** (copy of the fixture config from Task 1 plus comments and the `[audit]` section):
...
- [ ] **Step 5: Write `README.md`** — sections: what foreman is (3 sentences + the stage-flow diagram from spec §6); requirements (WSL2, Docker, ≥2 vendor CLIs); quickstart (`git clone`, `./install.sh`, `docker build -t foreman-worker:latest -f sandbox/Dockerfile.worker sandbox/`, copy config example, invoke `/foreman` or `$foreman`); security model summary table (from spec §7) with the honest-limits caveat; links to the spec and references.
...
- [ ] **Step 4: Record results in `docs/dogfood-log.md`**
