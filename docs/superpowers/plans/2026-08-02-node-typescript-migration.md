...
**Goal:** Run a separately approved post-v0.2.8.2 migration to Node.js 24 and
TypeScript, migrate the existing Python, Bash, and Bun product cores, and
prevent new legacy-runtime debt. This plan does not expand the current release.
...
- Apply the current `ROADMAP.md` release scope before this dated plan.
...
- Select a release number only after separate scope approval. Do not tag that
  release until all final gates pass at one unchanged commit.
