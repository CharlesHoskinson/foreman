...
export function runNativeCodingQualification(selection: ProviderQualificationSelection, live: LiveProviderContext, fixture?: NativeQualificationFixturePorts): Effect.Effect<QualificationReportV1, ProviderFailure> {
    return Effect.scoped(Effect.gen(function* () {
        ...
        if (!fixture) {
            ...
            if (selected.nativeProfileDirectory || (!selected.environment && !(selection.transportId === 'codex-app-server' && selected.chatgpt) && !(selection.transportId === 'grok-acp' && selected.grokLogin)))
                return yield* Effect.fail(failure('The isolated coding boundary requires an explicit environment credential or host-managed native login. Native profile directories are not mounted.'));
        }
        ...
        let report = yield* Effect.scoped(Effect.gen(function* () {
            ...
            return yield* runQualification({ request, requiredCapabilities: selection.requiredCapabilities, binding: selection.binding }, { transport, now: Date.now, onToolRequest: event => permissions.handle(event, transport), assess: (capability, events) => Effect.gen(function* () {
                    ...
                    if (capability === 'tools') {
                        ...
                        return { passed: complete && acknowledged, reason: !acknowledged ? 'No durable host permission result was acknowledged.' : !complete ? 'A durable host permission result was acknowledged, but no validated terminal result was received.' : 'The original host retained a permission result before acknowledging the native request.' };
                    }
