...
async function integration(source:string, variant:'dispatch'|'retry'|'predicate'|'read-result'|'needs-action'|'print'|'resource-denied'|'concurrency-denied') {
 ...
 try {
  ...
  const result=await Effect.runPromise(Effect.gen(function*(){
   ...
   const effectiveLedger=variant==='predicate'?{...ledger,executeChild:(input:Parameters<typeof ledger.executeChild>[0])=>Effect.sync(()=>{if(input.operation._tag==='ReserveAction')assert.equal(input.operation.originReservationId,input.operation.reservationId);childActions.push(input.operation._tag==='ReserveAction'?input.operation.reservationAction:'other');return {decision:{_tag:'Accepted' as const,events:[]},state:{} as import('./execution-terminal-policy.js').ExecutionFamilyStateV2};})}:ledger;
