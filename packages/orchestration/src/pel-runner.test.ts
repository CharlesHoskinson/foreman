...
test('M6 race settles a joined loser with confirmed external completion before its interrupted local receipt',async()=>{const ready=Effect.runSync(Deferred.make<void>());let cleaned=false;const fixture=runFixture('(fm/race :tasks [(lambda [] (fm/checkpoint "winner")) (lambda [] (fm/checkpoint "loser"))] :winner "first-valid")',ports=>{const checkpoint=ports.controls.get('fm/checkpoint')!;return {...ports,controls:new Map([...ports.controls,['fm/checkpoint',{execute:(request,context,parent,driver)=>request.boundArguments.name?.tag==='string'&&request.boundArguments.name.value==='winner'?Effect.zipRight(Deferred.await(ready),checkpoint.execute(request,context,parent,driver)):Effect.gen(function*(){const ref=yield* putPelRunData(context.binding,{stage:'fixture-completed'});yield* appendPelRecord(context.binding,'pel.effect.intent.v1',{effect:context.effect,argumentsRef:ref,expectedResultSchemaId:request.expectedResultSchemaId,preparationDigest:'a'.repeat(64),reservation:null,usageReservation:null});yield* appendPelRecord(context.binding,'pel.effect.observed.v1',{effectId:context.effect.effectId,observationRef:ref,providerIdentity:null,externalOutcome:'confirmed-complete'});yield* Effect.addFinalizer(()=>Effect.sync(()=>{cleaned=true;}));yield* Deferred.succeed(ready,undefined);return yield* Effect.never;})}]])};});try{const {result,events}=await Effect.runPromise(fixture.run);assert.equal(cleaned,true);assert.equal(result.state,'succeeded');assert.deepEqual(result.usage.unresolvedEffectIds,[]);const replay=replayPelRun(events);assert.ok(replay.ok);assert.equal(replay.value.records.filter(row=>row.type==='pel.race.decision.v1').length,1);assert.ok([...replay.value.results.values()].some(row=>replay.value.observations.get(row.effect.effectId)?.externalOutcome==='confirmed-complete'));}finally{rmSync(fixture.root,{recursive:true,force:true});}});
