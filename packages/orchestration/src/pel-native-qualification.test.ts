...
for(const scenario of ['exact','ignored-extra','late-write','auth-failure'] as const)test(`native coding qualification uses actual mount probes, durable permission, and exact changed bytes: ${scenario}`,async()=>{
