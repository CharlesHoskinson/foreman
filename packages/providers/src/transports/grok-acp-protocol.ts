...
export interface GrokAcpProtocolOptions {
...
/**
 * Internal ACP protocol implementation, not an admitted provider transport.
 * The public factory owns hard-budget admission. Protocol fixtures provide an
 * explicit process port here. This module must not be exported by the package.
 */
export function createGrokAcpProtocol(
  options: GrokAcpProtocolOptions,
): ProviderTransport {
  ...
  > =>
    Effect.gen(function* () {
      ...
      if (material.grokLogin && (nativeProfileDirectory || material.environment))
        return yield* Effect.fail(problem('AuthenticationRequired', 'credential.ambiguous'));
      const grokAuthJson = material.grokLogin
        ? yield* material.grokLogin.snapshot({ deadline: request.limits.deadline }) : undefined;
      if (grokAuthJson !== undefined) {
        if (!host.environment.HOME || !isAbsolute(host.environment.HOME))
          return yield* unsupported('credential.workerHome');
        environment.GROK_HOME = `${host.environment.HOME}/.grok`;
      }
      ...
      const connection = yield* options.process.open({
        cmd,
        cwd: host.cwd,
        environment,
        ...(grokAuthJson === undefined ? {} : { grokAuthJson }),
      ...
      const earlyCommandSessions: string[] = [];
      ...
      const rpc = (
        ...
        Effect.gen(function* () {
          if (terminalFailure) return yield* Effect.fail(terminalFailure);
          const id = nextId++;
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            const tool: ToolRequestV1 = {
              ...
              name: call.kind === "other" && record(call.rawInput) &&
                call.rawInput.variant === "ListDir" && typeof call.rawInput.target_directory === "string" &&
                call.rawInput.target_directory.length > 0 ? "read" : call.kind,
          ...
          if (request.toolPolicy.mode === "native-coding" && !session.cancelRequested) {
            const kind = record(call) && ["read", "edit", "delete", "move", "search", "execute", "think", "fetch", "other"].includes(String(call.kind)) ? String(call.kind) : "unknown";
            const raw = record(call) && record(call.rawInput) ? call.rawInput : {};
            ...
            return yield* Effect.fail({ ...problem("UnsupportedCapability", "permission.request"),
        ...
        connection.events.pipe(
          Stream.runForEach((message) =>
            Effect.gen(function* () {
              ...
              // Grok emits command-list metadata during session/new. Retain only
              // bounded identity receipts until its response binds the session.
              // Unbound text, tools, and permissions must never gain authority.
              if (!session && creatingSession && record(message.params) &&
                  typeof message.params.sessionId === "string" &&
                  message.params.sessionId.length > 0 && message.params.sessionId.length <= 4096 &&
                ...
                if (earlyCommandSessions.length >= 64 || earlyCommandBytes > 65536)
                  return yield* Effect.fail(problem("MalformedEvent", "session.earlyUpdates"));
              ...
              if (
                ...
                if (
                  ...
                  return yield* Effect.fail(
                    ...
                    ), message: `Grok ACP refused unapproved tool progress: permission=${
                      !session.permissionResults.has(update.toolCallId) ? "absent"
                        : session.permissionResults.get(update.toolCallId)?.resultDigest === undefined ? "pending" : "denied"
      ...
      const authMethod =
        ...
        methods.some((m) => m.id === "xai.api_key")
          ? "xai.api_key"
          : (nativeProfileDirectory || grokAuthJson !== undefined) &&
      ...
      const created = yield* rpc("session/new", {
        cwd: host.cwd,
        mcpServers: [],
        _meta: { systemPromptOverride: request.trustedInstructions },
      });
      ...
      if (earlyCommandSessions.some(id => id !== created.sessionId))
        return yield* Effect.fail(problem("MalformedEvent", "session.earlyUpdates.sessionId"));
          ...
          Effect.flatMap((result) =>
            Effect.gen(function* () {
              ...
              if (result.stopReason === "end_turn") {
                ...
                if (!record(meta))
                  return yield* Effect.fail(problem("MalformedEvent", "prompt.structuredOutput"));
                if (Object.hasOwn(meta, "structuredOutputError"))
                  return yield* Effect.fail(problem("MalformedEvent", "prompt.structuredOutputError"));
                if (!Object.hasOwn(meta, "structuredOutput"))
                  return yield* Effect.fail(problem("MalformedEvent", "prompt.structuredOutput"));
                if (meta.sessionId !== created.sessionId || meta.modelId !== model)
                  return yield* Effect.fail(problem("MalformedEvent", "prompt.structuredOutput.identity"));
                if (!jsonValue(meta.structuredOutput))
                  return yield* Effect.fail(problem("MalformedEvent", "prompt.structuredOutput"));
                const finalOutput = canonicalize(meta.structuredOutput);
                if (Buffer.byteLength(output) + Buffer.byteLength(finalOutput) > request.limits.maxOutputBytes)
                  return yield* Effect.fail(problem("OutputIncomplete", "limits.maxOutputBytes"));
  ...
  return {
    ...
    sendToolResult: (identity,result) => Effect.suspend(()=>{
      ...
      return submitToolResult(permissions,identity,result,()=>Effect.gen(function*(){
        ...
        // submitToolResult has verified the durable decision. The peer can
        // react before the write callback returns, so bind that authority now.
        pending.granted=content.decision==='accept';
        yield* session.connection.send({jsonrpc:'2.0',id:pending.rpcId,result:{outcome}}).pipe(
          Effect.onError(() => Effect.sync(() => { pending.granted=false; })),
        );
