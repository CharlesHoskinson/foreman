...
var let_ = (map16) => dual(3, (self, name, f) => map16(self, (a) => ({
  ...a,
  [name]: f(a)
})));
var bindTo = (map16) => dual(2, (self, name) => map16(self, (a) => ({
  [name]: a
})));
var bind = (map16, flatMap12) => dual(3, (self, name, f) => flatMap12(self, (a) => map16(f(a), (b) => ({
  ...a,
  [name]: b
}))));
...
var get2 = unsafeGet2;
var getOrElse2 = /* @__PURE__ */ dual(3, (self, tag, orElse7) => {
  if (!self.unsafeMap.has(tag.key)) {
    return isReference(tag) ? getDefaultValue(tag) : orElse7();
  }
  return self.unsafeMap.get(tag.key);
});
...
var ArrayNode = class _ArrayNode {
  ...
  children;
  _tag = "ArrayNode";
  constructor(edit, size10, children3) {
    this.edit = edit;
    this.size = size10;
    this.children = children3;
  }
  modify(edit, shift2, f, hash2, key, size10) {
    ...
    if (isEmptyNode(child) && !isEmptyNode(newChild)) {
      ...
      if (count <= MIN_ARRAY_NODE) {
        return pack(edit, count, frag, children3);
      }
...
function pack(edit, count, removed, elements) {
  const children3 = new Array(count - 1);
  let g = 0;
  let bitmap = 0;
...
var parallelCollectionCombine = (self, that) => new ParallelImpl(reduce5(self.map, that.map, (map16, value, key) => set3(map16, key, match2(get7(map16, key), {
  onNone: () => value,
...
var sequentialCollectionMake = (map16) => new SequentialImpl(map16);
var sequentialCollectionCombine = (self, that) => new SequentialImpl(reduce5(that.map, self.map, (map16, value, key) => set3(map16, key, match2(get7(map16, key), {
  onNone: () => empty4(),
    ...
    case OP_ZIP_WITH: {
      return suspend(() => pipe(fromFlatLoop(flat, prefix, op.left, split), either2, flatMap7((left3) => pipe(fromFlatLoop(flat, prefix, op.right, split), either2, flatMap7((right3) => {
        ...
        if (isRight2(left3) && isRight2(right3)) {
          ...
          const [lefts, rights] = extend(fail11, fail11, pipe(left3.right, map2(right2)), pipe(right3.right, map2(right2)));
...
var unsafeForkAs = (self, map16, fiberId3) => {
...
var filterOrElse = /* @__PURE__ */ dual(3, (self, predicate, orElse7) => flatMap7(self, (a) => predicate(a) ? succeed(a) : orElse7(a)));
...
var tryPromise = (arg) => {
  ...
  const fail11 = (e) => catcher ? failSync(() => catcher(e)) : fail2(new UnknownException(e, "An unknown error occurred in Effect.tryPromise"));
...
var dump = (self) => map8(self.status, (status3) => ({
  id: self.id(),
  status: status3
}));
var dumpAll = (fibers) => forEachSequential(fibers, dump);
var fail4 = (error) => done5(fail3(error));
var failCause4 = (cause3) => done5(failCause2(cause3));
var fromEffect = (effect2) => map8(exit(effect2), done5);
var id = (self) => self.id();
...
var map10 = /* @__PURE__ */ dual(2, (self, f) => mapEffect(self, (a) => sync(() => f(a))));
...
var mapFiber = /* @__PURE__ */ dual(2, (self, f) => map8(self.await, match5({
...
var orElse2 = /* @__PURE__ */ dual(2, (self, that) => ({
  ...
  await: zipWith2(self.await, that.await, (exit1, exit22) => isSuccess(exit1) ? exit1 : exit22),
  children: self.children,
  inheritAll: zipRight(that.inheritAll, self.inheritAll),
  poll: zipWith2(self.poll, that.poll, (option1, option22) => {
    switch (option1._tag) {
      case "None": {
        return none2();
      }
...
var orElseEither = /* @__PURE__ */ dual(2, (self, that) => orElse2(map10(self, left2), map10(that, right2)));
...
var pretty2 = (self) => flatMap7(currentTimeMillis2, (now) => map8(dump(self), (dump3) => {
...
var succeed4 = (value) => done5(succeed2(value));
...
var none8 = /* @__PURE__ */ globalValue("effect/Supervisor/none", () => fromEffect2(void_));
...
var raceAll = (all6) => withFiberRuntime((state, status3) => async_((resume2) => {
  const fibers = /* @__PURE__ */ new Set();
  let winner;
...
var fiberAll = (fibers) => {
  const _fiberAll = {
    ...
    [FiberTypeId]: fiberVariance2,
    id: () => fromIterable(fibers).reduce((id3, fiber) => combine3(id3, fiber.id()), none4),
...
var invokeWithInterrupt = (self, entries2, onInterrupt3) => fiberIdWith((id3) => ensuring(flatMap7(forkDaemon(interruptible2(self)), (processing) => async_((cb) => {
...
var makeCacheState = (map16, keys5, accesses, updating, hits, misses) => ({
  map: map16,
  keys: keys5,
  accesses,
  updating,
...
var supervised = /* @__PURE__ */ dual(2, (self, supervisor) => {
  const supervise2 = fiberRefLocallyWith(currentSupervisor, (s) => s.zip(supervisor));
  return supervise2(self);
});
...
var zipWithFiber = /* @__PURE__ */ dual(3, (self, that, f) => ({
  ...
  await: pipe(self.await, flatten4, zipWithOptions(flatten4(that.await), f, {
    concurrent: true
  }), exit),
  children: self.children,
  inheritAll: zipRight(that.inheritAll, self.inheritAll),
  poll: zipWith2(self.poll, that.poll, (optionA, optionB) => pipe(optionA, flatMap((exitA) => pipe(optionB, map((exitB) => zipWith3(exitA, exitB, {
    onSuccess: f,
    onFailure: parallel
  })))))),
...
var void_4 = void_3;
var zip4 = zipFiber;
var zipLeft2 = zipLeftFiber;
var zipRight2 = zipRightFiber;
var zipWith4 = zipWithFiber;
...
var unsafeFork3 = /* @__PURE__ */ makeDual((runtime4, self, options) => {
  ...
  let effect2 = self;
  if (options?.scope) {
    effect2 = flatMap7(fork2(options.scope, sequential2), (closeableScope) => zipRight(scopeAddFinalizer(closeableScope, fiberIdWith((id3) => equals(id3, fiberRuntime.id()) ? void_ : interruptAsFiber(fiberRuntime, id3))), onExit(self, (exit4) => close(closeableScope, exit4))));
  }
...
var die4 = (defect) => failCause6(die3(defect));
var dieSync2 = (evaluate2) => failCauseSync2(() => die3(evaluate2()));
var discard = (self) => map12(self, () => empty3());
...
var flatMap9 = /* @__PURE__ */ dual(2, (self, f) => match12(self, {
  onFailure: fail7,
  onSuccess: f
}));
...
var mockImpl = (tag, service3) => succeed6(tag, new Proxy({
  ...service3
}, {
...
var map12 = /* @__PURE__ */ dual(2, (self, f) => flatMap9(self, (context5) => succeedContext(f(context5))));
...
var retryUpdate = (schedule2, stateTag, error, state) => {
  return fromEffect4(stateTag, pipe(currentTimeMillis2, flatMap7((now) => pipe(schedule2.step(now, error, state), flatMap7(([state2, _, decision]) => isDone4(decision) ? fail2(error) : pipe(sleep2(millis(start2(decision.intervals) - now)), as({
    state: state2
  })))))));
};
...
var unwrapEffect = (self) => {
  ...
  return flatMap9(fromEffect4(tag, self), (context5) => get3(context5, tag));
};
var unwrapScoped = (self) => {
  const tag = GenericTag("effect/Layer/unwrapScoped/Layer.Layer<R1, E1, A>");
  return flatMap9(scoped2(tag, self), (context5) => get3(context5, tag));
};
...
var asVoid2 = (self) => map13(self, constVoid);
...
var checkEffect = /* @__PURE__ */ dual(2, (self, test) => makeWithState(self.initial, (now, input, state) => flatMap7(self.step(now, input, state), ([state2, out, decision]) => {
  ...
  return map8(test(input, out), (cont) => cont ? [state2, out, decision] : [state2, out, done7]);
})));
...
var map13 = /* @__PURE__ */ dual(2, (self, f) => mapEffect3(self, (out) => sync(() => f(out))));
...
var repeatOrElse_Effect = /* @__PURE__ */ dual(3, (self, schedule2, orElse7) => flatMap7(driver(schedule2), (driver2) => matchEffect(self, {
  onFailure: (error) => orElse7(error, none2()),
  onSuccess: (value) => repeatOrElseEffectLoop(provideServiceEffect(self, CurrentIterationMetadata, get11(driver2.iterationMeta)), driver2, (error, option3) => provideServiceEffect(orElse7(error, option3), CurrentIterationMetadata, get11(driver2.iterationMeta)), value)
})));
var repeatOrElseEffectLoop = (self, driver2, orElse7, value) => matchEffect(driver2.next(value), {
  onFailure: () => orDie(driver2.last),
  onSuccess: (b) => matchEffect(self, {
    onFailure: (error) => orElse7(error, some2(b)),
...
var retryOrElse_Effect = /* @__PURE__ */ dual(3, (self, policy, orElse7) => flatMap7(driver(policy), (driver2) => retryOrElse_EffectLoop(provideServiceEffect(self, CurrentIterationMetadata, get11(driver2.iterationMeta)), driver2, (e, out) => provideServiceEffect(orElse7(e, out), CurrentIterationMetadata, get11(driver2.iterationMeta)))));
var retryOrElse_EffectLoop = (self, driver2, orElse7) => {
  return catchAll(self, (e) => matchEffect(driver2.next(e), {
    onFailure: () => pipe(driver2.last, orDie, flatMap7((out) => orElse7(e, out))),
    ...
    if ("effect" in maker) {
      ...
      Object.defineProperty(TagClass, layerName, {
        get() {
          if (isFunction3) {
            return function() {
              return fromEffect4(TagClass, map14(maker.effect.apply(null, arguments), (_) => new this(_)));
            }.bind(this);
          }
          return layerCache ??= fromEffect4(TagClass, map14(maker.effect, (_) => new this(_)));
        }
      });
      ...
      Object.defineProperty(TagClass, layerName, {
        get() {
          if (isFunction3) {
            return function() {
              return scoped2(TagClass, map14(maker.scoped.apply(null, arguments), (_) => new this(_)));
            }.bind(this);
          }
          return layerCache ??= scoped2(TagClass, map14(maker.scoped, (_) => new this(_)));
        }
      });
...
function fnApply(options) {
  ...
  if (options.pipeables.length > 0) {
    try {
      ...
      effect2 = fnError ? failCause8(sequential(die(fnError), die(error))) : die5(error);
    }
  }
  let cache = false;
...
var span2 = (name, options) => {
  options = addSpanStackTrace(options);
  return scoped2(spanTag, options?.onEnd ? tap(makeSpanScoped(name, options), (span4) => addFinalizer((exit4) => options.onEnd(span4, exit4))) : makeSpanScoped(name, options));
};
...
function stripNodeArgv(argv) {
  ...
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("foreman-launch") || args2[0].includes("main"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
function formatVersionLine(nodeVersion = process.version) {
...
function parseArgs(argv) {
  ...
  const flagSlice = sepIdx === -1 ? argv : argv.slice(0, sepIdx);
  ...
  for (let i = 0; i < flagArgs.length; i++) {
    ...
    switch (a) {
      case "--timeout": {
        ...
        const v = Number(raw);
        if (raw === void 0 || !Number.isFinite(v) || v <= 0) {
          return {
            _tag: "UsageError",
...
function mapSuperviseExit(result) {
...
function formatHeartbeatLine(line) {
  const obj = {
    ts: line.ts,
...
function buildHeartbeatLine(input) {
  return {
    ...
    elapsed_s: Number(((input.nowMs - input.startedAtMs) / 1e3).toFixed(3))
  };
}
function validateHeartbeatLineText(text) {
  ...
  if (keys5.length !== expected.length || keys5.some((k, i) => k !== expected[i])) {
...
function firstValidHeartbeatLine(content) {
...
var DETACH_HANDOFF_BOUND_MS = 5e3;
...
function resolveLauncherPid(env, processPid) {
  ...
  if (raw === void 0 || raw === "") return processPid;
  const n = Number(raw);
...
function buildExecveEnv(baseEnv, overrides) {
  const merged = {};
...
function processGroupKillTarget(leaderPid) {
  return -leaderPid;
}
function buildTaskkillArgv(pid) {
  return ["taskkill.exe", "/PID", String(pid), "/T", "/F"];
}
function planTaskkill(pid) {
  return {
...
var ChildSpawner = class extends Context_exports.Tag("ChildSpawner")() {
};
var ProcessGroupTerminator = class extends Context_exports.Tag("ProcessGroupTerminator")() {
};
var WindowsTreeTerminator = class extends Context_exports.Tag("WindowsTreeTerminator")() {
};
var ExecveService = class extends Context_exports.Tag("ExecveService")() {
};
...
var LauncherClock = class extends Context_exports.Tag("LauncherClock")() {
};
var DetachSpawner = class extends Context_exports.Tag("DetachSpawner")() {
};
var StderrLog = class extends Context_exports.Tag("StderrLog")() {
};
var liveClock = {
...
var LiveClockLayer = Layer_exports.succeed(LauncherClock, liveClock);
function whichAbsolute(name, pathEnv) {
  if (!pathEnv) return null;
  const parts2 = pathEnv.split(process.platform === "win32" ? ";" : ":");
  for (const dir of parts2) {
    if (!dir) continue;
...
async function* nodeReadableToAsync(stream) {
  if (!stream) return;
...
var liveProcessGroupTerminator = {
  killGroup: (leaderPid, signal) => Effect_exports.sync(() => {
    try {
...
var liveWindowsTreeTerminator = {
  terminateTree: (req) => Effect_exports.sync(() => {
...
var liveExecve = {
  execve: (req) => Effect_exports.suspend(() => {
...
var liveUnshareProbe = {
  probe: () => Effect_exports.sync(() => {
...
var liveHeartbeatWriter = {
  reset: (path) => Effect_exports.try({
    try: () => {
      writeFileSync(path, "");
    },
    catch: (e) => ({
      _tag: "HeartbeatWriteError",
      message: e instanceof Error ? e.message : String(e)
    })
  }),
  appendLine: (path, line) => Effect_exports.try({
    try: () => {
  ...
  readText: (path) => Effect_exports.try({
...
var liveDetachSpawner = {
  spawnDetachedSelf: (argv) => Effect_exports.try({
    try: () => {
...
var LiveLauncherLayer = Layer_exports.mergeAll(
  Layer_exports.succeed(ChildSpawner, liveChildSpawner),
  Layer_exports.succeed(ProcessGroupTerminator, liveProcessGroupTerminator),
  ...
  Layer_exports.succeed(DetachSpawner, liveDetachSpawner),
  LiveClockLayer
);

// packages/launcher/src/supervise.ts
async function* emptyStream() {
}
async function collectPump(src, onChunk) {
  if (!src) return;
  for await (const chunk2 of src) {
    await onChunk(chunk2);
  }
}
function emitEvent(opts, nowMs, e) {
...
function runDetachHandoff(rawArgv, heartbeatFile, selfScriptArgv) {
  return Effect_exports.gen(function* () {
    ...
    const reset = yield* Effect_exports.either(hb.reset(heartbeatFile));
    if (reset._tag === "Left") {
      yield* log3.write(
    ...
    const spawned = yield* Effect_exports.either(detach.spawnDetachedSelf(childArgv));
...
`);
  ...
  const program = Effect_exports.gen(function* () {
    ...
    const superviseOpts = {
      cmd: args2.cmd,
      graceSecs: args2.graceSecs,
      heartbeatIntervalSecs: args2.heartbeatIntervalSecs,
      launcherPid,
      platform: process.platform,
      ...args2.timeoutSecs !== void 0 ? { timeoutSecs: args2.timeoutSecs } : {},
      ...args2.heartbeatFile !== void 0 ? { heartbeatFile: args2.heartbeatFile } : {}
    };
    const result = yield* supervise(superviseOpts).pipe(
      Effect_exports.mapError((e) => e)
    );
    ...
    Effect_exports.catchAll(
      (e) => Effect_exports.gen(function* () {
        ...
        yield* log3.write(`foreman-launch: launcher error: ${msg}`);
        return EXIT_LAUNCHER_ERROR;
      })
    )
  );
  return Effect_exports.runPromise(program.pipe(Effect_exports.provide(layer)));
}
