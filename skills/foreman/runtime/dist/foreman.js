...
var let_ = (map12) => dual(3, (self, name2, f) => map12(self, (a) => ({
  ...a,
  [name2]: f(a)
})));
var bindTo = (map12) => dual(2, (self, name2) => map12(self, (a) => ({
  [name2]: a
})));
var bind = (map12, flatMap10) => dual(3, (self, name2, f) => flatMap10(self, (a) => map12(f(a), (b) => ({
  ...a,
  [name2]: b
}))));
...
var keepDefectsAndElectFailures = (self) => match4(self, {
  onEmpty: none2(),
  onFail: (failure3) => some2(die(failure3)),
  onDie: (defect) => some2(die(defect)),
  onInterrupt: () => none2(),
  onSequential: mergeWith(sequential),
  onParallel: mergeWith(parallel)
  ...
  switch (op._tag) {
    ...
    case OP_HASHMAP: {
      return suspend(() => pipe(patch5(prefix, flat.patch), flatMap7((prefix2) => pipe(flat.enumerateChildren(prefix2), flatMap7((keys6) => {
        return pipe(keys6, forEachSequential((key) => fromFlatLoop(flat, concat(prefix2, of(key)), op.valueConfig, split)), map8((matrix) => {
          if (matrix.length === 0) {
            return of(empty8());
          }
          return pipe(transpose(matrix), map2((values3) => fromIterable6(zip(fromIterable(keys6), values3))));
        }));
      })))));
    }
    case OP_ZIP_WITH: {
...
var NativeSpan = class {
  ...
  events = [];
  links;
  constructor(name2, parent, context4, links, startTime, kind) {
    this.name = name2;
    this.parent = parent;
...
var nativeTracer = /* @__PURE__ */ make23({
  span: (name2, parent, context4, links, startTime, kind) => new NativeSpan(name2, parent, context4, links, startTime, kind),
  context: (f) => f()
});
...
var summarized = /* @__PURE__ */ dual(3, (self, summary6, f) => flatMap7(summary6, (start3) => flatMap7(self, (value) => map8(summary6, (end3) => [f(start3, end3), value]))));
...
var unsafeMakeSpan = (fiber, name2, options) => {
  ...
  if (disablePropagation) {
    ...
    span2 = tracer3.span(name2, parent, options.context ?? empty3(), links, timingEnabled ? clock3.unsafeCurrentTimeNanos() : bigint02, options.kind ?? "internal", options);
...
var withSpan = function() {
  ...
  if (dataFirst) {
    const self = arguments[0];
    return useSpan(name2, options, (span2) => withParentSpan(self, span2));
  }
  return (self) => useSpan(name2, options, (span2) => withParentSpan(self, span2));
};
...
var histogram4 = (key) => {
  ...
  const update5 = (value) => {
    ...
    while (from !== to) {
      ...
      const boundary2 = boundaries[mid];
      if (value <= boundary2) {
        to = mid;
      } else {
        from = mid;
  ...
  const getBuckets = () => {
    ...
    for (let i = 0; i < size9; i++) {
      const boundary2 = boundaries[i];
      const value = values3[i];
      cumulated = cumulated + value;
      builder[i] = [boundary2, cumulated];
    }
    return builder;
  };
  return make29({
...
var MetricRegistryImpl = class {
  ...
  getSummary(key) {
    ...
    if (value == null) {
      ...
      if (!pipe(this.map, has4(key))) {
        pipe(this.map, set4(key, summary6));
      }
      value = summary6;
    }
    return value;
...
var forEachParUnbounded = (self, f, batching) => suspend(() => {
  const as4 = fromIterable(self);
  const array4 = new Array(as4.length);
  const fn2 = (a, i) => flatMap7(f(a, i), (b) => sync(() => array4[i] = b));
  return zipRight(forEachConcurrentDiscard(as4, fn2, batching, false), succeed(array4));
});
...
var forEachParN = (self, n, f, batching) => suspend(() => {
  const as4 = fromIterable(self);
  const array4 = new Array(as4.length);
  const fn2 = (a, i) => map8(f(a, i), (b) => array4[i] = b);
  return zipRight(forEachConcurrentDiscard(as4, fn2, batching, false, n), succeed(array4));
});
...
var invokeWithInterrupt = (self, entries2, onInterrupt3) => fiberIdWith((id2) => ensuring(flatMap7(forkDaemon(interruptible2(self)), (processing) => async_((cb) => {
...
var withSpanScoped = function() {
  ...
  if (dataFirst) {
    const self = arguments[0];
    return flatMap7(makeSpanScoped(name2, addSpanStackTrace(options)), (span2) => provideService(self, spanTag, span2));
  }
  return (self) => flatMap7(makeSpanScoped(name2, addSpanStackTrace(options)), (span2) => provideService(self, spanTag, span2));
};
...
var makeCacheState = (map12, keys6, accesses, updating, hits, misses) => ({
  map: map12,
  keys: keys6,
  accesses,
  updating,
...
var unsafeMakeLatch = (open2) => new Latch(open2 ?? false);
var makeLatch = (open2) => sync(() => unsafeMakeLatch(open2));
...
var bindAll = /* @__PURE__ */ dual((args2) => isEffect(args2[0]), (self, f, options) => flatMap7(self, (a) => all2(f(a), options).pipe(map8((record2) => Object.assign({}, a, record2)))));
...
var unsafeFork3 = /* @__PURE__ */ makeDual((runtime4, self, options) => {
  ...
  let effect = self;
  if (options?.scope) {
    effect = flatMap7(fork2(options.scope, sequential2), (closeableScope) => zipRight(scopeAddFinalizer(closeableScope, fiberIdWith((id2) => equals(id2, fiberRuntime.id()) ? void_ : interruptAsFiber(fiberRuntime, id2))), onExit(self, (exit4) => close(closeableScope, exit4))));
  }
...
function parseFail(failure3) {
  return { [PARSE_FAIL]: true, failure: failure3 };
}
function isParseFail(v) {
  return typeof v === "object" && v !== null && v[PARSE_FAIL] === true;
}
function parseJsonRejectDuplicateKeys(text2) {
  let i = 0;
  const s = text2;
  let depth = 0;
  function skipWs() {
      ...
      if (c === "\\") {
        ...
        switch (e) {
          ...
          case "t":
            ...
            break;
          case "u": {
            if (i + 4 > s.length) return fail5();
            const hex = s.slice(i, i + 4);
            if (!/^[0-9a-fA-F]{4}$/.test(hex)) return fail5();
  ...
  function parseNumber() {
    ...
    if (!Number.isFinite(num)) return fail5();
    return num;
  }
  function parseValue() {
    skipWs();
    ...
    if (c === "t") {
      if (s.slice(i, i + 4) !== "true") return fail5();
      i += 4;
      return true;
    }
    if (c === "f") {
      if (s.slice(i, i + 5) !== "false") return fail5();
      i += 5;
      return false;
    }
    if (c === "n") {
      if (s.slice(i, i + 4) !== "null") return fail5();
      i += 4;
      return null;
    }
...
// packages/pel/dist/pel.js
import { createHash } from "node:crypto";
import { createHash as createHash2 } from "node:crypto";
import { createHash as createHash3 } from "node:crypto";
function combineSpans(first2, last3) {
  return {
    start: first2.start,
    end: last3.end,
    line: first2.line,
    column: first2.column,
    endLine: last3.endLine,
    endColumn: last3.endColumn
  };
}
...
var id = "pel-paper-v2-foreman-1";
var PEL_PROFILE = Object.freeze({
  id,
  digest: createHash("sha256").update(JSON.stringify({ id, limits: DEFAULT_LIMITS })).digest("hex"),
  limits: DEFAULT_LIMITS
});
function diagnostic(code, span3, message, extra = {}) {
  return {
    code,
    severity: "error",
    span: span3,
...
var keyChar = (c) => /^[A-Za-z0-9_\-+*/\\?!<>=.]$/.test(c);
var boundary = (c) => /\s/u.test(c) || `()[]'";^|>\u25B7`.includes(c);
function tokenizePel(source, profile = PEL_PROFILE) {
  let point = { byte: 0, line: 1, column: 1 };
  const origin = pointSpan(point, point);
  for (const [name2, value] of Object.entries(profile.limits))
  ...
  for (let offset = 0; offset < source.length; ) {
    ...
    let width = lead < 128 ? 1 : lead >= 194 && lead <= 223 ? 2 : lead >= 224 && lead <= 239 ? 3 : lead >= 240 && lead <= 244 ? 4 : 0;
    let scalar = width === 1 ? lead : lead & 127 >> width;
    for (let n = 1; n < width; n++) {
      const b = source[offset + n];
      if (b === void 0 || b < 128 || b > 191) {
        width = 0;
        break;
      }
      scalar = scalar << 6 | b & 63;
    }
    if (width === 0 || width === 2 && scalar < 128 || width === 3 && scalar < 2048 || width === 4 && scalar < 65536 || scalar > 1114111 || scalar >= 55296 && scalar <= 57343)
      return {
        ok: false,
        error: [
          diagnostic(
  ...
  let i = 0;
  const tokens = [];
  const span3 = (a, b) => pointSpan(points[a], points[b]);
  const fail32 = (start3, message, help) => ({
    ok: false,
    error: [
      diagnostic(
        "PEL_LEX",
        span3(start3, Math.max(start3 + 1, i)),
        message,
        help ? { help } : {}
      )
    ]
  ...
  while (i < chars.length) {
    ...
    if (punctuation[c]) {
      i++;
      token = { kind: punctuation[c], span: span3(start3, i) };
    } else if (c === "|" && chars[i + 1] === ">") {
      i += 2;
      token = { kind: "pipe", span: span3(start3, i) };
    } else if (c === "|" || c === ">" || c === "\u25B7") {
      i++;
      return fail32(
        start3,
        "Invalid pipe or symbol character",
        c === "\u25B7" ? "Use |> for a pipe" : void 0
      );
    } else if (c === '"') {
      i++;
      let value = "";
      ...
      while (i < chars.length) {
        ...
        const escape2 = chars[i++];
        if (escape2 === void 0) {
          i = chars.length;
          return fail32(start3, "Unterminated string escape");
        }
        const simple = {
          n: "\n",
          r: "\r",
        ...
        if (Object.hasOwn(simple, escape2)) {
          value += simple[escape2];
          continue;
        }
        const count = escape2 === "x" ? 2 : escape2 === "u" ? 4 : escape2 === "U" ? 8 : 0;
        if (!count) return fail32(start3, "Unknown string escape");
        const hex = chars.slice(i, i + count).join("");
        if (hex.length !== count || !/^[0-9a-fA-F]+$/.test(hex))
          return fail32(start3, "Malformed scalar escape");
        i += count;
        const scalar = Number.parseInt(hex, 16);
        if (scalar > 1114111 || scalar >= 55296 && scalar <= 57343)
          return fail32(start3, "Escape is not a Unicode scalar");
        value += String.fromCodePoint(scalar);
      }
      if (!closed) return fail32(start3, "Unterminated string");
      token = { kind: "string", value, span: span3(start3, i) };
    } else if (c === ":") {
      i++;
      while (i < chars.length && keyChar(chars[i])) i++;
      if (i === start3 + 1)
        return fail32(start3, "A keyword requires KEY characters");
      token = {
        kind: "key",
        value: chars.slice(start3 + 1, i).join(""),
        span: span3(start3, i)
      };
    } else {
      while (i < chars.length && !boundary(chars[i])) i++;
      const text2 = chars.slice(start3, i).join("");
      if (text2.startsWith("#")) {
        if (text2 !== "#t" && text2 !== "#f" && text2 !== "#nil")
          return fail32(start3, "Unknown hash constant");
        token = text2 === "#nil" ? { kind: "nil", span: span3(start3, i) } : { kind: "boolean", value: text2 === "#t", span: span3(start3, i) };
      } else if (/^-?[0-9]/.test(text2)) {
        if (!/^-?[0-9]+(?:\.[0-9]+)?$/.test(text2))
          return fail32(start3, "Malformed numeric literal");
        const [whole, fraction = ""] = text2.replace(/^-/u, "").split(".");
        const digits = whole.replace(/^0+/u, "") || "0";
        const maximum = String(Number.MAX_SAFE_INTEGER);
        if (digits.length > maximum.length || digits.length === maximum.length && (digits > maximum || digits === maximum && /[1-9]/u.test(fraction)))
          return fail32(start3, "Numeric literal is outside the safe range");
      ...
      } else token = { kind: "symbol", value: text2, span: span3(start3, i) };
    }
  ...
  return { ok: true, value: { tokens, end: span3(i, i) } };
}
...
function parsePel(source, profile = PEL_PROFILE) {
  const lexed = tokenizePel(source, profile);
  if (!lexed.ok) return lexed;
  const { tokens, end: end3 } = lexed.value;
  let at = 0;
  let astNodes = 0;
  let syntaxDepthPeak = 0;
  const sourceDigest = createHash2("sha256").update(source).digest("hex");
  const currentSpan3 = () => tokens[at]?.span ?? end3;
  const node = (fields2, span3) => {
    astNodes++;
    if (astNodes > profile.limits.maxAstNodes)
      throw new ParseFailure(
        diagnostic("PEL_LIMIT", span3, "AST node limit exceeded", {
          bound: "maxAstNodes",
          consumed: astNodes
        })
      );
    return { ...fields2, nodeId: "", span: span3 };
  };
  const checkDepth = (depth, span3) => {
    syntaxDepthPeak = Math.max(syntaxDepthPeak, depth);
    if (depth > profile.limits.maxSyntaxDepth)
      throw new ParseFailure(
        diagnostic("PEL_LIMIT", span3, "Syntax depth limit exceeded", {
          bound: "maxSyntaxDepth",
          consumed: depth
        })
      );
  };
  const expected = (message, forms, span3 = currentSpan3()) => {
    throw new ParseFailure(
      diagnostic("PEL_PARSE", span3, message, { expectedForms: forms })
    );
  };
  const work = [];
  const deliver = (receive, value) => {
    work.push(() => receive(value));
  };
  function primary(quoted2, depth, receive) {
    work.push(() => {
      ...
      switch (token.kind) {
        ...
        case "boolean":
          value = node(
            { kind: "boolean", value: token.value },
            token.span
          );
          break;
        case "nil":
        ...
        case "symbol":
          value = node(
            { kind: token.kind, name: token.value },
            token.span
          );
          break;
        case "caret":
        ...
        case "open-list": {
          checkDepth(depth + 1, token.span);
          const closing = token.kind === "open-call" ? "close-call" : "close-list";
          sequence(
            quoted2,
            depth + 1,
            (items) => {
              const close2 = tokens[at++];
              if (!close2 || close2.kind !== closing)
                return expected(
                  "Unclosed delimiter",
                  [closing === "close-call" ? ")" : "]"],
                  close2?.span ?? end3
                );
              const span3 = combineSpans(token.span, close2.span);
              deliver(
                receive,
                token.kind === "open-call" && items.length === 0 ? node({ kind: "nil" }, span3) : node(
                  {
  ...
  function expr(quoted2, depth, receive) {
    const pipe2 = (left3) => {
      if (tokens[at]?.kind !== "pipe") {
        deliver(receive, left3);
        return;
      }
      at++;
      primary(
        ...
        (right3) => deliver(
          pipe2,
          node(
            { kind: "pipe", left: left3, right: right3 },
            combineSpans(left3.span, right3.span)
          )
        )
      );
    };
    primary(quoted2, depth, pipe2);
  }
  function sequence(quoted2, depth, receive, closing) {
    const items = [];
    const append4 = (value) => {
      items.push(value);
      work.push(next);
    ...
    const next = () => {
      ...
      expr(quoted2, depth, (item) => {
        if (quoted2 || item.kind !== "key") {
          append4(item);
          return;
        }
        const token = tokens[at];
        const valuePresent = token !== void 0 && token.kind !== "key" && token.kind !== "close-call" && token.kind !== "close-list";
        const key = item.name;
        const keySpan = item.span;
        astNodes--;
        const pair = (value) => append4(
          node(
            { kind: "pair", key, value, valuePresent },
            valuePresent ? combineSpans(keySpan, value.span) : keySpan
          )
        );
        if (valuePresent) expr(quoted2, depth, pair);
        else
          pair(
            node(
              { kind: "nil" },
  ...
  try {
    ...
    const pending3 = expressions.map((expression, index) => ({
      expression,
      path: String(index),
      depth: 0
    })).reverse();
    while (pending3.length) {
      const { expression, path, depth } = pending3.pop();
      const nestedDepth = depth + (["list", "call", "quote", "pipe"].includes(expression.kind) ? 1 : 0);
      checkDepth(nestedDepth, expression.span);
      expression.nodeId = `${sourceDigest}:${path}`;
      const children = expression.kind === "list" || expression.kind === "call" ? expression.items : expression.kind === "pair" ? [expression.value] : expression.kind === "quote" ? [expression.expression] : expression.kind === "pipe" ? [expression.left, expression.right] : [];
...
var CORE_FAILURE_BRAND2 = Symbol("@foreman/core/CoreFailure");
function duplicateJsonKey2() {
  return { [CORE_FAILURE_BRAND2]: true, _tag: "DuplicateJsonKey" };
}
function invalidJson2() {
  return { [CORE_FAILURE_BRAND2]: true, _tag: "InvalidJson" };
}
function isCoreFailure2(v) {
  return typeof v === "object" && v !== null && v[CORE_FAILURE_BRAND2] === true;
}
function sha256Hex2(data2) {
  const hash2 = createHash3("sha256");
  if (typeof data2 === "string") {
    hash2.update(data2, "utf8");
  } else {
    hash2.update(data2);
  }
  return hash2.digest("hex");
}
var PARSE_FAIL2 = Symbol("@foreman/core/parseFail");
function parseFail2(failure3) {
  return { [PARSE_FAIL2]: true, failure: failure3 };
}
function isParseFail2(v) {
  return typeof v === "object" && v !== null && v[PARSE_FAIL2] === true;
}
function parseJsonRejectDuplicateKeys2(text2) {
  let i = 0;
  const s = text2;
  let depth = 0;
  function skipWs() {
  ...
  function fail32() {
    return parseFail2(invalidJson2());
  }
  function parseString() {
    if (peek() !== '"') return fail32();
    i += 1;
    let out = "";
    while (i < s.length) {
      const c = s[i];
      ...
      if (c === "\\") {
        ...
        switch (e) {
          ...
          case "t":
            ...
            break;
          case "u": {
            if (i + 4 > s.length) return fail32();
            const hex = s.slice(i, i + 4);
            if (!/^[0-9a-fA-F]{4}$/.test(hex)) return fail32();
            out += String.fromCharCode(parseInt(hex, 16));
            i += 4;
            break;
          }
  ...
  function parseNumber() {
    const start3 = i;
    if (peek() === "-") i += 1;
    if (peek() < "0" || peek() > "9") return fail32();
    if (peek() === "0") {
      i += 1;
      if (peek() >= "0" && peek() <= "9") return fail32();
    } else {
    ...
    const num = Number(s.slice(start3, i));
    if (!Number.isFinite(num)) return fail32();
    return num;
  }
  function parseValue() {
    skipWs();
    ...
    if (c === "[") return parseArray();
    if (c === "t") {
      if (s.slice(i, i + 4) !== "true") return fail32();
      i += 4;
      return true;
    }
    if (c === "f") {
      if (s.slice(i, i + 5) !== "false") return fail32();
      i += 5;
      return false;
    }
    if (c === "n") {
      if (s.slice(i, i + 4) !== "null") return fail32();
      i += 4;
      return null;
    }
    if (c === "-" || c >= "0" && c <= "9") return parseNumber();
  ...
  function parseObject() {
    ...
    while (true) {
      skipWs();
      const key = parseString();
      if (isParseFail2(key)) return key;
      if (seen.has(key)) return parseFail2(duplicateJsonKey2());
      seen.add(key);
      skipWs();
      if (peek() !== ":") return fail32();
      i += 1;
      const val = parseValue();
      if (isParseFail2(val)) return val;
      Object.defineProperty(obj, key, {
        value: val,
        writable: true,
        enumerable: true,
...
function canonicalize2(value) {
  ...
  if (typeof value === "object") {
    const obj = value;
    const keys32 = Object.keys(obj).sort();
    const parts2 = [];
    for (const k of keys32) {
      parts2.push(JSON.stringify(k) + ":" + canonicalize2(obj[k]));
    }
    return "{" + parts2.join(",") + "}";
  }
  throw new Error("unsupported_json_value");
}
var syntheticSpan = Object.freeze({
  start: 0,
  end: 0,
  line: 1,
  column: 1,
...
function literalDefault(owner, parameter2, value, literal2) {
  const base = {
    nodeId: `builtin:${owner}:${parameter2}:default`,
    span: syntheticSpan
  };
  switch (value) {
    case "nil":
...
function parameter(name2, required, evaluation, defaultExpression) {
  return defaultExpression === void 0 ? Object.freeze({ name: name2, required, evaluation }) : Object.freeze({ name: name2, required, evaluation, defaultExpression });
}
function fixed3(...parameters) {
  return Object.freeze({
    kind: "fixed",
    parameters: Object.freeze(parameters)
  });
}
var binaryStrict = fixed3(
  parameter("x", true, "strict"),
  parameter("y", true, "strict")
);
var unaryXStrict = fixed3(parameter("x", true, "strict"));
var builtinArgSpecs = Object.freeze(
  Object.assign(/* @__PURE__ */ Object.create(null), {
    ...
    sqrt: unaryXStrict,
    not: unaryXStrict,
    len: fixed3(parameter("value", true, "strict")),
    if: fixed3(
    ...
    "pel/nl-condition": fixed3(
      parameter("scrut", true, "strict"),
      parameter("condition", true, "strict")
    )
  })
);
function failure(code, span3, message) {
  return { ok: false, error: diagnostic(code, span3, message) };
}
function numberArgument(value, name2, span3) {
  if (value?.tag !== "number")
    return failure("PEL_TYPE", span3, `:${name2} must be a number`);
  if (!Number.isFinite(value.value) || Number.isInteger(value.value) && !Number.isSafeInteger(value.value)) {
    return failure(
      "PEL_NUMERIC_DOMAIN",
      span3,
      `:${name2} is outside the Pel numeric domain`
    );
  }
  return { ok: true, value: Object.is(value.value, -0) ? 0 : value.value };
}
function checkedNumber(value, span3) {
...
function binaryNumbers(args2, span3, operation) {
  const x = numberArgument(args2.x, "x", span3);
  if (!x.ok) return x;
  const y = numberArgument(args2.y, "y", span3);
  if (!y.ok) return y;
  return checkedNumber(operation(x.value, y.value), span3);
}
function normalizedNodeEqual(left3, right3) {
  if (left3.kind !== right3.kind) return false;
  switch (left3.kind) {
    case "number":
      return right3.kind === "number" && left3.value === right3.value;
    case "string":
      return right3.kind === "string" && left3.value === right3.value;
    case "boolean":
      return right3.kind === "boolean" && left3.value === right3.value;
    case "nil":
    case "caret":
      return true;
    case "key":
    case "symbol":
      return right3.kind === left3.kind && left3.name === right3.name;
    case "pair":
      return right3.kind === "pair" && left3.key === right3.key && left3.valuePresent === right3.valuePresent && normalizedNodeEqual(left3.value, right3.value);
    case "list":
    case "call":
      return right3.kind === left3.kind && left3.items.length === right3.items.length && left3.items.every((item, index) => {
        const other = right3.items[index];
        return other !== void 0 && normalizedNodeEqual(item, other);
      });
    case "quote":
      return right3.kind === "quote" && normalizedNodeEqual(left3.expression, right3.expression);
    case "pipe":
      return right3.kind === "pipe" && normalizedNodeEqual(left3.left, right3.left) && normalizedNodeEqual(left3.right, right3.right);
  }
}
function structuralEqual(left3, right3) {
  if (left3.tag === "closure" || right3.tag === "closure")
    return { ok: false, error: "closure" };
  if (left3.tag !== right3.tag) return { ok: true, value: false };
  switch (left3.tag) {
    case "number":
      return {
        ok: true,
        value: right3.tag === "number" && left3.value === right3.value
      };
    case "string":
      return {
        ok: true,
        value: right3.tag === "string" && left3.value === right3.value
      };
    case "boolean":
      return {
        ok: true,
        value: right3.tag === "boolean" && left3.value === right3.value
      };
    case "nil":
    ...
    case "symbol":
      return {
        ok: true,
        value: right3.tag === left3.tag && left3.name === right3.name
      };
    case "pair": {
      if (right3.tag !== "pair" || left3.key !== right3.key)
        return { ok: true, value: false };
      return structuralEqual(left3.value, right3.value);
    }
    case "list": {
      if (right3.tag !== "list" || left3.items.length !== right3.items.length)
        return { ok: true, value: false };
      for (let index = 0; index < left3.items.length; index += 1) {
        const leftItem = left3.items[index];
        const rightItem = right3.items[index];
    ...
    case "syntax":
      return {
        ok: true,
        value: right3.tag === "syntax" && normalizedNodeEqual(left3.node, right3.node)
      };
  }
}
function containsClosure(value, seen = /* @__PURE__ */ new WeakSet()) {
...
function pureBuiltin(name2, args2, span3) {
  switch (name2) {
    case "+": {
      if (args2.y === void 0 && args2.x?.tag === "list") {
        let total = 0;
        for (const item of args2.x.items) {
          const operand = numberArgument(item, "x", span3);
          if (!operand.ok) return operand;
          const next = checkedNumber(total + operand.value, span3);
          if (!next.ok) return next;
          total = next.value.tag === "number" ? next.value.value : total;
        }
        return { ok: true, value: { tag: "number", value: total } };
      }
      return binaryNumbers(args2, span3, (x, y) => x + y);
    }
    case "-":
      return binaryNumbers(args2, span3, (x, y) => x - y);
    case "*":
      return binaryNumbers(args2, span3, (x, y) => x * y);
    case "/": {
      const denominator = numberArgument(args2.y, "y", span3);
      ...
      const numerator = numberArgument(args2.x, "x", span3);
      if (!numerator.ok) return numerator;
      return checkedNumber(numerator.value / denominator.value, span3);
    }
    case "pow":
      return binaryNumbers(args2, span3, (x, y) => x ** y);
    case "gt": {
    ...
    case "sqrt": {
      const x = numberArgument(args2.x, "x", span3);
      if (!x.ok) return x;
      return checkedNumber(Math.sqrt(x.value), span3);
    }
    case "concat": {
      if (args2.x?.tag !== "string" || args2.y?.tag !== "string") {
        return failure("PEL_TYPE", span3, ":x and :y must be strings");
      }
      return {
        ok: true,
        value: { tag: "string", value: args2.x.value + args2.y.value }
      };
    }
    case "len": {
      const value = args2.value;
      if (value?.tag === "list") return checkedNumber(value.items.length, span3);
      if (value?.tag === "string")
        return checkedNumber([...value.value].length, span3);
      return failure("PEL_TYPE", span3, ":value must be a list or string");
    }
    case "not": {
      if (args2.x?.tag !== "boolean")
        return failure("PEL_TYPE", span3, ":x must be Boolean");
      return { ok: true, value: { tag: "boolean", value: !args2.x.value } };
    }
    case "eq": {
      if (args2.x === void 0 || args2.y === void 0)
        return failure("PEL_TYPE", span3, "eq requires two Pel values");
      if (containsClosure(args2.x) || containsClosure(args2.y)) {
        return failure(
          "PEL_TYPE",
          span3,
      ...
      const equal = structuralEqual(args2.x, args2.y);
      if (!equal.ok)
        return failure(
          "PEL_TYPE",
          span3,
    ...
    default:
      return failure(
        "PEL_TYPE",
        span3,
        `${JSON.stringify(name2)} is not a pure builtin`
      );
  }
}
function validatedIndex(value, length2, span3) {
  if (value.tag !== "number")
    return failure("PEL_TYPE", span3, "list index must be a number");
  if (!Number.isSafeInteger(value.value) || value.value < 1 || value.value > length2) {
    return failure(
      "PEL_INDEX",
      span3,
      `list index ${String(value.value)} is outside 1..${String(length2)}`
    );
  }
  return { ok: true, value: value.value - 1 };
}
function selectOne(items, selector, span3) {
  if (selector.tag === "number") {
    const index = validatedIndex(selector, items.length, span3);
    if (!index.ok) return index;
    const selected = items[index.value];
    return selected === void 0 ? failure("PEL_INDEX", span3, "list index did not select a value") : { ok: true, value: selected };
  }
  if (selector.tag === "key") {
...
function callableList(items, args2, span3) {
  const at = args2.at ?? { tag: "nil" };
  const from = args2.from ?? { tag: "nil" };
  const to = args2.to ?? { tag: "nil" };
  ...
  if (at.tag !== "nil") {
    if (at.tag === "list") {
      ...
      for (const selector of at.items) {
        const result2 = selectOne(items, selector, span3);
        if (!result2.ok) return result2;
        selected.push(result2.value);
      }
      return { ok: true, value: { tag: "list", items: selected } };
    }
    return selectOne(items, at, span3);
  }
  if (from.tag === "nil" && to.tag === "nil")
    return { ok: true, value: { tag: "list", items } };
  const first2 = from.tag === "nil" ? { ok: true, value: 0 } : validatedIndex(from, items.length, span3);
  if (!first2.ok) return first2;
  const last3 = to.tag === "nil" ? { ok: true, value: items.length - 1 } : validatedIndex(to, items.length, span3);
  if (!last3.ok) return last3;
  if (first2.value > last3.value)
    return { ok: true, value: { tag: "list", items: [] } };
  return {
    ok: true,
    value: { tag: "list", items: items.slice(first2.value, last3.value + 1) }
  };
}
function argumentFailure(code, span3, message) {
  return { ok: false, error: diagnostic(code, span3, message) };
}
function planArguments(closure2, args2, span3) {
  if (closure2.argSpec.kind === "sequence") {
  ...
  const namedCount = args2.reduce(
    (count, node) => count + (node.kind === "pair" ? 1 : 0),
    0
  );
  if (namedCount !== 0 && namedCount !== args2.length) {
    return argumentFailure(
      "PEL_ARGUMENT_MODE",
      span3,
  ...
  const parameters = closure2.argSpec.parameters;
  if (namedCount !== 0) {
    const parametersByName = new Map(
      parameters.map((entry) => [entry.name, entry])
    );
    const seen = /* @__PURE__ */ new Set();
    const planned = [];
    for (const argument of args2) {
      ...
      if (Object.hasOwn(closure2.boundArguments, argument.key)) {
        return argumentFailure(
          "PEL_ARGUMENT_NAME",
          span3,
  ...
  const remaining = parameters.filter(
    (entry) => !Object.hasOwn(closure2.boundArguments, entry.name)
  );
  if (args2.length > remaining.length) {
    return argumentFailure(
      "PEL_ARITY",
      span3,
      `call supplies ${String(args2.length)} positional arguments for ${String(remaining.length)} remaining parameters`
    );
  }
  return {
    ok: true,
    value: args2.map((node, index) => {
...
function directCall(node, name2) {
  if (node.kind !== "call") return void 0;
  const callable = node.items[0];
  return callable?.kind === "symbol" && callable.name === name2 ? node.items.slice(1) : void 0;
}
function fixedCallArguments(node, name2, parameterNames2) {
  const args2 = directCall(node, name2);
  if (args2 === void 0) return void 0;
  const result2 = /* @__PURE__ */ Object.create(
    null
  );
  if (args2.length !== 0 && args2.every((argument) => argument.kind === "pair")) {
    for (const argument of args2) {
      if (argument.kind === "pair" && parameterNames2.includes(argument.key) && !Object.hasOwn(result2, argument.key)) {
        result2[argument.key] = argument.value;
      }
    }
  } else {
    for (let index = 0; index < parameterNames2.length; index += 1) {
      const parameterName = parameterNames2[index];
      if (parameterName !== void 0) result2[parameterName] = args2[index];
    }
  }
  return result2;
}
function directDefinition(node) {
  const args2 = fixedCallArguments(node, "def", ["name", "value"]);
  if (args2 === void 0) return void 0;
  const binder = args2.name;
  if (binder?.kind !== "symbol") return void 0;
  return { name: binder.name, value: args2.value };
}
function lambdaParts(node) {
  const args2 = fixedCallArguments(node, "lambda", ["params", "body"]);
  return args2 === void 0 ? void 0 : { parameters: args2.params, body: args2.body };
}
function parameterNames(parameters) {
  const names3 = /* @__PURE__ */ new Set();
  if (parameters?.kind !== "list") return names3;
  for (const item of parameters.items) {
    if (item.kind === "pair") names3.add(item.key);
  }
  return names3;
}
function addRead(reads, seen, name2, bound) {
  if (!bound.has(name2) && !seen.has(name2)) {
    seen.add(name2);
    reads.push(name2);
  }
}
function blockNodes(args2) {
  if (args2.length === 1 && args2[0]?.kind === "list") return args2[0].items;
  return args2;
}
function collectFreeReads(node, bound, reads, seen) {
  switch (node.kind) {
    case "number":
    case "string":
    case "boolean":
  ...
  if (lambda !== void 0) {
    ...
    if (lambda.body !== void 0) {
      ...
      for (const name2 of parameterNames(lambda.parameters))
        lambdaBound.add(name2);
  ...
  if (doArgs !== void 0) {
    ...
    for (const expression of expressions) {
      ...
      if (local !== void 0) blockBound.add(local.name);
    }
    for (const expression of expressions)
...
function readsForExpression(node) {
  const reads = [];
  const seen = /* @__PURE__ */ new Set();
  const definition = directDefinition(node);
  if (definition === void 0) {
    collectFreeReads(node, /* @__PURE__ */ new Set(), reads, seen);
    return reads;
  }
  if (definition.value === void 0) return reads;
  const bound = /* @__PURE__ */ new Set();
  if (lambdaParts(definition.value) !== void 0) bound.add(definition.name);
  collectFreeReads(definition.value, bound, reads, seen);
  return reads;
}
function findCycles(expressions) {
  let nextIndex = 0;
  ...
  const visit = (vertex) => {
    ...
    component.sort((left3, right3) => left3 - right3);
  ...
  cycles.sort((left3, right3) => (left3[0] ?? 0) - (right3[0] ?? 0));
  return cycles;
}
function analyzeSequence(nodes) {
  const firstDefinition = /* @__PURE__ */ new Map();
  const definitions = nodes.map((node, index) => {
    const definition = directDefinition(node);
    const names3 = definition === void 0 ? [] : [definition.name];
    for (const name2 of names3)
      if (!firstDefinition.has(name2)) firstDefinition.set(name2, index);
    return names3;
  });
  const expressions = nodes.map((node, index) => {
    ...
    const dependencies = [
      ...new Set(
        reads.flatMap((name2) => {
          const source = firstDefinition.get(name2);
          return source === void 0 || source === index ? [] : [source];
        })
      )
    ].sort((left3, right3) => left3 - right3);
    return { index, defines: definitions[index] ?? [], reads, dependencies };
  });
  return { expressions, cycles: findCycles(expressions) };
}
var DESCRIPTOR_BRAND = Symbol("PelHostDescriptor");
var REGISTRY_BRAND = Symbol("PelHostRegistry");
var ZERO_SPAN = Object.freeze({
  start: 0,
  end: 0,
  line: 1,
  column: 1,
...
var MAX_ENTRIES = 1e3;
var BASE_FAILURE_ID = "schema:pel-host-failure-v1";
var registries = /* @__PURE__ */ new WeakSet();
var byteSize = (value) => Buffer.byteLength(canonicalize2(value), "utf8");
var text = (value, limit = 4096) => typeof value === "string" && value.isWellFormed() && Buffer.byteLength(value) <= limit;
var name = (value) => text(value, 1024) && value.length > 0 && !/\s|\u0000/u.test(value);
var natural = (value, maximum = Number.MAX_SAFE_INTEGER) => typeof value === "number" && Number.isSafeInteger(value) && value >= 0 && value <= maximum;
...
function array3(value) {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    if (!descriptor3 || !("value" in descriptor3) || !descriptor3.enumerable)
      return false;
  }
  return true;
}
function keys5(value, required, optional = []) {
...
function names(value, empty29 = true) {
  return array3(value) && value.length <= MAX_ENTRIES && (empty29 || value.length > 0) && value.every(name) && new Set(value).size === value.length;
}
function json(value, maxDepth = 256, maxBytes = MAX_BYTES) {
  const seen = /* @__PURE__ */ new Set();
  let nodes = 0;
  function walk(v, depth) {
    if (++nodes > 1e5 || depth > maxDepth) return false;
    ...
    seen.add(v);
    const valid = Array.isArray(v) ? array3(v) && v.every((x) => walk(x, depth + 1)) : record(v) && Object.entries(v).every(
...
function validSchema(input) {
  ...
  function walk(value, depth) {
    if (++nodes > MAX_ENTRIES || depth > 32 || !record(value) || active2.has(value))
      return false;
    active2.add(value);
    let valid = false;
    switch (value.type) {
      case "boolean":
      case "nil":
        valid = keys5(value, ["type"]);
        break;
      case "number":
        valid = keys5(value, ["type", "integer", "minimum", "maximum"]) && typeof value.integer === "boolean" && typeof value.minimum === "number" && Number.isFinite(value.minimum) && typeof value.maximum === "number" && Number.isFinite(value.maximum) && value.minimum <= value.maximum && (!value.integer || Number.isSafeInteger(value.minimum) && Number.isSafeInteger(value.maximum));
        break;
      case "string":
        valid = keys5(value, ["type", "maxBytes"], ["enum"]) && natural(value.maxBytes, MAX_BYTES) && (!Object.hasOwn(value, "enum") || array3(value.enum) && value.enum.length > 0 && value.enum.length <= MAX_ENTRIES && value.enum.every((v) => text(v, value.maxBytes)) && new Set(value.enum).size === value.enum.length);
        break;
      case "key":
        valid = keys5(value, ["type"], ["enum"]) && (!Object.hasOwn(value, "enum") || names(value.enum, false));
        break;
      case "pair":
        valid = keys5(value, ["type", "key", "value"]) && name(value.key) && walk(value.value, depth + 1);
        break;
      case "list":
        valid = keys5(value, ["type", "items", "minItems", "maxItems"]) && natural(value.minItems) && natural(value.maxItems) && value.minItems <= value.maxItems && walk(value.items, depth + 1);
        break;
      case "association": {
        if (!keys5(value, ["type", "fields", "additionalKeys"]) || value.additionalKeys !== false || !array3(value.fields) || value.fields.length > MAX_ENTRIES)
          break;
        const used = /* @__PURE__ */ new Set();
        valid = value.fields.every((field) => {
          if (!record(field) || !keys5(field, ["key", "schema", "required"]) || !name(field.key) || used.has(field.key) || typeof field.required !== "boolean")
            return false;
          used.add(field.key);
      ...
      case "union":
        valid = keys5(value, ["type", "variants"]) && array3(value.variants) && value.variants.length > 0 && value.variants.length <= MAX_ENTRIES && value.variants.every((v) => walk(v, depth + 1));
        break;
      case "data":
        valid = keys5(value, ["type", "maxDepth", "maxBytes"]) && natural(value.maxDepth, 256) && natural(value.maxBytes, MAX_BYTES);
        break;
    }
...
function validNode(input) {
  let nodes = 0;
  const active2 = /* @__PURE__ */ new Set();
  function walk(value, depth) {
    if (++nodes > MAX_ENTRIES || depth > 32 || !record(value) || active2.has(value) || !name(value.nodeId) || !record(value.span))
      return false;
    const span3 = value.span;
    if (!keys5(span3, ["start", "end", "line", "column", "endLine", "endColumn"]) || !Object.values(span3).every((v) => natural(v)) || Number(span3.end) < Number(span3.start) || Number(span3.line) < 1 || Number(span3.column) < 1 || Number(span3.endLine) < Number(span3.line) || Number(span3.endColumn) < 1)
      return false;
    ...
    switch (value.kind) {
      case "nil":
      case "caret":
        valid = keys5(value, base);
        break;
      case "number":
        valid = keys5(value, [...base, "value"]) && typeof value.value === "number" && Number.isFinite(value.value) && (!Number.isInteger(value.value) || Number.isSafeInteger(value.value));
        break;
      case "string":
        valid = keys5(value, [...base, "value"]) && text(value.value, MAX_BYTES);
        break;
      case "boolean":
        valid = keys5(value, [...base, "value"]) && typeof value.value === "boolean";
        break;
      case "key":
      case "symbol":
        valid = keys5(value, [...base, "name"]) && name(value.name);
        break;
      case "pair":
        valid = keys5(value, [...base, "key", "value", "valuePresent"]) && name(value.key) && typeof value.valuePresent === "boolean" && walk(value.value, depth + 1);
        break;
      case "call":
      case "list":
        valid = keys5(value, [...base, "items"]) && array3(value.items) && value.items.length <= MAX_ENTRIES && value.items.every((v) => walk(v, depth + 1));
        break;
      case "quote":
        valid = keys5(value, [...base, "expression"]) && walk(value.expression, depth + 1);
        break;
      case "pipe":
        valid = keys5(value, [...base, "left", "right"]) && walk(value.left, depth + 1) && walk(value.right, depth + 1);
        break;
    }
    active2.delete(value);
    return valid;
...
function validArgSpec(value) {
  if (!record(value)) return false;
  if (value.kind === "sequence") return keys5(value, ["kind"]);
  if (value.kind !== "fixed" || !keys5(value, ["kind", "parameters"]) || !array3(value.parameters) || value.parameters.length > 256)
    return false;
  const used = /* @__PURE__ */ new Set();
  return value.parameters.every((parameter2) => {
    if (!record(parameter2) || !keys5(
      parameter2,
      ["name", "required", "evaluation"],
      ["defaultExpression"]
    ) || !name(parameter2.name) || used.has(parameter2.name) || typeof parameter2.required !== "boolean" || !["strict", "syntax"].includes(String(parameter2.evaluation)))
      return false;
    used.add(parameter2.name);
    const hasDefault = Object.hasOwn(parameter2, "defaultExpression");
    return parameter2.required ? !hasDefault : hasDefault && validNode(parameter2.defaultExpression);
  });
}
function validFailureSchema(value) {
  return record(value) && keys5(value, [
    "codes",
    "maxMessageBytes",
    "maxCauseBytes",
    "requiredCauseKeys"
  ]) && names(value.codes, false) && natural(value.maxMessageBytes, MAX_BYTES) && natural(value.maxCauseBytes, MAX_BYTES) && names(value.requiredCauseKeys);
}
function validDescriptor(value) {
  return record(value) && keys5(value, [
    ...
    "resourceResolverId",
    "resourceEnvelope"
  ]) && name(value.id) && name(value.name) && validArgSpec(value.argSpec) && value.argSpec.kind === "fixed" && name(value.resultSchemaId) && name(value.failureSchemaId) && name(value.effectKind) && names(value.capabilities) && record(value.resources) && keys5(value.resources, ["reads", "writes", "unknown"]) && names(value.resources.reads) && names(value.resources.writes) && typeof value.resources.unknown === "boolean" && name(value.resourceResolverId) && json(value.resourceEnvelope, 32, 1024 * 1024);
}
function builtins() {
  const literal2 = (nodeId, kind, value) => ({ nodeId, span: ZERO_SPAN, kind, value });
  const common = {
      ...
      argSpec: {
        ...
        parameters: [
          { name: "vals", required: true, evaluation: "strict" },
          {
            ...
            required: false,
            evaluation: "strict",
            defaultExpression: literal2("print:sep", "string", "")
          },
          {
            name: "nl",
            required: false,
            evaluation: "strict",
            defaultExpression: literal2("print:nl", "boolean", false)
          }
        ]
      },
...
function createHostRegistry(descriptors = [], dataSchemas = {}, failureSchemas = {}, resolverCatalog = {
  static: { version: 1 }
}) {
  const fail32 = (message) => ({
    ok: false,
    error: [diagnostic("PEL_REGISTRY", ZERO_SPAN, message)]
  });
  try {
    if (!array3(descriptors) || descriptors.length > MAX_ENTRIES || !record(dataSchemas) || !record(failureSchemas) || !record(resolverCatalog))
      return fail32("Invalid registry container");
    if (Object.keys(dataSchemas).length > MAX_ENTRIES || Object.keys(failureSchemas).length > MAX_ENTRIES || Object.keys(resolverCatalog).length > MAX_ENTRIES)
      return fail32("Registry entry limit exceeded");
    if (!Object.entries(dataSchemas).every(
      ([id2, schema]) => name(id2) && validSchema(schema)
    ) || !Object.entries(failureSchemas).every(
      ([id2, schema]) => name(id2) && validFailureSchema(schema)
    ))
      return fail32("Invalid registry schema");
    if (!Object.entries(resolverCatalog).every(
      ([id2, value]) => name(id2) && json(value, 32, 1024 * 1024)
    ) || !Object.hasOwn(resolverCatalog, "static"))
      return fail32("Invalid resolver catalog");
    if (Object.hasOwn(dataSchemas, "schema:pel-boolean-v1") || Object.hasOwn(dataSchemas, "schema:pel-data-v1") || Object.hasOwn(failureSchemas, BASE_FAILURE_ID))
      return fail32("Builtin schema cannot be replaced");
    const allData = {
      ...dataSchemas,
    ...
    const ids3 = /* @__PURE__ */ new Set(), exported = new Set(
      Object.keys(builtinArgSpecs).filter(
        (name2) => !["print", "pel/nl-condition", "list"].includes(name2)
      )
    );
    const allDescriptors = [...builtins(), ...descriptors];
    for (const item of allDescriptors) {
      if (!validDescriptor(item) || ids3.has(item.id) || exported.has(item.name) || !Object.hasOwn(allData, item.resultSchemaId) || !Object.hasOwn(allFailure, item.failureSchemaId) || !Object.hasOwn(resolverCatalog, item.resourceResolverId))
        return fail32("Invalid or duplicate host descriptor");
      ids3.add(item.id);
    ...
    const payload = structuredClone({
      ...
      descriptors: allDescriptors.sort(
    ...
    if (byteSize(payload) > MAX_BYTES)
      return fail32("Registry byte limit exceeded");
    const digest2 = sha256Hex2(canonicalize2(payload));
    for (const item of payload.descriptors)
      Object.defineProperty(item, DESCRIPTOR_BRAND, { value: true });
    const result2 = Object.defineProperty(
      { ...payload, digest: digest2 },
      REGISTRY_BRAND,
      { value: true }
    );
    freeze(result2);
    registries.add(result2);
    return { ok: true, value: result2 };
  } catch {
    return fail32("Registry contains malformed or non-declarative data");
  }
}
var CODEC_MAX_BYTES = 16 * 1024 * 1024;
var DEFAULT_RUN_OPTIONS = Object.freeze({
...
function authoringProfileCeilingIssue(profile) {
  if (profile.grammarSupport === "qualified" && profile.evidenceKind !== "qualified")
    return "Qualified grammar requires qualified profile evidence";
  const ceiling = Object.hasOwn(ceilings, profile.profileId) ? ceilings[profile.profileId] : void 0;
  if (!ceiling)
    return profile.evidenceKind === "fixture" ? void 0 : `Unknown profile ${profile.profileId} requires explicit fixture evidence`;
  ...
  if (profile.profileId === "claude-fable-5-1" && (controls.thinkingModes.some((mode) => mode !== "adaptive") || controls.budgetTokens || controls.sampling.length > 0 || controls.toolChoices.some(
...
function isNormalizedAuthoringResourceId(value) {
  const separator = value.indexOf(":");
  if (separator < 1 || !/^[a-z][a-z0-9.-]*$/.test(value.slice(0, separator)))
    return false;
  const path = value.slice(separator + 1);
  return path.length > 0 && path.split("/").every(
...
function canonicalAuthoringJson(value) {
  ...
  function walk(item, depth) {
    ...
    if (typeof item !== "object" || active2.has(item))
      throw new Error("Expected acyclic JSON");
    for (const symbol3 of Object.getOwnPropertySymbols(item)) {
      const d = Object.getOwnPropertyDescriptor(item, symbol3);
      if (!["PelHostRegistry", "PelHostDescriptor"].includes(
        symbol3.description ?? ""
      ) || d.enumerable || !("value" in d) || d.value !== true)
        throw new Error("Symbol keys are forbidden");
    }
    const proto4 = Object.getPrototypeOf(item);
    if (Array.isArray(item) ? proto4 !== Array.prototype : proto4 !== null && proto4 !== Object.prototype)
      throw new Error("Expected JSON object");
    active2.add(item);
    const descriptors = Object.getOwnPropertyDescriptors(item);
    if (Array.isArray(item) && Object.keys(descriptors).length !== item.length + 1)
      throw new Error("Sparse or extended array");
    for (const [key, descriptor3] of Object.entries(descriptors)) {
      if (Array.isArray(item) && key === "length") continue;
      if (!("value" in descriptor3) || !descriptor3.enumerable)
        throw new Error("Expected inert JSON");
      if (Array.isArray(item) && !/^(0|[1-9][0-9]*)$/.test(key))
        throw new Error("Extended array");
      walk(descriptor3.value, depth + 1);
    }
    active2.delete(item);
  }
  walk(value, 0);
  const encoded = canonicalize2(value);
  if (Buffer.byteLength(encoded) > 16 * 1024 * 1024)
    throw new Error("Authoring JSON exceeds 16 MiB");
  return encoded;
}
var hashAuthoringContent = (value) => sha256Hex2(canonicalAuthoringJson(value));
function createPlanBindingV1(source, snapshot) {
  return Object.freeze({
    schemaVersion: 1,
    sourceDigest: sha256Hex2(source),
...
function validateBinding(binding, source, snapshot) {
    ...
    error: [
      {
        ...
        relatedSpans: [],
        expectedForms: [],
        message: "The source or authoring environment differs from the checked binding; check the source again."
      }
    ]
  };
}
var zero2 = { start: 0, end: 0, line: 1, column: 1, endLine: 1, endColumn: 1 };
function authoringDiagnostic(code, message, expectedForms = []) {
  return {
    code,
    message,
    severity: "error",
...
function fail22(message, code = "PEL_SCHEMA") {
  return { ok: false, error: [authoringDiagnostic(code, message)] };
}
function record3(v) {
  return typeof v === "object" && v !== null && !Array.isArray(v);
}
function keys22(v, required, optional = []) {
  return record3(v) && required.every((k) => Object.hasOwn(v, k)) && Object.keys(v).every((k) => required.includes(k) || optional.includes(k));
}
function names2(v) {
  return Array.isArray(v) && v.length <= 1e3 && v.every(
    (x) => typeof x === "string" && x.length > 0 && Buffer.byteLength(x) <= 4096
...
function freeze2(v) {
  if (typeof v === "object" && v !== null && !Object.isFrozen(v)) {
    for (const child2 of Object.values(v)) freeze2(child2);
    Object.freeze(v);
  }
  return v;
}
...
function validateAuthoringControlsV1(value, profile) {
  try {
    canonicalAuthoringJson(value);
    const metadataIssue = authoringProfileCeilingIssue(profile);
    if (metadataIssue) return fail22(metadataIssue, "PEL_PROFILE_UNSUPPORTED");
    if (!keys22(value, [
      "effort",
      "thinking",
      "sampling",
      "toolChoice",
      "execution"
    ]) || !keys22(value.thinking, ["mode"], ["budgetTokens"]) || !keys22(value.sampling, [], ["temperature", "topP", "topK"]) || !keys22(value.execution, ["mode", "store"]))
      return fail22(
    ...
    const tool = typeof controls.toolChoice === "string" ? controls.toolChoice : keys22(controls.toolChoice, ["name"]) && typeof controls.toolChoice.name === "string" && controls.toolChoice.name.length > 0 ? "named" : "invalid";
    if (!supported.efforts.includes(controls.effort) || !supported.thinkingModes.includes(controls.thinking.mode) || !supported.executionModes.includes(controls.execution.mode) || !supported.store.includes(controls.execution.store) || !supported.toolChoices.includes(tool))
      return fail22(
        `Controls are unsupported by ${profile.profileId}/${profile.transportId}`,
        "PEL_PROFILE_UNSUPPORTED"
      );
    if (controls.thinking.budgetTokens !== void 0 && (!supported.budgetTokens || !natural2(controls.thinking.budgetTokens) || controls.thinking.budgetTokens === 0 || controls.thinking.mode !== "enabled"))
      return fail22("Thinking budget is unsupported", "PEL_PROFILE_UNSUPPORTED");
    for (const [key, n] of Object.entries(controls.sampling))
      if (!supported.sampling.includes(key) || typeof n !== "number" || !Number.isFinite(n) || n < 0 || key === "topP" && n > 1 || key === "temperature" && n > 2 || key === "topK" && !natural2(n))
        return fail22(`Unsupported sampling ${key}`, "PEL_PROFILE_UNSUPPORTED");
    if (profile.profileId === "claude-opus-5" && ["xhigh", "max"].includes(controls.effort) && controls.thinking.mode === "disabled")
      return fail22(
        "Opus requires thinking at xhigh/max effort",
        "PEL_PROFILE_UNSUPPORTED"
      );
    return { ok: true, value: freeze2(structuredClone(controls)) };
  } catch {
    return fail22("Invalid provider controls", "PEL_PROFILE_UNSUPPORTED");
  }
}
function validateSelection(s, v, predicate = false) {
  if (!keys22(
    v,
    ["profileId", "transportId", "controls", "credentialProfileRef"],
    predicate ? ["outputSchemaId"] : []
  ) || typeof v.profileId !== "string" || typeof v.transportId !== "string" || typeof v.credentialProfileRef !== "string")
    return fail22("Invalid exact model selection", "PEL_PROFILE_UNSUPPORTED");
  ...
  if (!profile || !s.policy.allowedModelTransports.some(
    (p) => p.profileId === v.profileId && p.transportId === v.transportId
...
function resolveModelSelection(snapshot, model, transport) {
  if (model.startsWith("role:")) {
    const selection = snapshot.roleBindings[model];
    if (!selection)
      return fail22(`Unknown model role ${model}`, "PEL_PROFILE_UNSUPPORTED");
    if (transport !== void 0 && transport !== selection.transportId)
      return fail22(
  ...
  const profiles = snapshot.providerProfiles.filter(
    (p) => p.profileId === model && (transport === void 0 || p.transportId === transport) && snapshot.policy.allowedModelTransports.some(
      (pair) => pair.profileId === p.profileId && pair.transportId === p.transportId
    )
  );
...
function computed(input) {
  ...
  const nlConditionProfileDigest = content.nlConditionProfile === null ? null : hashAuthoringContent(content.nlConditionProfile);
  const options = {
  ...
  const payload = {
    ...
    nlConditionProfileDigest,
    artifactDigests: content.artifactDescriptors.map((a) => ({ id: a.id, digest: hashAuthoringContent(a) })).sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0),
    options,
    optionsDigest: hashAuthoringContent(options)
  };
  return { ...payload, snapshotDigest: hashAuthoringContent(payload) };
}
function createAuthoringSnapshotV1(content) {
  try {
    return validateAuthoringSnapshotV1(computed(content));
  } catch {
    return fail22("Snapshot contains invalid JSON content");
  }
}
function parseAuthoringSnapshotV1(text2) {
  if (Buffer.byteLength(text2) > 16 * 1024 * 1024)
    return fail22("Snapshot exceeds 16 MiB");
  const parsed = parseJsonRejectDuplicateKeys2(text2);
  return isCoreFailure2(parsed) ? fail22("Snapshot must be JSON with unique object keys") : validateAuthoringSnapshotV1(parsed);
}
function validateAuthoringSnapshotV1(value) {
  try {
    ...
    if (!keys22(v, [
      ...
      return fail22("Snapshot has missing or unknown fields");
    ...
    if (!keys22(r, [
      ...
      "resolverCatalog",
      "digest"
    ]) || !Array.isArray(r.descriptors) || !record3(r.dataSchemas) || !record3(r.failureSchemas) || !record3(r.resolverCatalog))
      return fail22("Invalid registry content");
    const dataSchemas = Object.fromEntries(
      Object.entries(r.dataSchemas).filter(
        ([id2]) => !["schema:pel-data-v1", "schema:pel-boolean-v1"].includes(id2)
      )
    );
    const failureSchemas = Object.fromEntries(
      Object.entries(r.failureSchemas).filter(
        ([id2]) => id2 !== "schema:pel-host-failure-v1"
      )
    );
    const registry = createHostRegistry(
      r.descriptors.filter(
        (d) => record3(d) && d.id !== "print" && d.id !== "pel/nl-condition"
      ),
      dataSchemas,
      failureSchemas,
      r.resolverCatalog
    ...
    if (!registry.ok || !same(r, registry.value))
      return fail22("Registry content or digest mismatch");
    if (registry.value.descriptors.some(
      (d) => [...d.resources.reads, ...d.resources.writes].some(
        (id2) => !isNormalizedAuthoringResourceId(id2)
      )
    ))
      return fail22("Registry resources must be finite normalized IDs");
    if (!same(v.resourceResolvers, registry.value.resolverCatalog))
      return fail22("Resolver content differs from the registry");
    const s = {
      ...v,
      registry: registry.value
    };
    if (!keys22(v.limits, Object.keys(PEL_PROFILE.limits)) || Object.entries(v.limits).some(
      ([key, n]) => !natural2(n) || n > PEL_PROFILE.limits[key]
    ))
      return fail22("Invalid or widened language limits");
    if (!keys22(v.generationLimits, [
      "maxSourceBytes",
      "maxInputTokens",
      "maxOutputTokens",
      "maxCostUnits",
      ...
      "maxElapsedMs",
      "maxRepairs"
    ]) || Object.values(v.generationLimits).some((n) => !natural2(n)) || s.generationLimits.maxSourceBytes > 1048576 || s.generationLimits.attemptTimeoutMs > 6e4 || s.generationLimits.maxElapsedMs > 18e4 || s.generationLimits.maxRepairs > 2)
      return fail22("Generation limits are missing, widened, or nonfinite");
    const p = v.policy;
    if (!keys22(p, [
      "allowedCapabilities",
      "allowedEffectKinds",
      ...
      return fail22("Policy fields are invalid");
    for (const key of [
      "allowedCapabilities",
      "allowedEffectKinds",
      "allowedSchemaIds",
      ...
      if (!names2(p[key])) return fail22(`Invalid policy ${key}`);
    if (!keys22(p.resourceEnvelope, ["reads", "writes"]) || !names2(p.resourceEnvelope.reads) || !names2(p.resourceEnvelope.writes) || [...p.resourceEnvelope.reads, ...p.resourceEnvelope.writes].some(
      (id2) => !isNormalizedAuthoringResourceId(id2)
    ) || !keys22(p.artifactConstraints, ["allowedIds", "maxBytes"]) || !names2(p.artifactConstraints.allowedIds) || p.artifactConstraints.allowedIds.some(
      (id2) => !id2.startsWith("artifact:") || !isNormalizedAuthoringResourceId(id2)
    ) || !natural2(p.artifactConstraints.maxBytes))
      return fail22("Policy resource or artifact bounds are invalid");
    if (![p.maxEffects, p.maxCostUnits, p.maxElapsedMs, p.maxOutputBytes].every(
      natural2
    ))
      return fail22("Policy budgets must be finite nonnegative integers");
    if (!Array.isArray(p.allowedModelTransports) || !p.allowedModelTransports.every(
      (pair) => keys22(pair, ["profileId", "transportId"]) && typeof pair.profileId === "string" && typeof pair.transportId === "string"
    ) || new Set(
      p.allowedModelTransports.map((pair) => canonicalAuthoringJson(pair))
    ).size !== p.allowedModelTransports.length)
      return fail22("Invalid model/transport policy");
    if (!Array.isArray(v.providerProfiles) || v.providerProfiles.length > 1e3)
      return fail22("Provider profiles are missing");
    ...
    for (const raw of v.providerProfiles) {
      if (!keys22(raw, [
        ...
        "applicationDefaults",
        "grammarSupport"
      ]) || typeof raw.profileId !== "string" || typeof raw.transportId !== "string" || !names2([raw.profileId]) || !names2([raw.transportId]) || !["documented", "qualified", "fixture"].includes(
        String(raw.evidenceKind)
      ) || !["qualified", "unsupported", "unknown"].includes(
        String(raw.grammarSupport)
      ))
        return fail22("Invalid provider profile");
      const pair = raw.profileId + "/" + raw.transportId;
      if (pairs.has(pair)) return fail22("Duplicate provider profile/transport");
      pairs.add(pair);
      ...
      if (!keys22(c, [
        ...
        c.toolChoices,
        c.executionModes
      ].every(names2) || !Array.isArray(c.store) || new Set(c.store).size !== c.store.length || !c.store.every(
        (x) => x === "provider-default" || typeof x === "boolean"
      ) || typeof c.budgetTokens !== "boolean")
        return fail22("Invalid supported controls");
      for (const [key, allowed] of Object.entries({
        ...
        if (!c[key].every((x) => allowed.includes(x)))
          return fail22("Unknown supported control value");
    ...
    if (s.policy.allowedModelTransports.some(
      (pair) => !pairs.has(pair.profileId + "/" + pair.transportId)
    ))
      return fail22("Policy references missing provider profile");
    if (s.policy.allowedSchemaIds.some(
      (id2) => !Object.hasOwn(registry.value.dataSchemas, id2)
    ))
      return fail22("Policy references missing schema");
    if (!s.policy.allowedCredentialProfileRefs.includes(
      s.defaultCredentialProfileRef
    ) || !["ordered", "automatic"].includes(s.dependencyMode) || !s.policy.allowedSchemaIds.includes(s.resultContract))
      return fail22("Invalid credential, dependency mode, or result contract");
    if (!record3(v.roleBindings) || Object.keys(v.roleBindings).some(
      (id2) => !/^role:[a-zA-Z0-9._-]+$/.test(id2)
    ))
      return fail22("Invalid role binding IDs");
    for (const role of Object.values(v.roleBindings)) {
      const result2 = validateSelection(s, role);
      if (!result2.ok) return result2;
    }
    if (v.nlConditionProfile !== null) {
      const result2 = validateSelection(s, v.nlConditionProfile, true);
      if (!result2.ok) return result2;
    }
    if (!Array.isArray(v.artifactDescriptors) || v.artifactDescriptors.length > 1e3)
      return fail22("Missing artifact descriptors");
    const ids3 = /* @__PURE__ */ new Set();
    for (const a of v.artifactDescriptors) {
      if (!keys22(a, ["id", "schemaId", "content"]) || typeof a.id !== "string" || !/^artifact:[a-zA-Z0-9][a-zA-Z0-9._/-]*$/.test(a.id) || a.id.includes("//") || a.id.split("/").some((x) => x === ".." || x === ".") || ids3.has(a.id) || typeof a.schemaId !== "string" || !s.policy.allowedSchemaIds.includes(a.schemaId) || !s.policy.artifactConstraints.allowedIds.includes(a.id) || Buffer.byteLength(canonicalAuthoringJson(a.content)) > s.policy.artifactConstraints.maxBytes)
        return fail22("Artifact identity, schema, or content bounds are invalid");
      ids3.add(a.id);
    }
    ...
    return { ok: true, value: freeze2(s) };
  } catch {
    return fail22("Snapshot contains malformed or non-declarative data");
  }
}
function buildEffectiveAuthoringSnapshotV1(base, selection = {}) {
  ...
  try {
    ...
    if (!keys22(
      ...
      return fail22("Unknown effective selection field", "PEL_INVALID_SELECTION");
    const limits2 = { ...base.limits };
    const policy = { ...base.policy };
    for (const [input, target, allowed] of [
      [selection.narrowingLimits, limits2, Object.keys(limits2)],
      [
      ...
      if (!keys22(input, [], allowed))
        return fail22("Unknown narrowing limit", "PEL_INVALID_SELECTION");
      for (const [key, n] of Object.entries(input)) {
        const obj = target;
        if (!natural2(n) || typeof obj[key] !== "number" || n > obj[key])
          return fail22(`Selection cannot widen ${key}`, "PEL_INVALID_SELECTION");
        obj[key] = n;
      }
    }
    return createAuthoringSnapshotV1({
      ...
      limits: limits2,
      policy,
      roleBindings: selection.roleBindings === void 0 ? base.roleBindings : { ...base.roleBindings, ...selection.roleBindings },
      nlConditionProfile: selection.nlConditionProfile === void 0 ? base.nlConditionProfile : selection.nlConditionProfile,
      dependencyMode: selection.dependencyMode ?? base.dependencyMode,
      resultContract: selection.resultContract ?? base.resultContract
    });
  } catch {
    return fail22("Invalid effective selection", "PEL_INVALID_SELECTION");
  }
}
var anySchema = {
  type: "data",
...
var nil2 = () => known({ tag: "nil" });
var known = (value, deps2 = /* @__PURE__ */ new Set()) => ({
  tag: "known",
  value,
  deps: deps2
});
var deps = (values3) => new Set(values3.flatMap((v) => [...v.deps]));
function concrete(v) {
  if (v.tag === "known") return v.value;
  if (v.tag === "pair") {
    const value = concrete(v.value);
    return value === void 0 ? void 0 : { tag: "pair", key: v.key, value };
  }
  if (v.tag === "list") {
    const values3 = v.items.map(concrete);
    return values3.some((x) => x === void 0) ? void 0 : { tag: "list", items: values3 };
  }
  return void 0;
}
function data(v) {
  return v.tag !== "closure" && v.tag !== "syntax" && v.tag !== "symbol" && (v.tag !== "list" || v.items.every(data)) && (v.tag !== "pair" || data(v.value));
}
function summary5(v) {
  ...
  if (v.tag === "functions")
    return {
      ...
      remainingArguments: [
        ...new Set(
          v.choices.flatMap(
            (f) => f.spec.kind === "fixed" ? f.spec.parameters.filter((p) => !Object.hasOwn(f.bound, p.name)).map((p) => p.name) : []
          )
        )
      ]
    };
  return {
    kind: "unresolved",
    ...v.schema ? { schema: v.schema, ...v.schemaId ? { schemaId: v.schemaId } : {} } : {},
    reason: value ? "quoted syntax" : "value depends on host results",
    originSpans: v.origins ?? []
  };
}
function unknown(schema, span3, dependencies = /* @__PURE__ */ new Set(), schemaId) {
  if (schema.type === "association")
    return {
      ...
      items: schema.fields.map((f) => ({
        tag: "pair",
        key: f.key,
        value: unknown(f.schema, span3, dependencies),
        deps: dependencies
      }))
    };
...
function merge7(a, b, span3) {
  if (a === b) return a;
  const ac = concrete(a), bc = concrete(b);
  const dependencies = deps([a, b]);
  if (ac && bc && canonicalize2(ac) === canonicalize2(bc))
    return known(ac, dependencies);
  if (a.tag === "functions" && b.tag === "functions")
    return {
      tag: "functions",
      choices: [.../* @__PURE__ */ new Set([...a.choices, ...b.choices])],
      deps: dependencies
    };
  if (a.tag === "pair" && b.tag === "pair" && a.key === b.key)
    return {
      tag: "pair",
      key: a.key,
      value: merge7(a.value, b.value, span3),
      deps: dependencies
    };
  if (a.tag === "unknown" && b.tag === "unknown") {
    const schema2 = canonicalize2(a.schema) === canonicalize2(b.schema) ? a.schema : { type: "union", variants: [a.schema, b.schema] };
    return unknown(
      schema2,
      span3,
      dependencies,
      a.schemaId === b.schemaId ? a.schemaId : void 0
    );
  }
  if (a.tag === "list" && b.tag === "list" && a.items.length === b.items.length)
    return {
      tag: "list",
      items: a.items.map((v, i) => merge7(v, b.items[i], span3)),
      deps: dependencies,
      ...a.schema ? { schema: a.schema } : {},
      ...a.schemaId && a.schemaId === b.schemaId ? { schemaId: a.schemaId } : {},
      origins: [...a.origins ?? [], ...b.origins ?? []]
    };
  const schema = ac && bc && ac.tag === bc.tag && ["number", "string", "boolean"].includes(ac.tag) ? ac.tag === "number" ? {
    type: "number",
    integer: false,
    minimum: -Number.MAX_VALUE,
    maximum: Number.MAX_VALUE
  } : ac.tag === "string" ? { type: "string", maxBytes: 1048576 } : { type: "boolean" } : anySchema;
  return unknown(schema, span3, dependencies);
}
...
function analyzePel(input) {
  const { program, snapshot: s } = input;
  const effects = [];
  const dependencies = [];
  const dynamicRegions = [];
  let reductions = 0, iterations = 0;
  let ordinal = 0;
  let effectCalls = 0;
  const effectWeights = /* @__PURE__ */ new Map();
  const localNames = /* @__PURE__ */ new WeakMap();
  const reserved = /* @__PURE__ */ new Set([
    ...Object.keys(builtinArgSpecs),
    ...s.registry.descriptors.map((d) => d.name)
  ]);
  function scope3(parent, copyLocals = false) {
    const result2 = new Map(parent);
    localNames.set(
      result2,
      copyLocals ? new Set(localNames.get(parent) ?? []) : /* @__PURE__ */ new Set()
    );
    return result2;
  }
  function caretUsed(n, c) {
    ...
    if (n.kind === "call") {
      const head5 = n.items[0];
      const value = head5?.kind === "symbol" ? c.env.get(head5.name) : void 0;
      if (value?.tag === "functions" && value.choices.every((f) => f.name === "case")) {
        return n.items.slice(1).some(
          (a, i) => a.kind === "pair" ? a.key !== "body" && caretUsed(a.value, c) : i === 0 && n.items.length > 2 && caretUsed(a, c)
        );
      }
    }
    return (n.kind === "list" || n.kind === "call") && n.items.some((i) => caretUsed(i, c));
  }
  function inject(n, value, c) {
    if (n.kind === "pipe") return { ...n, left: inject(n.left, value, c) };
    if (n.kind !== "call")
      fail32("PEL_TYPE", n, "Pipe without a caret requires a call");
    const name2 = `@abstract-injected/${ordinal++}`;
    c.env.set(name2, value);
    return {
      ...n,
      items: [n.items[0], { ...n, kind: "symbol", name: name2 }, ...n.items.slice(1)]
    };
  }
  function fail32(code, node, message, relatedSpans = []) {
  ...
  function tick(n, c) {
    if (++reductions > s.limits.maxReductions)
      fail32("PEL_LIMIT", n, "Abstract evaluation exceeds maxReductions");
    if (c.depth > s.limits.maxCallDepth)
      fail32("PEL_LIMIT", n, "Abstract evaluation exceeds maxCallDepth");
  }
  function expect(v, types, n) {
    const possible = (schema) => schema.type === "data" || (schema.type === "union" ? schema.variants.some(possible) : types.includes(schema.type));
    ...
    const cv = concrete(v);
    const type = cv?.tag ?? (v.tag === "list" ? "list" : v.tag === "functions" ? "closure" : v.tag === "unknown" ? v.schema.type : void 0);
    if (type && type !== "data" && type !== "union" && !types.includes(type))
      fail32("PEL_TYPE", n, `Expected ${types.join(" or ")}; received ${type}`);
  }
  function text2(v) {
    const cv = v && concrete(v);
    return cv?.tag === "string" ? cv.value : void 0;
  }
  function val(b, c) {
    if (!b) return nil2();
    return b.kind === "value" ? b.value : ev(b.node, {
      ...c,
      env: b.env,
  ...
  function syntax(b, n) {
    if (b?.kind !== "syntax") fail32("PEL_TYPE", n, "Expected syntax argument");
    return b;
  }
  function mergeEnvs(target, a, b, n) {
    for (const name2 of /* @__PURE__ */ new Set([...a.keys(), ...b.keys()])) {
      const av = a.get(name2), bv = b.get(name2);
      if (av && bv) target.set(name2, merge7(av, bv, n.span));
      else
        target.set(name2, {
          tag: "conditional",
          origins: [n.span],
    ...
    localNames.set(
      target,
      /* @__PURE__ */ new Set([...localNames.get(a) ?? [], ...localNames.get(b) ?? []])
    );
  }
  function region(n, c, reason, count, run) {
    if (c.structure) return run(c);
    const start3 = effects.length, id2 = `region/${n.nodeId}/${ordinal++}`;
    const value = run({
      ...c,
      region: id2,
      multiplier: (c.multiplier ?? 1) * count
    });
    const members = effects.slice(start3);
    const calls = members.reduce(
      (sum2, e) => sum2 + (effectWeights.get(e.effectId) ?? 1),
      0
    ) / (c.multiplier ?? 1);
    ...
    dynamicRegions.push({
      ...
      models: members.flatMap(
        (e) => e.model ? [{ profileId: e.model.profileId, transportId: e.model.transportId }] : []
      ),
  ...
  function host(f, args2, n, c) {
    const d = s.registry.descriptors.find((d2) => d2.id === f.name);
    if (!d)
      fail32("PEL_UNBOUND_SYMBOL", n, `Host descriptor ${f.name} is missing`);
    for (const name2 of [
      "id",
      "model",
      "output",
      ...
      if (args2[name2] && !(name2 === "transport" && concrete(args2[name2])?.tag === "nil"))
        expect(args2[name2], ["string"], n);
    if (args2.limit) {
      expect(args2.limit, ["number"], n);
      const v = concrete(args2.limit);
      if (v?.tag === "number" && (!Number.isInteger(v.value) || v.value < 1 || v.value > 20))
        fail32("PEL_TYPE", n, "Research limit must be an integer from 1 to 20");
    }
    if (c.structure)
      return unknown(
        s.registry.dataSchemas[d.resultSchemaId],
        n.span,
        deps(Object.values(args2)),
        d.resultSchemaId
      );
    for (const cap of d.capabilities)
      if (!s.policy.allowedCapabilities.includes(cap))
        fail32("PEL_CAPABILITY_DENIED", n, `Capability ${cap} is not admitted`);
    ...
    if (args2.model) {
      ...
      const selection = resolveModelSelection(s, id2, text2(args2.transport));
      if (!selection.ok)
    ...
    for (const [name2, allowed, code] of [
      ...
      if (args2[name2]) {
        const v = text2(args2[name2]);
        if (v && !allowed.includes(v))
          fail32(code, n, `${name2} ${v} is not admitted`);
        if (!v)
          fail32(
            "PEL_DYNAMIC_EFFECT_UNBOUNDED",
            n,
            `${name2} must resolve within a finite admitted set`
          );
      }
    }
    const reads = new Set(d.resources.reads), writes = new Set(d.resources.writes);
    let unresolved = d.resources.unknown;
    for (const name2 of ["input", "bundle"]) {
      const v = args2[name2];
      if (!v) continue;
      const value = text2(v);
      if (value) {
        ...
        if (value.startsWith("artifact:")) {
          if (!s.policy.artifactConstraints.allowedIds.includes(value))
            fail32("PEL_ARTIFACT_DENIED", n, `Artifact ${value} is not admitted`);
          reads.add(value);
        }
    ...
    if (unresolved && s.policy.resourceEnvelope.reads.length + s.policy.resourceEnvelope.writes.length === 0)
      fail32(
        "PEL_DYNAMIC_EFFECT_UNBOUNDED",
        n,
        "Unknown resource has no finite policy envelope"
      );
    const effectId = sha256Hex2(
    ...
    const effect = {
      ...
      arguments: Object.fromEntries(
        Object.entries(args2).map(([k, v]) => [k, summary5(v)])
      ),
      model,
      controlsSource: model ? text2(args2.model)?.startsWith("role:") ? "default" : "explicit" : null,
      ...
      gates: text2(args2.gate) ? [text2(args2.gate)] : [],
    ...
    for (const before2 of effects) {
      const conflict = before2.resources.writes.some((p) => writes.has(p) || reads.has(p)) || [...writes].some((p) => before2.resources.reads.includes(p)) || before2.resources.unknown && writes.size > 0 || unresolved && before2.resources.writes.length > 0;
      if (conflict)
        dependencies.push({
          from: before2.effectId,
          to: effectId,
          kind: "serialization",
    ...
    if (!Number.isSafeInteger(effectCalls) || effectCalls > s.policy.maxEffects)
      fail32("PEL_LIMIT", n, "Plan exceeds maxEffects");
    let resultSchema = s.registry.dataSchemas[d.resultSchemaId];
    if (d.name === "fm/research" && resultSchema.type === "association") {
      const limit = args2.limit && concrete(args2.limit);
      const maximum = limit?.tag === "number" ? limit.value : 20;
      resultSchema = {
        ...resultSchema,
        fields: resultSchema.fields.map(
          (field) => field.key === "results" && field.schema.type === "list" ? {
            ...field,
            schema: {
              ...field.schema,
    ...
    const result2 = unknown(
      resultSchema,
      n.span,
      /* @__PURE__ */ new Set([effectId]),
      d.resultSchemaId
    );
    if (d.name === "fm/retry" || d.name === "fm/race") {
      const runBody = (body, context22) => {
        if (body?.tag !== "functions" || body.choices.some(
          (f2) => f2.kind !== "user" || f2.spec.kind !== "fixed" || f2.spec.parameters.some(
            (p) => p.required && !Object.hasOwn(f2.bound, p.name)
          )
        ))
          fail32(
            "PEL_DYNAMIC_EFFECT_UNBOUNDED",
        ...
        return call(body, [], n, context22);
      };
      const context4 = {
        ...c,
        controlDeps: /* @__PURE__ */ new Set([...c.controlDeps ?? [], effectId])
      };
      if (d.name === "fm/retry") {
        const count = args2.attempts && concrete(args2.attempts);
        if (count?.tag !== "number" || !Number.isSafeInteger(count.value) || count.value < 1 || count.value > s.limits.maxIterations)
          fail32(
            "PEL_DYNAMIC_EFFECT_UNBOUNDED",
            n,
            "Retry attempts require a positive finite integer within the iteration limit"
          );
        const categories = args2.on && concrete(args2.on);
        if (categories?.tag !== "list" || categories.items.length === 0 || categories.items.some((v) => v.tag !== "key"))
          fail32(
            "PEL_TYPE",
            n,
        ...
        region(
          ...
          "unknown condition",
          count.value,
          (rc) => runBody(args2.body, rc)
        );
      } else {
        if (text2(args2.winner) !== "first-valid")
          fail32("PEL_TYPE", n, "Race winner must be first-valid");
        const tasks = args2.tasks;
        if (tasks?.tag !== "list" || tasks.items.length === 0 || tasks.items.length > s.policy.maxEffects)
          fail32(
            "PEL_DYNAMIC_EFFECT_UNBOUNDED",
            n,
            "Race tasks require a finite nonempty closure list"
          );
        region(n, context4, "unknown callable", 1, (rc) => {
          const values3 = tasks.items.map(
            (body, i) => runBody(body, {
              ...rc,
              ordered: false,
          ...
          return values3.reduce((a, b) => merge7(a, b, n.span));
        });
      }
    }
    if (unresolved && !c.region)
      dynamicRegions.push({
        ...
        models: model ? [{ profileId: model.profileId, transportId: model.transportId }] : [],
        capabilities: d.capabilities,
        resources: {
          reads: s.policy.resourceEnvelope.reads,
          writes: s.policy.resourceEnvelope.writes
  ...
  function sequence(nodes, c) {
    if (c.ordered) {
      let result2 = nil2();
      for (const n of nodes) result2 = ev(n, c);
      return result2;
    }
    ...
    const done7 = /* @__PURE__ */ new Map();
    while (done7.size < nodes.length) {
      let advanced = false;
      for (const row of graph.expressions)
        if (!done7.has(row.index) && row.dependencies.every((i) => done7.has(i))) {
          done7.set(row.index, ev(nodes[row.index], c));
          advanced = true;
        }
      if (!advanced && nodes[0])
        fail32(
    ...
    return done7.get(nodes.length - 1) ?? nil2();
  }
  function invoke2(f, raw, n, c) {
    tick(n, c);
    if (f.spec.kind === "sequence")
      return sequence(
        raw.length === 1 && raw[0]?.kind === "list" ? raw[0].items : raw,
        { ...c, env: scope3(c.env), ordered: f.name === "do" }
      );
    ...
    for (const p of plan.value)
      bound[p.name] = p.evaluation === "strict" ? { kind: "value", value: ev(p.node, c) } : {
        kind: "syntax",
        node: p.node,
        env: c.env,
    ...
    if (missing.length) {
      if (f.name === "+" && missing.length === 1 && bound.x?.kind === "value") {
        const x = bound.x.value, cv = concrete(x);
        if (cv?.tag === "list") {
          const r = pureBuiltin("+", { x: cv }, n.span);
          if (!r.ok) throw new AnalysisFailure(r.error);
          return known(r.value, x.deps);
        }
        if (x.tag === "list" || x.tag === "unknown" && x.schema.type === "list") {
          const items = x.tag === "list" ? x.items : [
            unknown(
      ...
      return { tag: "functions", choices: [{ ...f, bound }], deps: /* @__PURE__ */ new Set() };
    }
    for (const p of f.spec.parameters)
      if (!Object.hasOwn(bound, p.name)) {
        if (p.evaluation === "syntax" && p.defaultExpression)
          ...
          bound[p.name] = {
            kind: "value",
            value: f.defaults[p.name] ?? (p.defaultExpression ? ev(p.defaultExpression, { ...c, env: f.env }) : nil2())
          };
      }
    const child2 = {
      ...c,
    ...
    if (f.kind === "user") {
      ...
      const env3 = scope3(f.env);
      for (const [name2, arg] of Object.entries(bound)) {
        env3.set(name2, val(arg, c));
        localNames.get(env3).add(name2);
      }
      return ev(f.body, {
        ...child2,
        env: env3,
    ...
    if (f.kind === "host")
      return host(
        f,
        Object.fromEntries(
          Object.entries(bound).map(([name2, b]) => [name2, val(b, c)])
        ),
        n,
        child2
      );
    if (f.name === "def") {
      const target = syntax(bound.name, n), rhs = syntax(bound.value, n);
      if (target.node.kind !== "symbol")
        fail32("PEL_TYPE", n, "def name must be a symbol");
      const name2 = target.node.name;
      if (reserved.has(name2) || localNames.get(target.env)?.has(name2))
        fail32("PEL_DUPLICATE_BINDING", n, `Binding ${name2} already exists`);
      if (!localNames.has(target.env)) localNames.set(target.env, /* @__PURE__ */ new Set());
      localNames.get(target.env).add(name2);
      const cell = { tag: "cell", deps: /* @__PURE__ */ new Set() };
      target.env.set(name2, cell);
      const result2 = val(rhs, child2);
      cell.value = result2;
      target.env.set(name2, result2);
      return result2;
    }
    if (f.name === "lambda") {
      const params = syntax(bound.params, n), body = syntax(bound.body, n);
      if (params.node.kind !== "list")
        fail32("PEL_TYPE", n, "lambda requires an argument list");
      const names3 = /* @__PURE__ */ new Set();
      const parameters = [];
      const defaults = {};
      const capture2 = new Map(params.env);
      for (const p of params.node.items) {
        if (p.kind !== "pair" || names3.has(p.key))
          fail32("PEL_ARGUMENT_NAME", n, "Lambda parameters must be unique keys");
        names3.add(p.key);
        parameters.push({
          name: p.key,
      ...
      const checkEnv = new Map(capture2);
      for (const name2 of names3) checkEnv.set(name2, unknown(anySchema, n.span));
      ev(body.node, {
        ...child2,
        env: checkEnv,
        structure: true,
      ...
      return { tag: "functions", choices: [closure2], deps: /* @__PURE__ */ new Set() };
    }
    if (f.name === "if") {
      const condition = val(bound.cond, child2);
      expect(condition, ["boolean"], n);
      const cv = concrete(condition), yes = syntax(bound.then, n), no = syntax(bound.else, n);
      if (cv?.tag === "boolean") {
        const selected = cv.value ? yes : no, dead = cv.value ? no : yes;
        ev(dead.node, {
          ...child2,
          env: scope3(dead.env, true),
          structure: true
        });
        return val(selected, child2);
      }
      return region(
        n,
        {
          ...child2,
          controlDeps: /* @__PURE__ */ new Set([
            ...child2.controlDeps ?? [],
            ...condition.deps
          ])
        },
        ...
        1,
        (rc) => {
          const a = scope3(yes.env, true), b = scope3(no.env, true);
          ...
          const result2 = merge7(av, bv, n.span);
          return { ...result2, deps: deps([result2, condition]) };
        }
      );
    }
    if (f.name === "for") {
      const collection = val(bound.coll, child2), iterator = syntax(bound.iterator, n), body = syntax(bound.body, n);
      if (iterator.node.kind !== "symbol")
        fail32("PEL_TYPE", n, "for iterator must be a symbol");
      const name2 = iterator.node.name;
      expect(collection, ["list", "association"], n);
      const cv = concrete(collection);
      const items = collection.tag === "list" ? collection.items : cv?.tag === "list" ? cv.items.map((v) => known(v, collection.deps)) : void 0;
      const one = (value, context4, index) => {
        if (!context4.structure && (iterations += context4.multiplier ?? 1) > s.limits.maxIterations)
          fail32("PEL_LIMIT", n, "Abstract iteration exceeds maxIterations");
        const env3 = scope3(body.env);
        env3.set(name2, value);
        return ev(body.node, {
          ...context4,
          env: env3,
          path: `${context4.path}/iteration/${index}`
        });
      };
      if (items && !c.structure && items.length * (c.multiplier ?? 1) + iterations > s.limits.maxIterations)
        fail32(
          "PEL_LIMIT",
          n,
      ...
      if (items)
        return {
          tag: "list",
          items: items.map((v, i) => one(v, child2, i)),
          deps: collection.deps
        };
      const schema = collection.tag === "unknown" ? collection.schema : anySchema;
      if (schema.type !== "list")
        fail32(
          "PEL_DYNAMIC_EFFECT_UNBOUNDED",
          n,
      ...
      if (count === 0) {
        one(
          ...
          { ...child2, structure: true },
          0
        );
        return { tag: "list", items: [], deps: collection.deps };
      }
      return region(n, child2, "unknown collection", count, (rc) => {
    ...
    if (f.name === "case") {
      let next2 = function(index, context4) {
        if (index >= pairs.length) return nil2();
        const conditionNode = pairs[index], resultNode = pairs[index + 1];
        const conditionContext = { ...context4, caret: scrut };
        const conditionExpression = !caretUsed(conditionNode, conditionContext) && (conditionNode.kind === "call" || conditionNode.kind === "pipe") ? inject(conditionNode, scrut, conditionContext) : conditionNode;
        let condition = ev(conditionExpression, conditionContext);
        if (condition.tag === "functions")
          condition = applyValues(condition, [scrut], conditionNode, context4);
        else {
          const v = concrete(condition);
          if (conditionNode.kind === "string") {
            const predicate = env2.get("pel/nl-condition");
        ...
        if (cv?.tag === "boolean") {
          if (cv.value) {
            next2(index + 2, {
              ...context4,
              env: scope3(context4.env, true),
              structure: true
            });
            return ev(resultNode, context4);
          }
          ev(resultNode, {
            ...context4,
            env: scope3(context4.env, true),
        ...
        return region(
          ...
          1,
          (rc) => {
            const left3 = scope3(rc.env, true), right3 = scope3(rc.env, true);
            ...
            mergeEnvs(rc.env, left3, right3, n);
            return merge7(a, b, n.span);
          }
        );
      };
      var next = next2;
      const scrut = val(bound.scrut, child2), body = syntax(bound.body, n);
      if (body.node.kind !== "list" || body.node.items.length % 2 !== 0)
        fail32("PEL_TYPE", n, "case requires condition/result pairs");
      const pairs = body.node.items;
      return next2(0, child2);
    }
    const values3 = Object.fromEntries(
      Object.entries(bound).map(([k, b]) => [k, val(b, child2)])
    );
    const numeric = ["+", "-", "*", "/", "pow", "sqrt", "gt", "lt"];
    if (numeric.includes(f.name))
      for (const v of Object.values(values3)) expect(v, ["number"], n);
    if (f.name === "concat")
      for (const v of Object.values(values3)) expect(v, ["string"], n);
    if (f.name === "not") expect(values3.x, ["boolean"], n);
    if (f.name === "len")
      expect(values3.value, ["list", "string", "association"], n);
    const concreteArgs = Object.fromEntries(
      Object.entries(values3).map(([k, v]) => [k, concrete(v)])
    );
    if (Object.values(concreteArgs).every((v) => v !== void 0)) {
      const r = pureBuiltin(
        f.name,
        concreteArgs,
        n.span
      );
      if (r.ok) return known(r.value, deps(Object.values(values3)));
      if (!c.structure || r.error.code !== "PEL_NUMERIC_DOMAIN")
        throw new AnalysisFailure(r.error);
    }
    return unknown(
      ["gt", "lt", "eq", "not"].includes(f.name) ? { type: "boolean" } : f.name === "concat" ? { type: "string", maxBytes: s.limits.maxValueBytes } : {
        type: "number",
        integer: false,
        minimum: -Number.MAX_VALUE,
        maximum: Number.MAX_VALUE
      ...
      n.span,
      deps(Object.values(values3))
    );
  }
  function applyValues(fn2, values3, n, c) {
    const env3 = new Map(c.env);
    const nodes = values3.map((v, i) => {
      const name2 = `#abstract-argument-${i}`;
      env3.set(name2, v);
      return { ...n, kind: "symbol", name: name2 };
    });
    return call(fn2, nodes, n, { ...c, env: env3 });
  }
  function call(fn2, raw, n, c) {
    c = { ...c, controlDeps: /* @__PURE__ */ new Set([...c.controlDeps ?? [], ...fn2.deps]) };
    if (c.structure && fn2.tag === "unknown" && fn2.schema.type === "data") {
    ...
    if (fn2.tag === "cell") {
      if (fn2.value) return call(fn2.value, raw, n, c);
      if (c.structure) {
    ...
    if (fn2.tag === "functions") {
      if (fn2.choices.length === 1) return invoke2(fn2.choices[0], raw, n, c);
      return region(n, c, "unknown callable", 1, (rc) => {
        const values3 = fn2.choices.map(
          (f, i) => invoke2(f, raw, n, {
            ...rc,
            env: new Map(c.env),
    ...
    const cv = concrete(fn2);
    if (fn2.tag === "list" || cv?.tag === "list" || fn2.tag === "unknown" && (fn2.schema.type === "list" || fn2.schema.type === "association")) {
      ...
      const args2 = Object.fromEntries(
        p.value.map((a) => [a.name, ev(a.node, c)])
      );
      const at = args2.at && concrete(args2.at);
      const from = args2.from && concrete(args2.from), to = args2.to && concrete(args2.to);
      if (at && at.tag !== "nil" && (from && from.tag !== "nil" || to && to.tag !== "nil"))
        fail32(
          "PEL_ARGUMENT_MODE",
          n,
          ":at cannot be combined with :from or :to"
        );
      if ((!args2.at || at?.tag === "nil") && (!args2.from || from?.tag === "nil") && (!args2.to || to?.tag === "nil"))
        return fn2;
      for (const v of [args2.from, args2.to])
        if (v && concrete(v)?.tag !== "nil") expect(v, ["number"], n);
      if (at?.tag === "list") {
        const items = at.items.map(
          (selector) => applyValues(fn2, [known(selector)], n, c)
        );
        return {
          tag: "list",
          items,
          deps: deps([fn2, ...items, ...Object.values(args2)])
        };
      }
      if ((!args2.at || at?.tag === "nil") && fn2.tag === "unknown" && fn2.schema.type === "list") {
        for (const index of [from, to])
          if (index?.tag === "number" && (!Number.isSafeInteger(index.value) || index.value < 1 || index.value > fn2.schema.maxItems))
            fail32(
              "PEL_INDEX",
              n,
              "Slice index is outside every admitted collection length"
            );
        const first2 = from?.tag === "number" ? from.value : 1;
        const last3 = to?.tag === "number" ? to.value : fn2.schema.maxItems;
        return unknown(
          {
            ...fn2.schema,
            minItems: 0,
            maxItems: Math.max(0, last3 - first2 + 1)
          },
          n.span,
          deps([fn2, ...Object.values(args2)])
        );
      }
      if ((!args2.at || at?.tag === "nil") && fn2.tag === "list" && Object.values(args2).every((a) => concrete(a))) {
        const indices = callableList(
          fn2.items.map((_, i) => ({ tag: "number", value: i })),
          Object.fromEntries(
            Object.entries(args2).map(([k, v]) => [k, concrete(v)])
          ),
          n.span
        );
        ...
        if (indices.value.tag === "list")
          return {
            ...
            items: indices.value.items.map(
              (i) => fn2.items[i.tag === "number" ? i.value : -1]
            ),
            deps: deps([fn2, ...Object.values(args2)])
          };
      }
      if (fn2.tag === "unknown" && fn2.schema.type === "list" && at?.tag === "number" && (!Number.isSafeInteger(at.value) || at.value < 1 || at.value > fn2.schema.maxItems))
        fail32(
          "PEL_INDEX",
          n,
          "List index is outside every admitted collection length"
        );
      if (fn2.tag === "list" && at?.tag === "key")
        return fn2.items.find(
          (v) => v.tag === "pair" && v.key === at.name && v.value
        )?.tag === "pair" ? fn2.items.find(
          (v) => v.tag === "pair" && v.key === at.name
        ).value : nil2();
      if (fn2.tag === "list" && at?.tag === "number") {
        if (!Number.isSafeInteger(at.value) || at.value < 1 || at.value > fn2.items.length)
          fail32("PEL_INDEX", n, "List index is out of bounds");
        return fn2.items[at.value - 1];
      }
      if (cv?.tag === "list" && Object.values(args2).every((a) => concrete(a))) {
        ...
        if (!r.ok) throw new AnalysisFailure(r.error);
        return known(r.value, deps([fn2, ...Object.values(args2)]));
      }
      return unknown(
        fn2.tag === "unknown" && fn2.schema.type === "list" ? fn2.schema.items : anySchema,
        n.span,
        deps([fn2, ...Object.values(args2)])
      );
    }
  ...
  function ev(n, c) {
    const result2 = evaluateNode(n, c);
    const value = concrete(result2);
    if (value && Buffer.byteLength(canonicalize2(value)) > s.limits.maxValueBytes)
      fail32("PEL_LIMIT", n, "Abstract value exceeds maxValueBytes");
    return result2;
  }
  function evaluateNode(n, c) {
    tick(n, c);
    switch (n.kind) {
      case "number":
      case "string":
      case "boolean":
        return known({ tag: n.kind, value: n.value });
      case "nil":
        return nil2();
      case "key":
      ...
      case "symbol": {
        ...
        if (!v) fail32("PEL_UNBOUND_SYMBOL", n, `Unknown symbol ${n.name}`);
      ...
      case "caret":
        if (!c.caret) fail32("PEL_CARET_SCOPE", n, "Caret has no pipeline value");
        return c.caret;
      case "quote":
        return n.expression.kind === "key" ? known({ tag: "key", name: n.expression.name }) : known({ tag: "syntax", node: n.expression });
      case "pair": {
      ...
      case "call":
        if (!n.items[0]) fail32("PEL_TYPE", n, "Call has no target");
        return call(ev(n.items[0], c), n.items.slice(1), n, c);
      case "pipe": {
        const left3 = ev(n.left, c);
        const context4 = { ...c, caret: left3 };
        return ev(
          caretUsed(n.right, context4) ? n.right : inject(n.right, left3, context4),
          context4
        );
      }
    }
  }
  const env2 = /* @__PURE__ */ new Map();
  for (const [name2, spec] of Object.entries(builtinArgSpecs))
    if (name2 !== "list")
      env2.set(name2, {
        tag: "functions",
        choices: [
  ...
  try {
    const result2 = sequence(program.expressions, {
      env: env2,
      structure: false,
      depth: 0,
      path: "root",
    ...
    return {
      finalValueSummary: summary5(result2),
      effects,
      dependencies,
      dynamicRegions,
      diagnostics: [],
...
var checkedPrograms = /* @__PURE__ */ new WeakSet();
function freezeTree(value) {
  if (typeof value !== "object" || value === null || Object.isFrozen(value))
    return;
  for (const nested2 of Object.values(value)) freezeTree(nested2);
  Object.freeze(value);
}
...
function usage(error, nodes, snapshot) {
  const pending3 = [...nodes];
  let call;
  let symbol3;
  while (pending3.length) {
    const n = pending3.pop();
    if (n.span.start > error.span.start || n.span.end < error.span.end)
      continue;
    if (n.kind === "symbol") symbol3 = n.name;
    if (n.kind === "call") call = n;
    if (n.kind === "call" || n.kind === "list") pending3.push(...n.items);
    else if (n.kind === "pair") pending3.push(n.value);
    else if (n.kind === "pipe") pending3.push(n.left, n.right);
  }
  let name2 = call?.kind === "call" && call.items[0]?.kind === "symbol" ? call.items[0].name : symbol3;
  const catalog = new Map([
    ...Object.entries(builtinArgSpecs),
    ...snapshot.registry.descriptors.map((d) => [d.name, d.argSpec])
  ]);
  let suggested = false;
  if (name2 && !catalog.has(name2)) {
    const distance = (a, b) => {
    ...
    const closest = [...catalog.keys()].map((candidate) => ({ candidate, distance: distance(name2, candidate) })).filter((row) => row.distance <= 2).sort(
      (a, b) => a.distance - b.distance || a.candidate.localeCompare(b.candidate)
    )[0];
    name2 = closest?.candidate;
    suggested = name2 !== void 0;
  }
  const spec = name2 && catalog.get(name2);
  if (!spec) return error;
  const signature = spec.kind === "fixed" ? `(${name2} ${spec.parameters.map((p) => `:${p.name}${p.required ? "" : " [optional]"}`).join(" ")})` : `(${name2} expression...)`;
  return {
    ...error,
    signature,
    help: suggested ? `Did you mean ${name2}? Check this registered signature.` : "Use only named arguments or only positional arguments. Select values admitted by the authoring snapshot."
  };
}
function isCheckedProgramV1(value) {
  return checkedPrograms.has(value) && validateBinding(value.binding, value.source, value.snapshot).ok;
}
function checkPel(input) {
  const snapshot = validateAuthoringSnapshotV1(input.snapshot);
  if (!snapshot.ok) return { tag: "invalid", diagnostics: snapshot.error };
  const source = new Uint8Array(input.source), profile = input.profile ?? snapshot.value.languageProfile;
  if (canonicalize2(profile) !== canonicalize2(snapshot.value.languageProfile))
    return {
      tag: "invalid",
      diagnostics: [
        {
  ...
  const limits2 = snapshot.value.limits;
  const measured = {
  ...
  if (analysis.diagnostics.some((d) => d.severity === "error"))
    return {
      ...
      diagnostics: analysis.diagnostics.map(
        (d) => usage(d, parsed.value.expressions, snapshot.value)
      )
    };
  ...
  const checked = Object.freeze({
    ...
    snapshot: snapshot.value,
    binding,
    bindingDigest: sha256Hex2(canonicalize2(binding)),
...
// packages/orchestration/src/pel-authoring-contract.ts
function authoringFailure(code, message, exitCode = 2, input) {
  return {
...
// packages/orchestration/src/pel-draft-session.ts
import { createHash as createHash4 } from "node:crypto";
var failure2 = (code, message) => ({
  _tag: "DraftFailure",
  code,
  message
});
function makePelDraftSession(input) {
  return Effect_exports.gen(function* () {
    ...
    if (input.source.byteLength > maxBytes)
      return yield* Effect_exports.fail(
        failure2("PEL_LIMIT", "Draft exceeds source byte limit")
      );
    let serial = 0;
    const record2 = (source) => ({
      revision: serial++,
      sourceDigest: createHash4("sha256").update(source).digest("hex"),
      byteLength: source.byteLength,
      source: source.slice()
    });
    const revisions = [record2(input.source)];
    ...
    const allNodes = () => {
      ...
      while (pending3.length) {
        ...
        else if (n.kind === "quote") pending3.push(n.expression);
        else if (n.kind === "pipe") pending3.push(n.left, n.right);
      }
      return result;
    };
    return {
      ...
      check: () => currentCheck,
      nodes,
      history: () => revisions.map(({ revision, sourceDigest, byteLength }) => ({
        revision,
        sourceDigest,
        byteLength
      })),
      ...
      ].filter((x) => x.startsWith(prefix)).sort(),
      replace: (kind, fragment, nodeId) => Effect_exports.gen(function* () {
        if (fragment.byteLength > maxBytes)
          return yield* Effect_exports.fail(
            failure2("PEL_LIMIT", "Fragment exceeds source byte limit")
          );
        let source = fragment.slice();
        if (kind !== "program") {
          const parsed = parsePel(fragment, input.profile);
          if (!parsed.ok || kind === "expression" && parsed.value.expressions.length !== 1)
            return yield* Effect_exports.fail(
              failure2(
                "PEL_PARSE",
          ...
          source = Buffer.concat([
            ...
            fragment,
            kind === "expression" ? current().source.subarray(target.span.end) : new Uint8Array()
          ]);
        }
        if (source.byteLength > maxBytes)
          return yield* Effect_exports.fail(
            failure2("PEL_LIMIT", "Replacement exceeds source byte limit")
          );
        currentCheck = input.check(source.slice());
        revisions.push(record2(source));
        let total = revisions.reduce((sum2, r) => sum2 + r.byteLength, 0);
        while (revisions.length > 100 || total > 16 * 1024 * 1024)
          total -= revisions.shift().byteLength;
        return currentCheck;
      }),
      undo: () => Effect_exports.gen(function* () {
        if (revisions.length < 2)
          return yield* Effect_exports.fail(
            failure2("PEL_DRAFT_HISTORY", "No older revision remains")
          );
        revisions.pop();
...
var SOURCE_LIMIT = 1024 * 1024;
var JSON_LIMIT = 16 * 1024 * 1024;
function parseArgs(argv) {
  ...
  const result = { command, json: false, interactive: false, flags: {} };
  const valueFlags = [
    "--context",
    "--selection",
    "--controls",
  ...
  for (let i = 1; i < argv.length; i++) {
    ...
    if (arg.startsWith("--")) {
      ...
      if (!valueFlags.includes(arg) || argv[i + 1] === void 0 || argv[i + 1].startsWith("--"))
        return authoringFailure(
          "PEL_CLI_USAGE",
  ...
  if (result.file === void 0 === (prompt === void 0))
    return authoringFailure(
      "PEL_CLI_USAGE",
      "Select exactly one source file or --prompt"
    );
  if (command === "check" && (prompt !== void 0 || result.interactive || [
    "--model",
    "--transport",
    "--controls",
    "--credential-profile",
  ...
  if (prompt !== void 0 && (!prompt.trim() || Buffer.byteLength(prompt) > SOURCE_LIMIT || result.interactive))
    return authoringFailure(
      "PEL_CLI_USAGE",
  ...
  if (result.file !== void 0 && !result.interactive && [
    "--model",
    "--transport",
    "--controls",
    "--credential-profile",
  ...
  if (prompt !== void 0 && (!result.flags["--model"] || !result.flags["--transport"]))
    return authoringFailure(
      "PEL_CLI_USAGE",
      "Generation requires exact --model and --transport"
    );
  const grammar = result.flags["--grammar-mode"];
  if (grammar !== void 0 && !["auto", "grammar-required", "envelope"].includes(grammar))
    return authoringFailure("PEL_CLI_USAGE", "Unknown grammar mode");
  return result;
}
var diagnosticFailure = (diagnostics, input) => ({
  ...authoringFailure(
...
function renderAuthoringDiagnostic(file, source, d) {
...
function makeForemanCli(services) {
  return {
    run: (argv) => {
      let source = new Uint8Array();
      let file = "<input>";
      const json2 = argv.includes("--json");
      const readJson = (path) => Effect_exports.gen(function* () {
        const data2 = yield* services.input.read(path, JSON_LIMIT);
        const parsed = yield* Effect_exports.try({
          try: () => parseJsonRejectDuplicateKeys(
            new TextDecoder("utf-8", { fatal: true }).decode(data2)
          ),
      ...
      const execute = Effect_exports.gen(function* () {
        const args2 = parseArgs(argv);
        if ("_tag" in args2) return yield* Effect_exports.fail(args2);
        file = args2.file ?? "<prompt>";
        if (args2.file !== void 0)
          source = yield* services.input.read(args2.file, SOURCE_LIMIT);
        const contextPath = args2.flags["--context"] ?? services.context.defaultSnapshotPath;
        const contextBytes = yield* services.input.read(
          contextPath,
          JSON_LIMIT
        );
        const parsed = yield* Effect_exports.try({
          try: () => parseAuthoringSnapshotV1(
            new TextDecoder("utf-8", { fatal: true }).decode(contextBytes)
          ),
          catch: (e) => authoringFailure("PEL_SCHEMA", String(e), 2, contextPath)
        });
        if (!parsed.ok)
          return yield* Effect_exports.fail(
        ...
        if (args2.flags["--selection"]) {
          const selection = yield* readJson(args2.flags["--selection"]);
          ...
          if (!effective.ok)
            return yield* Effect_exports.fail(
              diagnosticFailure(effective.error, args2.flags["--selection"])
            );
          snapshot = effective.value;
        }
        const generate = (prompt) => Effect_exports.gen(function* () {
          const model = args2.flags["--model"];
          const transport = args2.flags["--transport"];
          if (!model || !transport)
            return yield* Effect_exports.fail(
              authoringFailure(
                "PEL_PROFILE_UNSUPPORTED",
          ...
          if (!resolved.ok)
            return yield* Effect_exports.fail(diagnosticFailure(resolved.error));
          ...
          if (args2.flags["--controls"]) {
            const parsedControls = validateAuthoringControlsV1(
              yield* readJson(args2.flags["--controls"]),
              profile
            );
          ...
          const credential = args2.flags["--credential-profile"] ?? resolved.value.credentialProfileRef;
          if (!snapshot.policy.allowedCredentialProfileRefs.includes(credential))
            return yield* Effect_exports.fail(
              authoringFailure(
                "PEL_PROFILE_UNSUPPORTED",
          ...
          const grammarMode = args2.flags["--grammar-mode"] ?? "auto";
          if (grammarMode === "grammar-required" && profile.grammarSupport !== "qualified")
            return yield* Effect_exports.fail(
              authoringFailure(
                "PEL_PROFILE_UNSUPPORTED",
          ...
          return yield* services.generate({
            prompt,
            snapshot,
            ...
            Effect_exports.mapError((e) => ({
              ...
              ...authoringFailure(
                e.code,
                e.message,
                e.code === "PEL_GENERATION_CANCELLED" ? 4 : ["PEL_PROFILE_UNSUPPORTED", "PEL_SCHEMA"].includes(
                  e.code
                ) ? 2 : 1
              )
            }))
        ...
        if (args2.flags["--prompt"] !== void 0) {
          const generated = yield* generate(args2.flags["--prompt"]);
          yield* services.output.stdout(
            args2.json ? JSON.stringify(generated) + "\n" : generated.pelSource
          );
          if (!args2.json)
        ...
        const check2 = (bytes) => checkPel({ source: bytes, snapshot });
        if (args2.interactive) {
          ...
          }).pipe(Effect_exports.mapError((e) => authoringFailure(e.code, e.message)));
          ...
          while (true) {
            ...
            const step4 = Effect_exports.gen(function* () {
              if (command === "export") {
                yield* services.output.stdout(
              ...
              if (command === "show") {
                yield* services.output.stderr(
                  new TextDecoder().decode(draft.source()) + "\n" + JSON.stringify(
                    draft.nodes().map((n) => ({ nodeId: n.nodeId, span: n.span }))
                  ) + "\n"
                );
                return;
              }
              ...
              if (command === "undo") {
                yield* draft.undo().pipe(
                  Effect_exports.mapError((e) => authoringFailure(e.code, e.message))
                );
              } else if (command === "repair") {
                yield* services.output.stderr(
                  `Provider use requested: ${args2.flags["--model"] ?? "<unselected>"}/${args2.flags["--transport"] ?? "<unselected>"}
`
                );
...
${new TextDecoder().decode(draft.source())}`
                ...
                const kind = command === "replace-expression" ? "expression" : command === "replace-suffix" ? "suffix" : command === "replace-program" ? "program" : void 0;
                if (!kind || extra.length || (kind === "program" ? nodeId !== void 0 : nodeId === void 0))
                  return yield* Effect_exports.fail(
                    authoringFailure(
                      "PEL_CLI_USAGE",
                ...
                yield* draft.replace(kind, new TextEncoder().encode(fragment), nodeId).pipe(
                  Effect_exports.mapError((e) => authoringFailure(e.code, e.message))
                );
              ...
              if (checked.tag === "invalid") {
                ...
                yield* services.output.stderr(
                  JSON.stringify(
                    command === "preview" ? planPel(checked.checked) : {
                      schemaVersion: 1,
                      tag: "ok",
            ...
            yield* step4.pipe(
              Effect_exports.catchAll(
                (e) => services.output.stderr(`${e.code}: ${e.message}
`)
              )
            );
          }
        }
        const result = check2(source);
        if (result.tag === "invalid")
          return yield* Effect_exports.fail(
            diagnosticFailure(result.diagnostics, file)
          );
        if (args2.command === "plan") {
          const preview = planPel(result.checked);
          yield* services.output.stdout(
            JSON.stringify(preview, null, args2.json ? void 0 : 2) + "\n"
          );
        } else
          yield* services.output.stdout(
            args2.json ? JSON.stringify({
              schemaVersion: 1,
              tag: "ok",
...
`
            ...
            if (!json2 && "attemptDiagnostics" in e && "cumulativeUsage" in e)
              yield* services.output.stderr(
                JSON.stringify({
...
// packages/orchestration/src/pel-authoring-main.ts
var nodeAuthoringInput = {
  read: (path, maxBytes) => Effect_exports.scoped(
...
var write = (stream, text2) => Effect_exports.async((resume2) => {
  stream.write(
    text2,
    (error) => resume2(
      error ? Effect_exports.fail(authoringFailure("PEL_OUTPUT", error.message, 1)) : Effect_exports.void
    )
  );
});
var nodeAuthoringOutput = {
  stdout: (text2) => write(process.stdout, text2),
  stderr: (text2) => write(process.stderr, text2)
};
function defaultAuthoringSnapshotPath(moduleUrl = import.meta.url) {
  return fileURLToPath(
    new URL("../assets/pel/default-authoring-snapshot.json", moduleUrl)
  );
}
function runPelAuthoringMain(argv, overrides = {}) {
  return Effect_exports.scoped(
    Effect_exports.gen(function* () {
      let terminal = overrides.terminal;
      if (argv.includes("--interactive") && !terminal) {
        let complete3 = (prefix) => [];
        const reader = yield* Effect_exports.acquireRelease(
          Effect_exports.sync(
            () => createInterface({
              input: process.stdin,
              ...
              historySize: 100,
              completer: (line) => [[...complete3(line)], line]
            })
          ),
          (reader2) => Effect_exports.sync(() => reader2.close())
        );
        ...
        terminal = {
          ...
          readLine: (prompt) => Effect_exports.gen(function* () {
            if (process.stdin.isTTY)
              yield* nodeAuthoringOutput.stderr(prompt);
            const value = yield* Effect_exports.tryPromise({
              try: () => iterator.next(),
    ...
    Effect_exports.catchAll(
      (e) => Effect_exports.gen(function* () {
        yield* nodeAuthoringOutput.stderr(`${e.code}: ${e.message}
...
if (process.argv[1] && basename(process.argv[1]) === "foreman.js" && fileURLToPath(import.meta.url) === process.argv[1]) {
  const controller = new AbortController();
  const cancel = () => controller.abort();
  process.once("SIGINT", cancel);
  try {
    process.exitCode = await Effect_exports.runPromise(
