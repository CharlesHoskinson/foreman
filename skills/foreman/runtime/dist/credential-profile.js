...
function renderCredentialProfileJson(result) {
...
function stripCredentialProfileNodeArgv(argv) {
  let args2 = [...argv];
  if (args2.length > 0 && (args2[0].endsWith("node") || args2[0].endsWith("node.exe") || args2[0].includes("/node") || args2[0].includes("\\node"))) {
    args2 = args2.slice(1);
  }
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("credential-profile"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
...
function parseCredentialProfileArgv(argv) {
  const args2 = stripCredentialProfileNodeArgv(argv);
  if (args2.length === 0) return { _tag: "Invalid" };
  ...
  if (i !== args2.length) return { _tag: "Invalid" };
  const vendorRaw = values3[3];
  if (!isCredentialVendor(vendorRaw)) return { _tag: "Invalid" };
  return {
    _tag: "Ok",
    command,
    stateRoot: values3[0],
    worktreeRoot: values3[1],
    profileId: values3[2],
    vendor: vendorRaw
  };
}
function runCredentialProfileCli(argv, io) {
  return Effect_exports.gen(function* () {
    ...
    const result = parsed.command === "init" ? yield* initProfile(input) : yield* resolveProfile(input);
...
// packages/orchestration/src/credential-profile-main.ts
function startCredentialProfileMain() {
  const pending3 = [];
  const io = {
    writeStdout: (text) => {
  ...
  const program = runCredentialProfileCli(process.argv, io).pipe(
    Effect_exports.provide(liveCredentialProfile)
  );
