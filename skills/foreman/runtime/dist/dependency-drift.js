...
// packages/orchestration/src/dependency-drift.ts
...
import { dirname, isAbsolute as isAbsolute2, join as join5 } from "node:path";
import { fileURLToPath } from "node:url";

// packages/orchestration/src/vendor-preflight-live.ts
import { mkdtempSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { isAbsolute, join as join4, resolve as resolvePath } from "node:path";
...
var inspectVendor = (capability, options) => Effect_exports.gen(function* () {
  ...
  const absoluteResolved = isAbsolute(resolved) ? resolved : resolvePath(resolved);
...
function resolveRepoRoot(url = import.meta.url) {
  ...
  if (normalized.includes("/skills/foreman/runtime/dist/")) {
    return resolveRealPath(join5(dirname(file), "../../../.."));
  }
  ...
  for (let i = 0; i < 8; i += 1) {
    if (existsSync3(join5(dir, "env/reference-manifest.toml"))) {
...
var EXIT_AGREE = 0;
var EXIT_DRIFT = 1;
var EXIT_FAIL_CLOSED = 2;
var MAX_DRIFT_INPUT_BYTES = 1048576;
...
var MSG_NO_DRIFT = "dependencies: no drift (manifest, tool-check and bootstrap agree)";
var MSG_DRIFT_FOOTER = "DEPENDENCY DRIFT -- records disagree. Reconcile all three, then re-run.";
var REL_MANIFEST = "env/reference-manifest.toml";
var REL_BOOTSTRAP = "env/bootstrap-wsl.sh";
function sortedUnique(ids3) {
  return [...new Set(ids3)].sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
}
function providedBy(id) {
  switch (id) {
    case "flock":
      return "util-linux";
    case "timeout":
...
function buildCheckerAuthority(rows) {
  const all4 = /* @__PURE__ */ new Set();
...
function parseManifestTools(text) {
  ...
  const ids3 = sortedUnique(records.map((r) => r.id));
  const requiredIds = sortedUnique(
    records.filter((r) => r.required).map((r) => r.id)
  );
  return { _tag: "Ok", records, ids: ids3, requiredIds };
}
function reconcileDependencyDrift(input) {
  const stdout = [];
  const stderr = [];
...
function stripDriftNodeArgv(argv) {
  let args2 = [...argv];
  if (args2.length > 0 && (args2[0].endsWith("node") || args2[0].endsWith("node.exe") || args2[0].includes("/node") || args2[0].includes("\\node"))) {
    args2 = args2.slice(1);
  }
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("dependency-drift") || args2[0].includes("check-drift"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
function readLabel(result, rel) {
  switch (result._tag) {
    case "Ok":
      return null;
    case "Absent":
...
function runDependencyDrift(argv, io, options = {}) {
  const args2 = stripDriftNodeArgv(argv);
  if (args2.length > 0) {
    io.writeStderr(
      `ERROR invalid arguments: usage: check-drift.sh (no options); unknown: ${args2[0]}
`
    );
    return Effect_exports.succeed(EXIT_FAIL_CLOSED);
  }
  const repoRoot = options.repoRoot ?? resolveDriftRepoRoot(typeof import.meta !== "undefined" ? import.meta.url : void 0);
  ...
  const program = Effect_exports.gen(function* () {
    ...
    const manifestPath = join6(repoRoot, REL_MANIFEST);
    const bootstrapPath = join6(repoRoot, REL_BOOTSTRAP);
...
`);
  ...
  return program.pipe(Effect_exports.provide(layer));
}
function resolveDriftRepoRoot(url) {
  if (url !== void 0) {
    try {
  ...
  let dir = process.cwd();
  for (let i = 0; i < 8; i += 1) {
    if (existsSync4(join6(dir, REL_MANIFEST))) {
      return dir;
    }
...
var isMain = process.argv[1] !== void 0 && (() => {
  try {
...
if (isMain) {
  ...
  Effect_exports.runPromise(runDependencyDrift(process.argv, io)).then(
    async (code) => {
      try {
