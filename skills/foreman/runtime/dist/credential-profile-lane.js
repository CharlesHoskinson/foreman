...
function profileAuthorityDir(stateRoot, profileId) {
  return join4(stateRoot, PROFILES_DIR_NAME, profileId);
}
function profileJsonPath(stateRoot, profileId) {
  return join4(profileAuthorityDir(stateRoot, profileId), PROFILE_JSON_NAME);
}
function profileHomesDir(stateRoot, profileId) {
  return join4(profileAuthorityDir(stateRoot, profileId), HOMES_DIR_NAME);
}
function profileVendorHomeDir(stateRoot, profileId, vendor) {
  return join4(profileHomesDir(stateRoot, profileId), vendor);
}
function absoluteConfigRoot(stateRoot, profileId, configRootRel) {
  const segments = configRootRel.split("/").filter((s) => s.length > 0);
  return join4(stateRoot, PROFILES_DIR_NAME, profileId, ...segments);
}
...
function isAbsolutePathInput(p) {
  return typeof p === "string" && p.length > 0 && !p.includes("\0") && isAbsolute2(p);
}
...
var liveCredentialProfile = liveCredentialProfileFsLayer;
...
import { basename, dirname as dirname2, isAbsolute as isAbsolute4, join as join5 } from "node:path";
...
function checkRecordConsistency(rec) {
  ...
  if (disc === "discoverable") {
    if (rec.resolvedPath === null || !isAbsolute3(rec.resolvedPath)) {
      return vendorPreflightContractFailure("inconsistent_state");
    }
  }
  if (rec.resolvedPath !== null && !isAbsolute3(rec.resolvedPath)) {
...
function decodeVendorPreflightRecordV1(value) {
  ...
  if (obj["resolvedPath"] === null) {
    ...
    if (!isAbsolute3(obj["resolvedPath"])) {
      return vendorPreflightContractFailure("relative_path");
    }
...
function recordIsFullyReady(rec) {
  return rec.facts.discoverable.value === "discoverable" && rec.facts.authenticated.value === "authenticated" && rec.facts.current.value === "current";
}
...
function profilePreflightDirectoryAnchorSupported() {
  ...
  try {
    const st = lstatSync2("/proc/self/fd");
...
function authorityDirChain(filePath) {
  const preflightDir = dirname2(filePath);
  const profileDir = dirname2(preflightDir);
  const profilesRoot = dirname2(profileDir);
  const stateRoot = dirname2(profilesRoot);
...
function requireProfileJson(profileDir, failReason) {
  const path = join5(profileDir, PROFILE_JSON_NAME);
  const kind = observeNoFollow(path);
  if (kind === "missing") {
    throw new ProfilePreflightStoreFailure("absent");
  }
...
function requireProfileBinding(profileJsonPath2, expectedIdentity, wrapper) {
  ...
  try {
    let st;
    try {
      st = lstatSync2(profileJsonPath2);
    } catch (e) {
      const code = e.code;
      if (code === "ENOENT") {
        throw new ProfilePreflightStoreFailure("absent");
    ...
    try {
      fd = openSync3(profileJsonPath2, authorityOpenFlags2());
    } catch (e) {
      const code = e.code;
      if (code === "ENOENT") {
        throw new ProfilePreflightStoreFailure("absent");
...
function ensureAuthorityDirsForWrite(filePath, wrapper) {
  ...
  let preflightFd;
  try {
    const profileLstat = lstatSync2(profileDir);
    ...
    profileFd = openSync3(profileDir, parentFlags);
    ...
    const preflightAnchored = join5(anchor, PROFILE_PREFLIGHT_DIR_NAME);
    let preflightKind;
    try {
      const st = lstatSync2(preflightAnchored);
      if (st.isSymbolicLink()) {
        throw new ProfilePreflightStoreFailure("linked_path");
      }
      if (!st.isDirectory()) {
    ...
    if (preflightKind === "missing") {
      ...
      try {
        mkdirSync2(preflightAnchored, { recursive: false, mode: 448 });
      } catch (e) {
        const code = e.code;
        if (code !== "EEXIST") {
          throw new ProfilePreflightStoreFailure("write_failed");
    ...
    try {
      preflightFd = openSync3(preflightAnchored, parentFlags);
    } catch (e) {
      const code = e.code;
      if (code === "ELOOP" || code === "EMLINK") {
        throw new ProfilePreflightStoreFailure("linked_path");
  ...
  return Effect_exports.try({
    try: () => {
      ...
      try {
        ...
        try {
          fd = openSync3(absolutePath, authorityOpenFlags2());
        } catch (e) {
          const code = e.code;
          if (code === "ENOENT") {
            throw new ProfilePreflightStoreFailure("absent");
        ...
        const pathAfter = lstatSync2(absolutePath);
...
function writeProfilePreflightRecord(absolutePath, wrapper) {
  return Effect_exports.try({
    try: () => {
      ...
      const preflightDir = dirname2(absolutePath);
      ...
      try {
        const existing = lstatSync2(absolutePath);
      ...
      let fileFd;
      try {
        const parentLstat = lstatSync2(preflightDir);
        ...
        parentFd = openSync3(preflightDir, parentFlags);
        ...
        recheckAuthorityDirs(authorityDirs);
        tmpName = `.preflight.${randomBytes2(16).toString("hex")}.tmp`;
        const tmpAnchored = join5(anchor, tmpName);
        const finalAnchored = join5(anchor, finalName);
        fileFd = openSync3(
          tmpAnchored,
        ...
        let offset = 0;
        while (offset < buf.byteLength) {
          const n = writeSync2(fileFd, buf, offset, buf.byteLength - offset);
          offset += n;
        }
        try {
        ...
        if (tmpName !== void 0 && parentFd !== void 0) {
          const anchor = procFdPath(parentFd);
          if (anchor !== null) {
            cleanupTemp2(join5(anchor, tmpName));
          }
        }
...
// packages/orchestration/src/credential-profile-lane-cli.ts
import { isAbsolute as isAbsolute5 } from "node:path";

// packages/orchestration/src/credential-profile-lane.ts
function refuse2(reason) {
  return { _tag: "Refused", reason };
}
function mapProfileRefusal(reason) {
  switch (reason) {
...
function mapStoreFailure(reason) {
  switch (reason) {
    case "absent":
      return "authority_missing";
    case "linked_path":
...
function admitCredentialProfileLane(input) {
  return Effect_exports.gen(function* () {
    const first2 = yield* resolveProfile(input);
    if (first2._tag === "Refused") {
      return refuse2(mapProfileRefusal(first2.reason));
    }
    if (first2._tag !== "Ready") return refuse2("authority_invalid");
    ...
    const read = yield* store.read(
      ...
      {
        profileId: first2.profileId,
        profileIdentity: first2.profileIdentity,
        vendor: first2.vendor
      }
    ).pipe(
      Effect_exports.map((wrapper2) => ({ _tag: "Read", wrapper: wrapper2 })),
      Effect_exports.catchAll(
        (failure) => Effect_exports.succeed({
          _tag: "ReadRefused",
          reason: mapStoreFailure(failure.reason)
        })
      )
    );
    if (read._tag === "ReadRefused") return refuse2(read.reason);
    const wrapper = read.wrapper;
    if (wrapper.profileId !== first2.profileId || wrapper.profileIdentity !== first2.profileIdentity || wrapper.vendor !== first2.vendor) {
      return refuse2("profile_mismatch");
    }
    if (!recordIsFullyReady(wrapper.record)) {
      return refuse2("preflight_not_ready");
    }
    const second = yield* resolveProfile(input);
    if (second._tag === "Refused") {
      return refuse2(mapProfileRefusal(second.reason));
    }
    if (second._tag !== "Ready") return refuse2("authority_invalid");
    if (second.profileId !== first2.profileId || second.vendor !== first2.vendor || second.profileIdentity !== first2.profileIdentity || second.configRoot !== first2.configRoot) {
      return refuse2("profile_mismatch");
    }
    return {
      _tag: "Admitted",
...
// packages/orchestration/src/credential-profile-lane-cli.ts
var CREDENTIAL_PROFILE_LANE_EXIT_OK = 0;
var CREDENTIAL_PROFILE_LANE_EXIT_REFUSED = 1;
var FLAG_ORDER = [
  "--state-root",
  "--worktree",
  "--profile",
...
function stripNodeArgv(argv) {
  let args2 = [...argv];
  if (args2[0]?.match(/(?:^|[\\/])node(?:\.exe)?$/u)) args2 = args2.slice(1);
  if (args2[0]?.includes("credential-profile-lane")) args2 = args2.slice(1);
  return args2;
}
function isSafeArgument(value) {
  return value.length > 0 && !value.startsWith("--") && !value.includes("\0") && !value.includes("\r") && !value.includes("\n");
}
function parseCredentialProfileLaneArgv(argv) {
  const args2 = stripNodeArgv(argv);
  if (args2[0] !== "admit" || args2.length !== 9) {
    return { _tag: "Invalid" };
  }
  const values3 = [];
  let offset = 1;
  for (const flag of FLAG_ORDER) {
    if (args2[offset] !== flag) return { _tag: "Invalid" };
    const value = args2[offset + 1];
    if (value === void 0 || !isSafeArgument(value)) {
      return { _tag: "Invalid" };
    }
    values3.push(value);
    offset += 2;
  }
  const stateRoot = values3[0];
  const worktreeRoot = values3[1];
  const profileId = values3[2];
  const vendor = values3[3];
  if (!isAbsolute5(stateRoot) || !isAbsolute5(worktreeRoot) || !isValidProfileId(profileId) || !isCredentialVendor(vendor)) {
    return { _tag: "Invalid" };
  }
  return {
    _tag: "Ok",
...
function runCredentialProfileLaneCli(argv, io) {
  return Effect_exports.gen(function* () {
...
function startCredentialProfileLaneMain() {
  const pending3 = [];
  const io = {
    writeStdout: (text) => {
  ...
  const layer = Layer_exports.merge(
