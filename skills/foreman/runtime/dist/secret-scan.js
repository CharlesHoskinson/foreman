...
// packages/orchestration/src/secret-scan.ts
...
// packages/orchestration/src/secret-scan.ts
var SECRET_SCAN_SCHEMA_VERSION = 1;
var PRUNE_TOP_LEVEL = /* @__PURE__ */ new Set([".git", ".harness"]);
var FIXTURE_SUBTREE_PREFIX = "tests/fixtures/";
var FIXTURE_DECLARATION_RELPATH = "tests/fixtures/secret-scan-exemptions.json";
var MAX_DIRECTORY_ENTRIES = 2e5;
var MAX_FILES = 1e5;
var MAX_RELATIVE_PATH_BYTES = 4096;
var MAX_FILE_BYTES = 16777216;
var MAX_TOTAL_INSPECTED_BYTES = 268435456;
var MAX_LINE_INSPECTIONS = 5e6;
var MAX_EXEMPTIONS = 256;
var MAX_FIXTURE_DECLARATION_BYTES = 65536;
var DEFAULT_SECRET_SCAN_BOUNDS = {
...
var EXIT_CLEAN = 0;
var EXIT_NOT_CLEAN = 1;
var EXIT_INVALID_ARGUMENTS = 2;
var PEM_LINE_RE = /^[ \t\f\v\r]*-----BEGIN[ \t\f\v\r].*PRIVATE KEY-----[ \t\f\v\r]*$/;
var PRIVATE_KEY_BASENAMES = /* @__PURE__ */ new Set([
  "id_rsa",
  "id_dsa",
  "id_ecdsa",
  "id_ed25519"
...
function renderSecretScanJson(result) {
...
function isRefusedSecretFilename(basename) {
...
function isPemPrivateKeyLine(line) {
  return PEM_LINE_RE.test(line);
}
function isValidAbsoluteRoot(path) {
...
function isUnderFixtureSubtree(posixRel) {
  return posixRel === FIXTURE_SUBTREE_PREFIX.slice(0, -1) || posixRel.startsWith(FIXTURE_SUBTREE_PREFIX);
}
function isSha256HexLower(s) {
  return /^[0-9a-f]{64}$/.test(s);
}
function isSafeExemptionPath(path) {
  if (typeof path !== "string" || path.length === 0) return false;
...
function isPositiveSafeIntegerBound(n) {
  return typeof n === "number" && Number.isSafeInteger(n) && n > 0;
}
function validateSecretScanBounds(bounds) {
  return isPositiveSafeIntegerBound(bounds.maxDirectoryEntries) && isPositiveSafeIntegerBound(bounds.maxFiles) && isPositiveSafeIntegerBound(bounds.maxRelativePathBytes) && isPositiveSafeIntegerBound(bounds.maxFileBytes) && isPositiveSafeIntegerBound(bounds.maxTotalInspectedBytes) && isPositiveSafeIntegerBound(bounds.maxLineInspections) && isPositiveSafeIntegerBound(bounds.maxExemptions) && isPositiveSafeIntegerBound(bounds.maxFixtureDeclarationBytes);
}
function closeDirQuiet(dir) {
...
function readRegularFileUnder(parent, name, maxBytes) {
  ...
  try {
    ...
    if (after3.ino !== opened.ino || after3.dev !== opened.dev || after3.size !== opened.size || after3.mtimeMs !== opened.mtimeMs) {
      return { _tag: "IdentityChanged" };
    }
...
function scanPemLines(bytes, counters, bounds) {
  const text = bytes.toString("latin1");
  let start3 = 0;
  while (start3 < text.length) {
    let end3 = text.indexOf("\n", start3);
    if (end3 < 0) end3 = text.length;
    const raw = text.slice(start3, end3);
    ...
    if (end3 === text.length) break;
    start3 = end3 + 1;
  }
  return null;
}
function tryExempt(posixRel, bytes, exemptions) {
...
function processDirectory(frame, exemptions, counters, bounds) {
  if (frame.posixRel !== "") {
...
function scanWorktreeSync(input) {
...
var liveSecretScan = Layer_exports.succeed(
  SecretScan,
  {
    scan: (input) => Effect_exports.sync(() => scanWorktreeSync(input))
  }
);
function stripSecretScanNodeArgv(argv) {
  let args2 = [...argv];
  if (args2.length > 0 && (args2[0].endsWith("node") || args2[0].endsWith("node.exe") || args2[0].includes("/node") || args2[0].includes("\\node"))) {
    args2 = args2.slice(1);
  }
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("secret-scan"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
function parseSecretScanArgv(argv) {
  const args2 = stripSecretScanNodeArgv(argv);
  if (args2.length !== 1) return { _tag: "Invalid" };
  const worktreeRoot = args2[0];
