...
var transpose = (array3) => {
  return Object.keys(array3[0]).map((column) => array3.map((row2) => row2[column]));
};
...
function vendorPreflightContractFailure(reason) {
  return {
...
function isVendorPreflightContractFailure(v) {
  return typeof v === "object" && v !== null && v[VENDOR_PREFLIGHT_CONTRACT_FAILURE_BRAND] === true && v._tag === "VendorPreflightContractFailure";
}
...
var TABLE_JSON_KEYS = ["schemaVersion", "capabilities"];
...
function decodeNonBlank(v, maxBytes = 4096) {
  ...
  if (hasNul2(v) || v.trim().length === 0) {
    return v.includes("\0") ? vendorPreflightContractFailure("nul_rejected") : vendorPreflightContractFailure("blank_string");
  }
  if (utf8ByteLength2(v) > maxBytes) {
    return vendorPreflightContractFailure("bound_exceeded");
  }
  return v;
}
function decodeStringArray(v, opts) {
  if (!Array.isArray(v)) {
    return vendorPreflightContractFailure("invalid_schema");
  }
  const max5 = opts.maxEntries ?? 64;
  if (v.length > max5) {
    return vendorPreflightContractFailure("bound_exceeded");
  }
...
function decodeVendorCapabilityV1(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return vendorPreflightContractFailure("invalid_schema");
  }
  const obj = value;
  ...
  return {
    ...
    authPositiveMarkers,
    authNegativeMarkers,
    updateMutates: obj["updateMutates"],
    updateCheckArgv,
    loginInstruction,
    installInstruction,
    updateInstruction,
...
function decodeVendorCapabilityTableV1(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return vendorPreflightContractFailure("invalid_schema");
  }
  const obj = value;
  ...
  capabilities.sort(
    (a, b) => a.vendor < b.vendor ? -1 : a.vendor > b.vendor ? 1 : 0
  );
...
function stripCommentsAndBlank(line) {
  let inStr = false;
  let out = "";
...
function parseVendorCapabilitiesFromToml(text) {
...
function mapTomlCapability(t) {
  ...
  const updateCheck = "update_check_argv" in t ? t["update_check_argv"] : null;
  return {
    vendor: t["vendor"],
    cliName: t["cli_name"],
    ...
    updateCheckArgv: updateCheck === void 0 ? null : updateCheck,
...
// packages/orchestration/src/vendor-preflight-embedded.ts
...
function tryGetEmbeddedCapabilityTable() {
  ...
  const table2 = decodeVendorCapabilityTableV1(parsed);
  if (isVendorPreflightContractFailure(table2)) return null;
  return table2;
}
function loadCapabilityTableFromTomlText(text) {
  const table2 = parseVendorCapabilitiesFromToml(text);
  if (isVendorPreflightContractFailure(table2)) {
    throw new Error(
      `vendor capability table invalid: ${table2.reason}`
    );
  }
  return table2;
}
...
function classifyCurrency(versionOutput, versionFloor, versionProbeOutcome) {
...
function classifyClaudeAuth(stdout, stderr, exitCode, outcome) {
...
function classifyCodexAuth(stdout, stderr, exitCode, outcome, negativeMarkers) {
  if (outcome !== "completed") {
    return {
      value: "unknown",
...
${stderr}`;
  ...
  return {
    value: "unknown",
    reason: "codex login status nonzero exit without a recognized signed-out marker"
  };
}
function classifyGrokAuth(stdout, stderr, exitCode, outcome, positiveMarkers, negativeMarkers) {
  if (outcome !== "completed") {
    return {
      value: "unknown",
...
function classifyAuthForVendor(vendor, stdout, stderr, exitCode, outcome, capability) {
  switch (vendor) {
    ...
    default: {
      const _exhaustive = vendor;
      return { value: "unknown", reason: `unsupported vendor ${_exhaustive}` };
    }
  }
}
function decideRemediation(discoverable, authenticated, current, capability) {
...
function makeFact(value, evidenceClass, reason) {
  return { value, evidenceClass, reason };
}
function buildMissingRecord(args2) {
  const evidence = args2.capability.evidenceClass;
  ...
  return {
    ...
    resolvedPath: null,
    reportedVersion: null,
    versionFloor: args2.capability.versionFloor,
    facts: {
      discoverable: makeFact(
        "missing",
        evidence,
        `CLI ${args2.capability.cliName} not found on PATH`
      ),
...
function buildDiscoveredRecord(args2) {
  const evidence = args2.capability.evidenceClass;
  const remediation = decideRemediation(
    "discoverable",
    args2.auth.value,
    args2.currency.value,
    args2.capability
  );
  return {
    schemaVersion: 1,
    vendor: args2.vendor,
    timestamp: args2.timestamp,
    resolvedPath: args2.resolvedPath,
    reportedVersion: args2.currency.reportedVersion,
    versionFloor: args2.capability.versionFloor,
    facts: {
      discoverable: makeFact(
        "discoverable",
        evidence,
        `CLI resolved at ${args2.resolvedPath}`
      ),
      authenticated: makeFact(args2.auth.value, evidence, args2.auth.reason),
      current: makeFact(args2.currency.value, evidence, args2.currency.reason)
    },
    probes: args2.probes,
    remediation
  };
}
function processFailureToProbeOutcome(reason) {
  return reason;
}
...
var VendorPreflightFailure = class {
  ...
  _tag = "VendorPreflightFailure";
};
var PreflightClock = class extends Context_exports.Tag("PreflightClock")() {
};
var livePreflightClock = Layer_exports.succeed(PreflightClock, {
  nowUtcRfc3339: () => Effect_exports.sync(() => {
...
function runProbe(executable, tailArgv, vendorBinding, env) {
  return Effect_exports.gen(function* () {
    ...
    if (either4._tag === "Left") {
      ...
      if (fail8 instanceof ProcessFailure) {
        return {
          _tag: "failed",
          outcome: processFailureToProbeOutcome(fail8.reason),
          exitCode: null
        };
      }
      return {
...
function probeRecord(kind, executable, tailArgv, capture2) {
  const argv = [executable, ...tailArgv];
  if (capture2._tag === "failed") {
    return {
      kind,
      argv,
      outcome: capture2.outcome,
      exitCode: null
    };
  }
  return {
...
var inspectVendor = (capability, options) => Effect_exports.gen(function* () {
  ...
  let versionOutcome = versionProbe.outcome;
  ...
  let authStderr = "";
  let authExit = null;
  let authOutcome = authProbe.outcome;
  if (authCap._tag === "ok") {
  ...
  if (capability.vendor === "claude" && authProbe.outcome === "completed" && auth.value === "unknown" && auth.reason.includes("malformed JSON")) {
    finalAuthProbe = {
      ...authProbe,
  ...
  const probes = [versionProbe, finalAuthProbe];
...
// packages/orchestration/src/tool-check-cli.ts
var TOOL_CHECK_PROFILES = ["soft", "hard", "full", "durable"];
var TOOL_CHECK_LANES = ["grok", "codex"];
var EXIT_READY = 0;
...
function stripToolCheckNodeArgv(argv) {
  ...
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("tool-check"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
function isProfile(v) {
  return TOOL_CHECK_PROFILES.includes(v);
}
function isLane(v) {
  return TOOL_CHECK_LANES.includes(v);
}
function parseToolCheckArgv(argv) {
  const args2 = stripToolCheckNodeArgv(argv);
  let profile = "soft";
  let json = false;
  let out = null;
  let lane = null;
...
function detectWslFromEnv(env, procVersionText) {
  const force = env.FOREMAN_TEST_WSL_FORCE;
  if (force === "1") {
    return {
      isWsl: true,
      overrideNote: "[foreman] TEST OVERRIDE: FOREMAN_TEST_WSL_FORCE=1 forced WSL detection to wsl=1"
    };
  }
  if (force === "0") {
    return {
      isWsl: false,
      overrideNote: "[foreman] TEST OVERRIDE: FOREMAN_TEST_WSL_FORCE=0 forced WSL detection to wsl=0"
    };
  }
...
function checkFsClassFromProbe(path, fstype, mountTarget) {
  ...
  if (ft === "nfs" || ft === "nfs4" || ft === "cifs" || ft === "smb" || ft === "smb3" || ft === "smbfs" || ft === "afs" || ft === "ncpfs") {
    return "network";
  }
  if (probe.startsWith("/mnt/") || mountTarget === "/mnt" || mountTarget.startsWith("/mnt/")) {
    return "mnt-drvfs";
  }
...
function nearestExistingPath(path) {
  ...
  if (!existsSync2(probe)) return process.platform === "win32" ? path : "/";
  return probe;
}
function resolveFsClass(path) {
  return Effect_exports.gen(function* () {
    if (process.platform === "win32") {
    ...
    let target = "";
    if (findmnt) {
      const ft = yield* exec.runCaptured({
        command: findmnt,
      ...
      const tg = yield* exec.runCaptured({
        command: findmnt,
      ...
      if (dfBin) {
        const df = yield* exec.runCaptured({
          command: dfBin,
          args: ["-T", probe],
          timeoutMs: 3e3,
          maxOutputBytes: 8192
        ...
        if (df._tag === "Right") {
          ...
          const parts2 = last3.trim().split(/\s+/);
          if (parts2.length >= 2) {
            fstype = parts2[1] ?? "";
            target = parts2[parts2.length - 1] ?? "";
          }
        }
      }
    }
...
function resolveCommonSkillsRoot(repoRoot2) {
  return Effect_exports.gen(function* () {
    const paths = yield* PathLookup;
...
function probeMkdirOnce(mkdirBin, workParent) {
  return Effect_exports.gen(function* () {
    ...
    if (strace) {
      ...
      const traceRc = r._tag === "Right" ? r.right.exitCode : r.left.reason === "timeout" ? 124 : 1;
      let trace = "";
      let hasTrace = false;
      try {
        if (existsSync3(traceFile)) {
          const st = readFileSync2(traceFile);
      ...
      let verdict2 = "unknown";
      let evidence2 = "syscall";
      let notes2 = "";
      if (!hasTrace) {
        verdict2 = "unknown";
        evidence2 = "syscall";
        notes2 = `tracer did not run (strace exit=${traceRc}; no trace output)`;
      } else if (/mkdir(at)?\([^)]*\/x[^)]*\)\s*=\s*-1\s+EEXIST/.test(trace)) {
        verdict2 = "atomic";
        evidence2 = "syscall";
        notes2 = "mkdir(2) on probe target; kernel returned EEXIST";
      } else if (/statx\(/.test(trace) && !/mkdir(at)?\([^)]*\/x[^)]*\)\s*=\s*-1\s+EEXIST/.test(trace)) {
        if (!/mkdir(at)?\([^)]*\/x/.test(trace)) {
          verdict2 = "non-atomic";
          evidence2 = "syscall";
          notes2 = "userspace statx check; no mkdir(2) EEXIST (TOCTOU)";
        } else {
          verdict2 = "unknown";
          evidence2 = "syscall";
          notes2 = "mkdir syscall observed without clear EEXIST signature on target";
        }
      } else {
        verdict2 = "unknown";
        evidence2 = "syscall";
...
function probeFlockOnce(flockBin, workParent) {
  return Effect_exports.gen(function* () {
    const fsClass = yield* resolveFsClass(workParent);
    if (process.platform === "win32") {
      return {
  ...
  return Effect_exports.gen(function* () {
    ...
    if (mkdirResolved) {
      ...
      for (const r of roots) {
        const once4 = yield* probeMkdirOnce(mkdirResolved, r);
        classVerdict.set(once4.fsClass, once4.verdict);
        notesAcc.push(`${once4.fsClass}:${once4.notes}`);
        if (once4.verdict === "non-atomic") {
          bestVerdict = "non-atomic";
          bestEvidence = once4.evidence;
        } else if (once4.verdict === "atomic" && bestVerdict !== "non-atomic") {
          bestVerdict = "atomic";
          bestEvidence = once4.evidence;
      ...
      const fsCsv = [];
      for (const cl of ["local", "mnt-drvfs", "network", "fuse"]) {
      ...
      if (bestVerdict === "atomic" && (bestEvidence === "syscall" || bestEvidence === "pinned-mechanism")) {
        trustedAtomic = true;
      }
    ...
    if (flockResolved) {
      ...
      let bestEvidence = "flavour";
      for (const r of roots) {
        const once4 = yield* probeFlockOnce(flockResolved, r);
        classVerdict.set(once4.fsClass, once4.verdict);
        notesAcc.push(`${once4.fsClass}:${once4.notes}`);
        if (once4.verdict === "non-atomic") {
          bestVerdict = "non-atomic";
    ...
    for (const row2 of rows) {
      if (row2.mechanism === "mkdir" && row2.verdict === "non-atomic") {
        mkdirNonAtomic = true;
      }
      if (row2.mechanism === "flock" && row2.verdict === "atomic" && (row2.evidence_class === "syscall" || row2.evidence_class === "pinned-mechanism")) {
        flockTrusted = true;
      }
    }
  ...
  return Effect_exports.gen(function* () {
    ...
    switch (id) {
      case "git": {
        ...
        return row("git", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "python3": {
        ...
        let detail = verR ? firstLine2(captureText3(verR)) : "";
        if (!py3) {
      ...
      case "jq": {
        ...
        const r = yield* runCmd(p, ["--version"]);
        return row("jq", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "coreutils": {
        const stdbuf = (yield* whichOrNull("stdbuf")) ?? (yield* whichOrNull("gstdbuf"));
        if (!stdbuf) return row("coreutils", "missing", "");
        const r = yield* runCmd(stdbuf, ["--version"]);
        return row("coreutils", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "bash": {
        const p = yield* whichOrNull("bash");
        if (!p) return row("bash", "missing", "");
        const r = yield* runCmd(p, ["--version"]);
        return row("bash", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "nats-server": {
        const p = yield* whichOrNull("nats-server");
        if (!p) return row("nats-server", "missing", "");
        const r = yield* runCmd(p, ["--version"]);
        return row("nats-server", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "nats-cli": {
        const p = yield* whichOrNull("nats");
        if (!p) return row("nats-cli", "missing", "");
        const r = yield* runCmd(p, ["--version"]);
        return row("nats-cli", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "grok":
      case "codex":
      ...
      case "node": {
        ...
        const r = yield* runCmd(p, ["--version"]);
        return row("node", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "npm": {
        const p = yield* whichOrNull("npm");
        if (!p) return row("npm", "missing", "");
        const r = yield* runCmd(p, ["--version"]);
        return row("npm", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "docker": {
        const p = yield* whichOrNull("docker");
        if (!p) return row("docker", "missing", "");
        const info = yield* runCmd(p, ["info"], 15e3);
        if (info && info.exitCode === 0) {
          ...
          const detail = ver ? firstLine2(captureText3(ver)) : firstLine2(
            captureText3(
      ...
      case "shellcheck": {
        ...
        const r = yield* runCmd(p, ["--version"]);
        const detail = r ? captureText3(r).split("\n").slice(0, 2).join(" ").trim() : "";
      ...
      case "strace": {
        ...
        if (p) {
          const r = yield* runCmd(p, ["--version"]);
          return row("strace", "ok", r ? firstLine2(captureText3(r)) : "");
        }
        return row(
          "strace",
          "missing",
      ...
      case "codespell": {
        ...
        if (p) {
          const r = yield* runCmd(p, ["--version"]);
          if (r && r.exitCode === 0) {
            return row("codespell", "ok", firstLine2(captureText3(r)));
          }
        }
        for (const pyName of ["python3", "python"]) {
          ...
          if (r && r.exitCode === 0) {
            return row(
              "codespell",
              "ok",
              `python3 -m codespell_lib ${firstLine2(captureText3(r))}`
            );
          }
        }
      ...
      case "bun": {
        ...
        const r = yield* runCmd(p, ["--version"]);
        const detail = r ? firstLine2(captureText3(r)) : "";
      ...
      case "pueue": {
        ...
        if (p) {
          const r = yield* runCmd(p, ["--version"]);
          return row("pueue", "ok", r ? firstLine2(captureText3(r)) : "");
        }
        const paths = yield* PathLookup;
      ...
      case "lychee": {
        ...
        if (r && r.exitCode === 0) {
          return row("lychee", "ok", firstLine2(captureText3(r)));
        }
      ...
      case "gh": {
        ...
        const r = yield* runCmd(p, ["--version"]);
        return row("gh", "ok", r ? firstLine2(captureText3(r)) : "");
      }
      case "timeout": {
        const p = (yield* whichOrNull("timeout")) ?? (yield* whichOrNull("gtimeout"));
...
function checkVendorRow(vendor, ctx) {
  return Effect_exports.gen(function* () {
...
function checkSkills(repoRoot2, processEnv, commonSkillsRoot) {
...
function runToolCheck(argv, io2, env) {
  return Effect_exports.gen(function* () {
    ...
    const program = Effect_exports.gen(function* () {
      const clock3 = yield* PreflightClock;
      const now = env.nowUtc?.() ?? (yield* clock3.nowUtcRfc3339()).replace(/\.\d{3}Z$/, "Z");
...
`);
    ...
    return yield* program.pipe(Effect_exports.provide(baseLayer));
  }).pipe(
    Effect_exports.catchAllDefect(
      () => Effect_exports.sync(() => {
        io2.writeStderr("tool-check: internal failure\n");
        return {
          exitCode: EXIT_INVALID_ARGUMENTS,
...
function resolveRepoRoot(url = import.meta.url) {
...
// ../../foreman/packages/event-log/src/failures.ts
var EVENT_LOG_FAILURE_BRAND = Symbol("@foreman/event-log/Failure");

// ../../foreman/packages/event-log/src/run-journal.ts
...
function buildVendorHomeChildEnv(processEnv, vendor, configRoot, platform = process.platform) {
...
function dirOpenFlags() {
  ...
  return fsConstants6.O_RDONLY | c.O_DIRECTORY | c.O_NOFOLLOW;
}
var anchorSupportCache;
function profilePreflightDirectoryAnchorSupported() {
  ...
  try {
    const st = lstatSync4("/proc/self/fd");
...
function authorityDirChain(filePath) {
  const preflightDir = dirname5(filePath);
  const profileDir = dirname5(preflightDir);
  const profilesRoot = dirname5(profileDir);
  const stateRoot = dirname5(profilesRoot);
  return [stateRoot, profilesRoot, profileDir, preflightDir];
}
function captureDirIdentity2(path) {
  try {
...
function requireRealDir(path, failReason) {
  ...
  const id = captureDirIdentity2(path);
  if (id === null) {
    throw new ProfilePreflightStoreFailure(failReason);
  }
  return id;
...
function ensureAuthorityDirsForWrite(filePath, wrapper) {
  ...
  const components = [];
  for (const path of [stateRoot, profilesRoot, profileDir]) {
...
function recheckAuthorityDirs(captured) {
  for (const c of captured.components) {
    ...
    const id = captureDirIdentity2(c.path);
    if (id === null || !identitiesEqual2(id, c.identity)) {
      throw new ProfilePreflightStoreFailure("identity_changed");
    }
  }
  ...
  if (jsonId === null || !identitiesEqual2(jsonId, captured.profileJsonIdentity)) {
    throw new ProfilePreflightStoreFailure("identity_changed");
  }
}
...
function writeProfilePreflightRecord(absolutePath, wrapper) {
  return Effect_exports.try({
    try: () => {
      ...
      recheckAuthorityDirs(authorityDirs);
      raceHook2?.afterCaptureAuthority?.();
      recheckAuthorityDirs(authorityDirs);
      let parentFd;
      let tmpName;
      let fileFd;
      try {
        const parentLstat = lstatSync4(preflightDir);
        ...
        parentFd = openSync5(preflightDir, parentFlags);
        const parentOpened = fstatSync3(parentFd);
        if (!parentOpened.isDirectory() || !identitiesEqual2(parentOpened, parentLstat)) {
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        const pathRecheck = captureDirIdentity2(preflightDir);
        if (pathRecheck === null || !identitiesEqual2(pathRecheck, parentOpened)) {
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        const capturedPreflight = authorityDirs.components[3];
        if (capturedPreflight === void 0 || !identitiesEqual2(parentOpened, capturedPreflight.identity)) {
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        ...
        raceHook2?.afterBindParentDir?.();
        const parentAfterHook = fstatSync3(parentFd);
        if (!parentAfterHook.isDirectory() || !identitiesEqual2(parentAfterHook, parentOpened)) {
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        recheckAuthorityDirs(authorityDirs);
        ...
        let offset = 0;
        while (offset < buf.byteLength) {
          const n = writeSync4(fileFd, buf, offset, buf.byteLength - offset);
          offset += n;
        }
        try {
          fchmodSync2(fileFd, 384);
        } catch {
          if (IS_POSIX2) {
            closeQuiet2(fileFd);
            fileFd = void 0;
            cleanupTemp2(tmpAnchored);
            throw new ProfilePreflightStoreFailure("write_failed");
          }
        }
        if (IS_POSIX2) {
          const mode = fstatSync3(fileFd).mode & 511;
          if (mode !== 384) {
            closeQuiet2(fileFd);
            fileFd = void 0;
            cleanupTemp2(tmpAnchored);
        ...
        closeSync5(fileFd);
        fileFd = void 0;
        const parentBeforeRename = fstatSync3(parentFd);
        if (!parentBeforeRename.isDirectory() || !identitiesEqual2(parentBeforeRename, parentOpened)) {
          cleanupTemp2(tmpAnchored);
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        recheckAuthorityDirs(authorityDirs);
        raceHook2?.beforePublishRename?.();
        const parentPreRename = fstatSync3(parentFd);
        if (!parentPreRename.isDirectory() || !identitiesEqual2(parentPreRename, parentOpened)) {
          cleanupTemp2(tmpAnchored);
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        recheckAuthorityDirs(authorityDirs);
        ...
        if (!parentAfterRename.isDirectory() || !identitiesEqual2(parentAfterRename, parentOpened)) {
          throw new ProfilePreflightStoreFailure("identity_changed");
        }
        recheckAuthorityDirs(authorityDirs);
        try {
...
function lexicalStateRootPreflight(stateRoot, repoRoot2) {
  ...
  if (!isAbsolute10(stateRoot)) {
    return { _tag: "Refuse", reason: "invalid_state_root" };
  }
  if (typeof repoRoot2 !== "string" || repoRoot2.length === 0 || !isAbsolute10(repoRoot2)) {
    return { _tag: "Refuse", reason: "invalid_state_root" };
  }
  const normalizedState = normalizeAbsolutePath(stateRoot);
  const normalizedRepo = normalizeAbsolutePath(repoRoot2);
...
function nearestExistingAncestor(path) {
  ...
  for (; ; ) {
    ...
    missing.unshift(basename2(probe));
    probe = parent;
  }
}
function physicalAbsoluteDir(path) {
  try {
    const st = lstatSync5(path);
    if (st.isSymbolicLink()) {
      return normalizeAbsolutePath(realpathSync5(path));
    }
...
function ensureExternalStateRoot(stateRoot, repoRoot2) {
  try {
    ...
    const physicalRepo2 = physicalAbsoluteDir(repoRoot2);
    if (physicalState === null || physicalRepo2 === null) return false;
    if (isEqualOrDescendant(physicalState, physicalRepo2)) return false;
    if (isEqualOrDescendant(stateRoot, normalizeAbsolutePath(repoRoot2))) {
      return false;
    }
    return true;
  } catch (e) {
...
`
      ...
      if (resolved.profileId !== b.profileId || resolved.vendor !== b.vendor || resolved.profileIdentity !== b.profileIdentity || resolved.configRoot !== b.configRoot) {
        io2.writeStderr(
...
`
    ...
    for (const pending4 of pendingWrites) {
      const preWrite = yield* resolveProfile({
        stateRoot: foremanHome,
        worktreeRoot: env.repoRoot,
        profileId: pending4.profileId,
        vendor: pending4.vendor
      }).pipe(Effect_exports.provide(profileFsLayer));
...
`
      ...
      if (preWrite.profileId !== pending4.profileId || preWrite.vendor !== pending4.vendor || preWrite.profileIdentity !== pending4.profileIdentity || preWrite.configRoot !== pending4.configRoot) {
        io2.writeStderr(
          `${MSG_CREDENTIAL_PROFILE_REFUSED} (identity_changed)
`
        );
...
var repoRoot = resolveRepoRoot(import.meta.url);
var table;
try {
