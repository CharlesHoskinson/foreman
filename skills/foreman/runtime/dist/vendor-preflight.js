...
var MSG_INVALID_ARGUMENTS = "vendor-preflight: invalid arguments";
var MSG_UNCONFIGURED_VENDOR = "vendor-preflight: vendor is not configured";
var MSG_BOUNDARY_FAILURE = "vendor-preflight: boundary failure";
var MSG_INTERNAL_FAILURE = "vendor-preflight: internal failure";
function stripPreflightNodeArgv(argv) {
  let args2 = [...argv];
  if (args2.length > 0 && (args2[0].endsWith("node") || args2[0].endsWith("node.exe") || args2[0].includes("/node") || args2[0].includes("\\node"))) {
    args2 = args2.slice(1);
  }
  if (args2.length > 0 && (args2[0].endsWith(".js") || args2[0].endsWith(".ts") || args2[0].includes("vendor-preflight") || args2[0].includes("vendor-preflight-main") || args2[0].includes("vendor-preflight-cli"))) {
    args2 = args2.slice(1);
  }
  return args2;
}
...
function runVendorPreflightCli(argv, io2, env) {
  return Effect_exports.gen(function* () {
    ...
    if (capability === null) {
      io2.writeStderr(MSG_UNCONFIGURED_VENDOR + "\n");
      return EXIT_INVALID_ARGUMENTS;
    }
    ...
    const either4 = yield* inspectFn(capability).pipe(
      Effect_exports.provide(baseLayer),
...
// packages/orchestration/src/vendor-preflight-main.ts
...
var table = tryGetEmbeddedCapabilityTable();
if (table === null) {
  pending3.push(
    writeFully(
      process.stderr,
