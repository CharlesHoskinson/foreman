...
The architecture is approved and recorded as an OpenSpec change. The versioned schemas, pure domain foundation, ACE Profile 1 parser and semantic linter, Effect prompt compiler, provider-schema lowerer, Node prompt-materialization boundary, scoped schema-file materializer (POSIX/WSL supported; native Windows refused pending ACL-aware privacy), bounded process runner with optional stdin transport, provider-health canary service, ready-token issuance, Grok canary adapter, Claude canary adapter, Codex canary adapter, and bundled TypeScript preflight CLI are implemented. The complete local quality gate covers these components. The hosted root Linux and Windows gates validate Council under Node 24 without provider credentials or live provider calls.

Council is not ready for live release decisions. The durable runtime, review coordinator, MCP server, Gemini adapter, and native host plugins are not implemented. Bounded xAI, Anthropic, and OpenAI preflight canaries passed on the provisional v0.2.9.0 implementation candidate. Exact-final-candidate receipts and an admissible Council quorum remain required before release.
