...
The architecture is approved and recorded as an OpenSpec change. The versioned schemas, pure domain foundation, ACE Profile 1 parser and semantic linter, Effect prompt compiler, provider-schema lowerer, Node prompt-materialization boundary, scoped schema-file materializer (POSIX/WSL supported; native Windows refused pending ACL-aware privacy), bounded process runner with optional stdin transport, provider-health canary service, ready-token issuance, Grok canary adapter, Claude canary adapter, and Codex canary adapter are implemented. The complete local quality gate covers these components. The existing hosted root Linux and Windows gates now validate Council under Node 24 (install, `pnpm check`, and strict OpenSpec-all) without provider credentials or live provider calls.

Council is not ready for live release decisions. The TypeScript preflight CLI, durable runtime, MCP server, Gemini adapter, and native host plugins are not implemented. A bounded xAI smoke test on 2026-08-03 confirmed an earlier boundary: prompt compilation passed, but the provider returned no admissible structured output. See [the xAI live-smoke record](docs/evidence/2026-08-03-xai-live-smoke.md).
...
- `packages/platform-node/`: Node process runner, prompt materializer, schema-file materializer (POSIX/WSL; native Windows fail-closed), digests, and artifact helpers.
- `packages/adapter-grok/`: Grok canary adapter (file prompt, inline schema, `stdin: null`).
- `packages/adapter-claude/`: Claude canary adapter (stdin prompt bytes, inline schema).
- `packages/adapter-codex/`: Codex canary adapter (stdin prompt bytes, file schema).
