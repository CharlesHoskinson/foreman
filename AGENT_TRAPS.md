...
| A backend seam keyed on `FM_SESSION_CMD` reads false under the entire Bats suite while every golden stays green | That variable holds the **invocation command** (`tests/session.bats:13`) and is named in v0.3.0 exit predicate 3. It is not a backend selector. Use a distinct name. `FM_SESSION_BACKEND` was this migration's. |
| After flipping a default, a two-arm test suite silently runs the same arm twice | The "old default" arm selected itself by *deleting* the environment variable, which is only correct while unset means the old value. Set the value explicitly on every arm. Measured here: flipping the default with the delete still in place produced 24/30, with exactly the six labeled-legacy arms emitting port output. |
| An entire golden corpus goes red at once with no code change | With both `NO_COLOR` and `FORCE_COLOR` set, Node writes `Warning: The 'NO_COLOR' env is ignored due to the 'FORCE_COLOR' env being set.` onto every child's stderr, and goldens compare stderr exactly. Strip `NO_COLOR`, `FORCE_COLOR`, `CLICOLOR` and `CLICOLOR_FORCE` in the test harness rather than inheriting the operator's shell. |
| Running `fm-session` from a worktree silently migrates the store in the **main** checkout | `repoRoot()` resolves through `git rev-parse --git-common-dir`, which from a worktree points at the main checkout's `.git`. Even `fm-session --help` triggers it. It bootstraps before dispatch. This fired in this migration: a reviewer's exploratory `--help` migrated the live store, 93 rows. Pin `FOREMAN_SESSION_DB` under `/tmp` on every invocation, including exploratory ones. |
| `bats tests/session.bats` is 29/29 while the change under test is not exercised at all | That suite runs the compiled bundle at `skills/foreman/runtime/dist/fm-session.js`, not the TypeScript. Measured here: two tasks shipped without a rebuild, so the bundle carried zero occurrences of the new symbols while source carried 3/3/9/5/1, and `verify-runtime` had been failing with `fm-session drift` for two commits before anyone measured it. Run `npm run build && npm run verify-runtime` on any task touching `packages/orchestration/src`, and stage the rebuilt bundle. |
