...
- [ ] 1.9 Create the root contract, write eleven child briefs with exact-path or `/**` allowed paths equal to each package's declared scope, register the family authority with the audit and user receipts, and activate the family. Record the contract id and digests as SessionDB facts.
...
- [ ] 8.3a Re-run `lane-runtime-typescript` task 7 on the candidate and store the three receipts under `docs/research/v050/` with the candidate commit in each.
