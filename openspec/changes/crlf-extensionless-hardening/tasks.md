...
- [ ] **1. `.gitattributes` catch-all** — add `* text=auto eol=lf` to root
  ...
  intent.
- [ ] **2. Explicit path rules for the three offenders** — add path-specific
...
- [ ] **3. Exec-bit fix** — `git update-index --chmod=+x` the same three
...
- [ ] **4. Renormalize** — run `git add --renormalize .`; diff the result to
...
- [ ] **5. `tests/line-endings.bats`** — assert, for every tracked file whose
...
- [ ] **6. Live WSL confirmation** — re-run all three previously-broken
