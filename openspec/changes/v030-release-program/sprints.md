...
|---:|---|---|
| 0 | Authority, baseline, reconciliation, and destruction inventory | Release truth is current. `SpecCorrectnessV1` reports full mapping and zero defects. |
...
| 2 | Typed `@foreman/event-log` foundation | Closed event-log contracts pass. The accepted Sprint 2 commit is available to dependents. |
...
| 13 | `@foreman/knowledge` and Graphify convergence | Current Graphify CLI and version are pinned. One immutable generation has no stale current-authority references. |
