...
| 3 | Queue, attempt identity, report freshness, resume, external runtime state, credentials, and fixture-aware scans | Queue admission module and thin `lane-queue.sh` adapter are complete (reliable ensure-to-add, fixed topology, quoting). Attempt propagation, report freshness, round-preserving resume, external runtime state, credentials, and fixture-aware secret scanning remain open. |
