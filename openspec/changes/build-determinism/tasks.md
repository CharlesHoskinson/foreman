...
- [ ] 2. RED: three fixtures: one hidden lockfile with a different version, one with a missing required package, one missing only a platform-optional package. Expected: the first two assert `lockfile_mismatch` and fail, the third asserts pass and fails. GREEN: implement the expected-set derivation and the tuple comparison. Expected: all three pass. Commit.
