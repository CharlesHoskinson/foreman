...
- [ ] Create `formal/run-checks.sh` with a single classifier that maps checker
      output to `VIOLATED` / `HOLDS` / `ERROR`.
- [ ] Match anchored outcome lines only: `^\[violation\]` and `^\[ok\]`. No
...
- [ ] Output matching neither anchor is `ERROR`, never `PASS`. A crash, a
      timeout or a changed output format is an unknown, and unknowns are not
      green.
- [ ] Positive control: `formal/fixtures/` holds one known-violating and one
...
- [ ] `lane_lifecycle`: `init_postfix`/`step_shipped_resume_bug` REACHABLE for
...
- [ ] The manifest is never generated from a run. Regeneration is not a
      supported operation.

## T3 — the runner

- [ ] `formal/run-checks.sh` executes every gating row and compares observed to
      expected in both directions.
- [ ] A VIOLATED-expected row observed to hold fails the run and reports the
      model as having lost its discriminating power.
- [ ] A HOLDS-expected row observed to violate fails the run and reports a
      regression in the modelled fix.
- [ ] Refuse to record a result against a named configuration when no explicit
      entrypoint was supplied; report which entrypoint was exercised instead.
- [ ] Pin and assert Quint 0.32.0 and Apalache 0.56.1; fail naming both
      versions on mismatch.
- [ ] Resolve the checker through a stable absolute path, not the `fnm`
      multishell path (`/run/user/0/fnm_multishells/...`) that does not survive
      a new shell; an absent checker fails, it does not skip.
- [ ] Emit a machine-readable per-row report for CI upload.

## T4 — vacuous-predicate registry

- [ ] Create `formal/vacuous-predicates.tsv`: predicate, property it cannot
      answer, property that can, evidence.
- [ ] Seed `rework_rounds_bounded` → cannot answer UNVERIFIED-loop termination
...
- [ ] Cross-reference the correction in `three-outcome-verdicts`: the loop
...
- [ ] Gate: a change touching a covered file with no model or manifest change
...
- [ ] Record M1's `eventually_terminal` as a no-fairness stuttering artifact,
      explicitly not a liveness defect.

## T7 — CI and operational discipline

- [ ] Add `.github/workflows/formal.yml`: typecheck all three models plus the
      per-commit manifest tier; upload the per-row report.
- [ ] Tier the manifest so the per-commit set fits the CI budget — measured
      Apalache runtimes span 7.7 s to 385.8 s per configuration — and schedule
      the deep-bound tier separately.
- [ ] Ban `pkill -f` in `formal/` and in lane recipes; terminate an owned PID
...
- [ ] All three models typecheck under the pinned Quint 0.32.0.
...
- [ ] Positive control: feed the classifier a `[ok] No violation found` line
...
- [ ] `openspec validate formal-model-suite --strict` passes.
