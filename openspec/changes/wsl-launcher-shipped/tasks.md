...
- [x] **1. Build-if-absent step** — add an idempotent
...
- [x] **2. `foreman-launch` readiness entry** — add the tool entry to
...
- [x] **3. Degraded-alert hint** — add a Setup-actionable hint (the build
...
- [x] **4. Docs** — update `docs/INSTALL.md`, the POSIX-cascade
  ...
  fallback command.
- [x] **5. Bats** — a fixture simulating a fresh WSL clone: build-if-absent
