...
- [ ] Report the coreutils flavour and the SHA-256 digest of `mkdir` as
      inventory rows.
- [ ] Write the probe row into the JSON inventory (`--json --out`): absolute
      ...
      never writes it (T14).
- [ ] Probe `flock` on the same schema, not only `mkdir`: an available `flock`
...
- [ ] Evidence classes are exactly `syscall`, `pinned-mechanism`, `contention`
...
- [ ] INFO when non-atomic but `flock` present and trusted for the filesystem
...
- [ ] Degrade honestly where no tracer is available: report the weaker class as
...
- [ ] Add the **pinned atomicity register**: one entry per pinned primitive
...
- [ ] Document that entries are added only from a trace taken on a
...
- [ ] Add `fm_lock_reclaim <lock>` to the helper: reclaims exactly the named
      lock, never a sweep, usable while a run is live.
- [ ] Enumerate the locks in scope and their reclaiming process: `.seq.lock`
      and `.attempt.lock` at `el_init`; `.nats-bridge.lock` at bridge start;
      `worktrees/.index.lock` at `wt-new.sh` start.
- [ ] Owner token records the holder's PID **and** its process start time, so a
      reused PID cannot be mistaken for a live holder.
- [ ] Reclaim only when the recorded holder is provably dead. If liveness
      cannot be determined, refuse to reclaim and record the refusal -- deleting
      a live holder's lock is worse than a wedged lock.
- [ ] Keep reclamation conditional on the mechanism the helper selected; never
...
- [ ] Record a reclamation event naming the lock and the dead holder; never
      reclaim silently.
...
- [ ] State the flat rule in `lib/lock.sh`'s header: no foreman lock is held
      while another is acquired. Do **not** state an ordering.
- [ ] Refuse nesting at runtime: an acquisition requested while this process
      holds a foreman lock exits non-zero with `FM_LOCK_NESTED`.
- [ ] Repeat the flat rule in `references/durable-lanes.md`'s locking section
      (T3 already touches that file); remove any ordering sentence.
- [ ] Do not add nesting support to the helper. A future change that genuinely
      needs two locks amends the requirement in its own package, with its own
      deadlock argument.
- [ ] Test: a nested acquisition attempt is refused and the outer lock is still
...
- [ ] Helper initialization reads the verdict from
      `${FOREMAN_TOOL_CHECK_JSON:-${HOME}/.foreman/last-tool-check.json}` --
      the record `env/bootstrap-wsl.sh:411` already writes.
- [ ] Trust exactly two evidence classes: `syscall` (a trace on **this** host
...
- [ ] Treat `non-atomic`, `unknown`, an absent record, and a digest matching no
      register entry as untrusted. No caller may promote a degraded verdict, and
      a **version-string match is not a digest match**.
- [ ] Currency check, all six conditions: same absolute resolved path, same
...
- [ ] Compute the filesystem class for the directory that will hold the lock --
      ...
      classes.
- [ ] Absent, unreadable, unparsable, stale or mismatched record -> run one
...
- [ ] Refuse per the ordered causes in T15: no trusted verdict of either polarity
...
- [ ] Resolve mechanism selection and verdict trust once, at initialization,
...
- [ ] Document the pinning procedure in `references/durable-lanes.md` (T3 already
      touches that file) so an unpinned host has a named route back to
      availability rather than a permanent exclusion.
- [ ] State, per host class, whether durable lanes are available and on what
...
- [ ] Test: force the `mkdir` fallback on a host whose probe returns a trusted
...
- [ ] Test: a pinned digest with a covered filesystem class -> the fallback is
...
- [ ] Define the refusal vocabulary in `lib/lock.sh` as exactly six codes:
...
- [ ] Implement the causes as an ordered chain in exactly that order, first
      matching guard wins, so the causes are made disjoint by the code path rather
      than of a comment.
- [ ] Guard 3 (`FM_LOCK_NO_ATOMIC_PRIMITIVE`) fires only when a **trusted**
...
- [ ] `FM_LOCK_FS_UNSUPPORTED`: the lock path's filesystem class is covered by no
      ...
      locking.
- [ ] `FM_LOCK_UNAVAILABLE` is the **residual** code and makes the enum total:
...
- [ ] Every refusal has one shape, **scoped to the refused acquisition**: that
...
- [ ] Trust and filesystem causes are decided at initialization, before any spin;
      ...
      implied.
- [ ] Audit every call site: no caller distinguishes between causes by
      continuing into the critical section.
- [ ] Test: each of the six causes produces the same observable refusal shape,
...
- [ ] Test: `flock` present, lock path forced onto an uncovered filesystem class
      -> `FM_LOCK_FS_UNSUPPORTED` before any acquisition attempt.
- [ ] Test: a nested acquisition is refused and the outer lock is still held and
