...
- [ ] `tools/lanectl.sh` with `launch` / `adopt` / `ps` / `reap` / `sweep`.
- [ ] Ownership recorded redundantly: `FM_LANE_OWNER` and `FM_LANE_LABEL` in
...
- [ ] `adopt` claims the whole descendant subtree, not the named pid alone.
- [ ] `ps` lists the caller's lanes by default and foreign ones only under
      `--all`, always labelled with their owner.
- [ ] `reap` acts on the calling owner's lanes ONLY. A foreign or unattributable
      process is listed and never signalled.
- [ ] Kill by recorded pid. `pkill -f` is forbidden — it has previously matched
      its own command line and killed the shell issuing it.

## T2 — state-based liveness

- [ ] `tools/reap-stale-lanes.sh` judges on `STAT` and CPU-since-start.
- [ ] `STAT` beginning `T` -> `SUSPENDED`. Zero CPU after grace -> `WEDGED`.
- [ ] Candidates restricted to processes with a `timeout` ancestor, so
      interactive sessions are structurally excluded rather than excluded by
      heuristic.
- [ ] Record inline, where the predicate lives, that a CPU-delta hang check was
...
- [ ] Replace every `pgrep`-existence liveness check in `watch.sh` and
      `lane-supervise.sh`.

## T3 — stall taxonomy

- [ ] Distinguish `SUSPENDED`, `NEVER_LAUNCHED`, `NO_OUTPUT`, `WEDGED`, each
      named by the evidence that produced it.
- [ ] `NEVER_LAUNCHED`: no vendor process after grace. Report the vendor process
      searched for and not found.
- [ ] `NO_OUTPUT`: deliverable set unchanged by content hash — not by
      `git status --porcelain`, which collapses an untracked directory to one
      line and is blind to content edits inside it.
- [ ] Every stall report names its evidence; "not responding" alone is not a
      permitted state.
...
- [ ] Prove `SUSPENDED` reachable: `SIGSTOP` a stub lane, assert the supervisor
      reports SUSPENDED and does NOT report it alive.
- [ ] Prove the pgrep regression: assert an existence-only predicate would have
      called the same stopped process alive. This is the defect under test.
- [ ] Prove `NEVER_LAUNCHED` reachable with a lane whose vendor never starts.
- [ ] Prove foreign safety: a process owned by another owner is listed and NOT
      signalled by reap, asserted by the process still being alive afterwards.
- [ ] Prove subtree adoption: adopt a wrapper+child pair by the wrapper pid and
      assert both are attributed.
- [ ] Prove the healthy-lane negative: a lane blocked awaiting a model response,
...
- [ ] `openspec validate lane-ownership-and-reaping --strict`.
